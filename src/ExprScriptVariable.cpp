...
std::string
ExprScriptVariable::getValue(DtaScriptVariable& visibility) const
{
...
ExprScriptVariable::clone() const
{
  if ((_pNext != NULL) || (_pExpression != NULL) || (_pPackage != NULL) ||
      (_iAccess != ATTRIBUTE_ACCESS)) {
    throw UtlException("internal error: ExprScriptVariable::clone() not "
...
ExprScriptVariable::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  if (isEvaluation()) {
    theCompilerEnvironment.hasEvaluatedExpressionInScope(true);
    if (theCompilerEnvironment.getCurrentClause() != NULL)
      CW_BODY_STREAM << "_compilerClauseScope.getEvaluatedNode(";
    else
      CW_BODY_STREAM << "CGRuntime::getThisTree().getEvaluatedNode(";
...
bool
ExprScriptVariable::compileCppBoolean(
...
bool
ExprScriptVariable::compileCppInt(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
...
bool
ExprScriptVariable::compileCppDouble(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
...
bool
ExprScriptVariable::compileCppString(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
...
void
ExprScriptVariable::compileCppFollowing(
  CppCompilerEnvironment& theCompilerEnvironment,
  bool bFirstCall) const
{
  ...
  if (isEvaluation()) {
    ...
    ExprScriptConstant* pConstant =
      dynamic_cast<ExprScriptConstant*>(_pExpression);
    bool bResolved = (pConstant != NULL);
    if (pConstant != NULL) {
      std::string sConstant = pConstant->getConstant();
      if ((sConstant == "front") || (sConstant == "0"))
        CW_BODY_STREAM << ".getFirstArrayNode()";
      else if (sConstant == "back")
        CW_BODY_STREAM << ".getLastArrayNode()";
      else if (sConstant == "parent")
        CW_BODY_STREAM << ".getParentNode()";
      else if (sConstant == "root")
        CW_BODY_STREAM << ".getRootNode()";
      else
        bResolved = false;
    }
    if (!bResolved) {
...
void
ExprScriptVariable::compileCppForGetOrCreateLocal(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  if (isEvaluation()) {
    theCompilerEnvironment.hasEvaluatedExpressionInScope(true);
    if (theCompilerEnvironment.getCurrentClause() != NULL)
      CW_BODY_STREAM << "_compilerClauseScope.getOrCreateLocalEvaluatedNode(";
    else
      CW_BODY_STREAM
        << "CGRuntime::getThisTree().getOrCreateLocalEvaluatedNode(";
    _pExpression->compileCppString(theCompilerEnvironment);
    CW_BODY_STREAM << ")";
  } else if (_sName == "this") {
    CW_BODY_STREAM << "CGRuntime::getThisTree()";
    ...
    CW_BODY_STREAM << "CGRuntime::getThisTree().getOrCreateLocalNode(\""
                   << _sName << "\")";
  }
  if (isArray()) {
...
void
ExprScriptVariable::compileCppForSet(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  if (isEvaluation()) {
    theCompilerEnvironment.hasEvaluatedExpressionInScope(true);
    if (theCompilerEnvironment.getCurrentClause() != NULL)
      CW_BODY_STREAM << "_compilerClauseScope.insertClassicalEvaluatedNode(";
    else
      CW_BODY_STREAM << "CGRuntime::getThisTree().insertEvaluatedNode(";
    _pExpression->compileCppString(theCompilerEnvironment);
    CW_BODY_STREAM << ")";
  } else if (_sName == "this") {
    CW_BODY_STREAM << "CGRuntime::getThisTree()";
...
void
ExprScriptVariable::compileCppForBNFSet(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  if (isEvaluation()) {
    theCompilerEnvironment.hasEvaluatedExpressionInScope(true);
...
void
ExprScriptVariable::compileCppFollowingForSet(
  CppCompilerEnvironment& theCompilerEnvironment,
  bool bFirstCall) const
{
  if (!_sName.empty() && !bFirstCall) {
...
bool
ExprScriptVariable::compileCppExpr(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  char tcNumber[32];
  ...
  sVariable += tcNumber;
  std::string sDeclaration = "\tstatic CppParsingTree_varexpr " + sVariable +
...
std::string
ExprScriptVariable::toString() const
{
  std::string sText;
  if (isEvaluation()) {
    ...
    if (_pPackage != NULL)
      sText = _pPackage->getName() + "::" + _sName;
    else
      sText = _sName;
    if (isArrayKey()) {
      ...
      if ((sPosition == "front") || (sPosition == "back") ||
          (sPosition == "parent") || (sPosition == "root"))
        sText += "#" + sPosition;
      else
        sText += "#[" + sPosition + "]";
    }
...
std::string
ExprScriptAlienVariable::getValue(DtaScriptVariable& visibility) const
{
  throw UtlException("internal error: an alien variable expression shouldn't "
                     "be executed in CodeWorker!");
}

ExprScriptExpression*
ExprScriptAlienVariable::clone() const
{
  return new ExprScriptAlienVariable(_sName.c_str(), _sTargetLanguage);
}

EXPRESSION_RETURN_TYPE
ExprScriptAlienVariable::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
...
bool
ExprScriptAlienVariable::compileCppBoolean(
  CppCompilerEnvironment& theCompilerEnvironment,
  bool bNegative) const
{
  compileCpp(theCompilerEnvironment);
...
bool
ExprScriptAlienVariable::compileCppInt(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
  return true;
...
bool
ExprScriptAlienVariable::compileCppDouble(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
  return true;
...
bool
ExprScriptAlienVariable::compileCppString(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
  return true;
...
void
ExprScriptAlienVariable::compileCppFollowing(
  CppCompilerEnvironment& theCompilerEnvironment,
  bool bFirstCall) const
{
  throw UtlException(
    "internal error: an alien variable expression can't be navigable!");
}

void
ExprScriptAlienVariable::compileCppForGetOrCreateLocal(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
}
...
void
ExprScriptAlienVariable::compileCppForSet(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
}
...
void
ExprScriptAlienVariable::compileCppForBNFSet(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  compileCpp(theCompilerEnvironment);
}
...
void
ExprScriptAlienVariable::compileCppFollowingForSet(
  CppCompilerEnvironment& theCompilerEnvironment,
  bool bFirstCall) const
{
  compileCpp(theCompilerEnvironment);
...
bool
ExprScriptAlienVariable::compileCppExpr(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  throw UtlException("internal error: an alien variable expression can't be "
                     "translated to a CodeWorker expression!");
}

std::string
ExprScriptAlienVariable::toString() const
{
  return _sName;
}
}
