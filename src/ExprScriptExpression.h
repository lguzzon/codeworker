...
public:
  ExprScriptReference(ExprScriptExpression* pRef)
    : _pRef(pRef)
    , _pVariable(NULL)
  {}
  ExprScriptReference(DtaScriptVariable* pVariable)
    : _pRef(NULL)
...
public:
  ExprScriptUnaryMinus(ExprScriptExpression* pExpression)
...
public:
  ExprScriptBitwiseNot(ExprScriptExpression* pExpression)
    : _pExpression(pExpression)
  {}
...
public:
  ExprScriptConstant() {}
  ExprScriptConstant(const char* sValue)
    : _sValue(sValue)
  {}
  ...
  inline void setConstant(const char* tcValue, int iLength)
  {
    _sValue.assign(tcValue, iLength);
  }
  virtual std::string getValue(DtaScriptVariable& /*visibility*/) const;
  virtual double getDoubleValue(DtaScriptVariable& visibility) const;
  virtual int getIntValue(DtaScriptVariable& visibility) const;
...
public:
  ...
  void setTree(DtaScriptVariable& visibility,
               DtaScriptVariable& theVariable) const;
  virtual std::string getValue(DtaScriptVariable& /*visibility*/) const;

  virtual EXPRESSION_RETURN_TYPE compileCpp(
    CppCompilerEnvironment& theCompilerEnvironment) const;
  EXPRESSION_RETURN_TYPE compileCppInit(
...
public:
  inline ExprScriptResolvedVariable(DtaScriptVariable* pVariable)
    : _pVariable(pVariable)
  {}
...
public:
  inline ExprScriptScriptFile(ExprScriptExpression* pFileName)
    : _bFileName(true)
  {
    script._pFileName = pFileName;
  }
  inline ExprScriptScriptFile(DtaScript* pBody)
    : _bFileName(false)
  {
    script._pBody = pBody;
  }
...
public:
  ExprScriptAlien(const char* tcAlien, const std::string& sTargetLanguage)
    : ExprScriptConstant(tcAlien)
