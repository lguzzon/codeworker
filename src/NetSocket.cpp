...
bool NetSocket::flushOutputToSocket(int hSocket)
{
    ...
    if (iLength > 0) {
        if (!NetSocket::sendToSocket(hSocket, CGRuntime::_pOutputStream->readBuffer(), iLength)) {
            return false;
        }
...
unsigned long  NetSocket::nToHl(unsigned long  iLong)
{
    return ntohl(iLong);
}
unsigned short NetSocket::nToHs(unsigned short iShort)
{
    return ntohs(iShort);
}
unsigned long  NetSocket::hToNl(unsigned long  iLong)
{
    return htonl(iLong);
}
unsigned short NetSocket::hToNs(unsigned short iShort)
{
    return htons(iShort);
}
}
