...
public:
    ...
    inline const std::list<std::string>& getProjectModules() const
    {
        return _listOfProjectModules;
    }
    ScpStream& getHeader() const;
    ScpStream& getMainHeader() const;
    ScpStream& getBody() const;
    inline const std::string& getIndentation() const
    {
        return getCurrentScriptSession()._sIndentation;
    }
    inline void setIndentation(const std::string& sIndentation)
    {
        _listOfScriptSessions.front()._sIndentation = sIndentation;
    }
    void incrementIndentation();
    void pushVariableScope();
    inline const CppScriptSession& getCurrentScriptSession() const
    {
        return _listOfScriptSessions.front();
    }
    inline const GrfFunction* getCurrentFunction() const
    {
        return _pCurrentFunction;
    }
    inline void setCurrentFunction(const GrfFunction* pFunction)
    {
        _pCurrentFunction = pFunction;
    }
    inline const BNFClause* getCurrentClause() const
    {
        return _pCurrentClause;
    }
    inline void setCurrentClause(const BNFClause* pClause)
    {
        _pCurrentClause = pClause;
    }
    inline int getPointerToDeclarations() const
    {
        return _iPointerToDeclarations;
    }
    inline void setPointerToDeclarations(int iPointerToDeclarations)
    {
        _iPointerToDeclarations = iPointerToDeclarations;
    }
    inline int getBNFStepperCursor() const
    {
        return _iBNFStepperCursor;
    }
    inline void setBNFStepperCursor(int iCursor)
    {
        _iBNFStepperCursor = iCursor;
    }
    void pushLastRepeatCursor(int iCursor);
    int getLastRepeatCursor(bool bUsed = false);
    bool popLastRepeatCursor();
    inline const std::set<std::string>& getGlobalVariables() const
    {
    ...
    void decrementIndentation();
    inline bool bracketsToNextBlock() const
    {
        return _bBracketsToNextBlock;
    }
    inline void bracketsToNextBlock(bool bBrackets)
    {
        _bBracketsToNextBlock = bBrackets;
    }
    inline bool carriageReturnAfterBlock() const
    {
        return _bCarriageReturnAfterBlock;
    }
    inline void carriageReturnAfterBlock(bool bCarriage)
    {
        _bCarriageReturnAfterBlock = bCarriage;
    }
    void pushForeach(GrfForeach* pForeach);
    GrfForeach* getLastForeach() const;
    void popForeach();
