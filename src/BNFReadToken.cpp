...
BNFReadToken::BNFReadToken(DtaBNFScript* pBNFScript,
                           GrfBlock* pParent,
                           bool bContinue,
                           bool bNoCase)
...
void
BNFReadToken::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadToken(*this, env);
}

bool
BNFReadToken::isABNFCommand() const
{
  return true;
}

void
BNFReadToken::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                  bool bConcat,
                                  BNFClause& theClause)
{
...
BNFReadToken::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
...
void
BNFReadToken::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "std::string _compilerClauseText_" << iCursor << " = "
                 << compileCppExtraction() << ";";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "_compilerClauseSuccess = !_compilerClauseText_" << iCursor
                 << ".empty();";
  CW_BODY_ENDL;
  if (!_listOfConstants.empty()) {
...
std::string
BNFReadToken::toString() const
{
  std::string sText =
    getFunctionName() + DtaBNFScript::constantsToString(_listOfConstants) +
