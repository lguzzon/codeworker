...
public:
  ...
  struct SizeAttributes
  {
    ...
    inline SizeAttributes()
      ...
      , _cEndChar('\0')
    {}
    inline SizeAttributes(const SizeAttributes& copy)
      : _iSize(copy._iSize)
...
public:
  ...
  ScpStream(const std::string& sFilename,
            int iMode,
            int iCacheMemory = 2000000,
            int iLength = -1);
  ...
  inline END_STREAM_CALLBACK getStreamReaderCallback() const
  {
    return _pfStreamReaderCallback;
  }
  void setStreamReaderCallback(END_STREAM_CALLBACK pfCBK, void* pData = NULL);
  inline END_STREAM_CALLBACK getStreamWriterCallback() const
  {
  ...
  inline void setInputLocation(int iLocation)
  {
    if (iLocation <= _iSize)
      _iReadCursor = iLocation;
  }
  inline int getOutputLocation() const { return _iWriteCursor; }
  inline void setOutputLocation(int iLocation)
  {
    if (iLocation <= _iSize)
  ...
  inline bool isEqualTo(const std::string& sText)
  {
    return isEqualTo(sText.c_str());
  }
  inline bool isEqualToIgnoreCase(const std::string& sText)
  {
    return isEqualToIgnoreCase(sText.c_str());
  }
  bool findString(const char* sText);
  bool findString(const char* sText, int iBoundary);
  inline bool findString(const std::string& sText)
  {
    return findString(sText.c_str());
  }
  inline bool findString(const std::string& sText, int iBoundary)
  {
    return findString(sText.c_str(), iBoundary);
  }

  inline const std::map<std::string, int>& allFloatingLocations() const
  {
  ...
  bool equalsFromInputLocations(const ScpStream& theStream,
                                int& iPosition) const;
  bool insertText(const std::string& sText,
                  int iLocation,
                  int iAreaToRecover = 0);
  bool insertTextOnce(const std::string& sText,
                      int iLocation,
                      int iAreaToRecover = 0);
  bool insertStream(const ScpStream& theStream,
                    int iLocation,
                    int iAreaToRecover = 0);
  bool copy(const ScpStream& theStream, int iLocation, int iLength = -1);
  void saveIntoFile(const std::string& sFilename,
  ...
  static const std::list<std::string>& getListOfIncludePaths()
  {
    return _listOfIncludePaths;
  }
  static void setListOfIncludePaths(
    const std::list<std::string>& listOfIncludePaths)
  {
  ...
  static bool loadVirtualFile(const std::string& sHandle,
                              std::string& sContent);
  static bool appendVirtualFile(const std::string& sHandle,
                                const std::string& sContent);
...
std::fstream*
openIOFile(const char* sInputFile, bool bCreateDirectories = false);

std::ifstream*
openInputFileFromIncludePath(const char* sFileName,
                             std::string& sCompleteFileName);
int
...
void
setLocationAtXLines(std::istream& stream, int iCursor);
int
...
bool
readIdentifier(std::istream& stream, std::string& sIdentifier);
bool
readLine(std::istream& stream, std::string& sLine);
bool
readInt(std::istream& stream, int& iResult);
bool
readLong(std::istream& stream, long& iResult);
bool
...
readFormattedText(std::istream& stream,
                  ...
                  std::string& sResult);
bool
readPointer(std::istream& stream, unsigned long& iPointer);
bool
...
std::ofstream*
openOutputFile(const char* sOutputFile, bool bCreateDirectories = false);
std::ofstream*
...
void
writePointer(std::ostream& stream, unsigned long iPointer);
