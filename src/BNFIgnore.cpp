...
BNFIgnoreScope::BNFIgnoreScope(DtaBNFScript* pBNFScript,
...
BNFIgnore::BNFIgnore(DtaBNFScript* pBNFScript,
                     GrfBlock* pParent,
                     IGNORE_MODE eMode,
...
void
BNFIgnore::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFIgnore(*this, env);
}

bool
BNFIgnore::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFIgnore::executeInternal(DtaScriptVariable& visibility)
{
  BNFIgnoreScope theScope(_pBNFScript, _eMode, _pIgnoreClause);
  int iImplicitCopyPosition =
    (_pBNFScript->implicitCopy()) ? CGRuntime::getOutputLocation() : -1;
  SEQUENCE_INTERRUPTION_LIST result = GrfBlock::executeInternal(visibility);
  if ((result == BREAK_INTERRUPTION) && (iImplicitCopyPosition >= 0))
    CGRuntime::resizeOutputStream(iImplicitCopyPosition);
  return result;
}

std::string
BNFIgnore::toString() const
{
  std::string sText;
  if (_eMode == NOT_IGNORE)
    sText = "#!ignore";
  else if (_eMode == IGNORE_CLAUSE) {
    std::string sTemp = _pIgnoreClause->getSignature();
    if (sTemp.size() == 7 /*sizeof("#ignore")*/)
      sText = "#ignore";
    else {
    ...
    sText = "#ignore(" + DtaBNFScript::IGNORE_MODEtoString(_eMode) + ")";
...
void
BNFIgnore::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  CW_BODY_INDENT << "{";
  CW_BODY_ENDL;
  ...
  int iCursor)
{
  ...
  if (pIgnoreClause != NULL) {
    ...
    sIgnoreClauseName =
      DtaScript::convertFilenameAsIdentifier(CppCompilerEnvironment::getRadical(
        theCompilerEnvironment.getClauseModule(pIgnoreClause->getName()))) +
      "::" + sIgnoreClauseName;
  }
  ...
  CW_BODY_INDENT << "CGBNFRuntimeIgnore _compilerClause_ignore" << iCursor
                 << "(theEnvironment, " << (int)eMode << ", "
                 << sIgnoreClauseName << ");";
  CW_BODY_ENDL;
}
}
