...
BNFCharBoundaries::BNFCharBoundaries(DtaBNFScript* pBNFScript,
                                     GrfBlock* pParent,
                                     int iStartChar,
                                     int iEndChar,
                                     bool bContinue,
...
{
  if (_bNoCase) {
    if ((iStartChar >= (int)'A') && (_iStartChar <= (int)'Z'))
      iStartChar += 32;
    if ((iEndChar >= (int)'A') && (_iEndChar <= (int)'Z'))
      iEndChar += 32;
...
void
BNFCharBoundaries::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFCharBoundaries(*this, env);
}

bool
BNFCharBoundaries::isABNFCommand() const
{
  return true;
}

void
BNFCharBoundaries::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                       bool bConcat,
                                       BNFClause& theClause)
{
...
BNFCharBoundaries::executeInternal(DtaScriptVariable& visibility)
{
  ...
  int iChar = iOrgChar;
  if (_bNoCase && (iChar <= (int)'Z') && (iChar >= (int)'A'))
    iChar += 32;
  ...
  if (_pVariableToAssign != NULL) {
    ...
    if (_iClauseReturnType == BNFClause::LIST_RETURN_TYPE) {
      ...
      if (_bConcatVariable)
        pVariable->concatValue(tcText);
      else
        pVariable->setValue(tcText);
    }
  }
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "int " << tcOrgChar << " = CGRuntime::readCharAsInt();";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "int " << tcChar << " = " << tcOrgChar << ";";
  CW_BODY_ENDL;
  if (_bNoCase) {
    CW_BODY_INDENT << "\tif ((" << tcChar << " <= (int) 'Z') && (" << tcChar
                   << " >= (int) 'A')) " << tcChar << " += 32;";
    CW_BODY_ENDL;
  }
  ...
  if (!_listOfConstants.empty()) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tswitch(" << tcChar << ") {";
    CW_BODY_ENDL;
    for (std::vector<std::string>::const_iterator i = _listOfConstants.begin();
         i != _listOfConstants.end();
         i++) {
      CW_BODY_INDENT << "\t\tcase " << (int)((*i)[0]) << ":";
      CW_BODY_ENDL;
    }
  ...
  if (_pVariableToAssign != NULL) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\ttcText[0] = (char) " << tcOrgChar << ";";
    CW_BODY_ENDL;
    theCompilerEnvironment.incrementIndentation();
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\t\tunsigned char cChar = (unsigned char) " << tcOrgChar
                 << ";";
  CW_BODY_ENDL;
...
std::string
BNFCharBoundaries::toString() const
{
  std::string sString;
  if (_bContinue)
    sString = "#continue ";
