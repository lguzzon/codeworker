...
BNFString::BNFString(DtaBNFScript* pBNFScript,
                     GrfBlock* pParent,
...
void
BNFString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFString(*this, env);
}

bool
BNFString::isABNFCommand() const
{
  return true;
}

void
BNFString::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                               bool bConcat,
                               BNFClause& theClause)
{
...
BNFString::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
  int iLocation = CGRuntime::getInputLocation();
  int iImplicitCopyPosition = _pBNFScript->skipEmptyChars(visibility);
  if (_bNoCase)
    bSuccess = CGRuntime::readIfEqualToIgnoreCase(_sText);
  else
    bSuccess = CGRuntime::readIfEqualTo(_sText);
  if (!bSuccess) {
  ...
  if (_pVariableToAssign != NULL) {
    ...
    if (_iClauseReturnType == BNFClause::LIST_RETURN_TYPE) {
      ...
      if (_bConcatVariable)
        pVariable->concatValue(_sText.c_str());
      else
        pVariable->setValue(_sText.c_str());
    }
  }
  if (iImplicitCopyPosition >= 0)
    _pBNFScript->writeBinaryData(_sText.c_str(), _sText.size());
  return NO_INTERRUPTION;
}

void
BNFString::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
