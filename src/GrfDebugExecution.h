...
public:
  GrfDebugExecution(GrfBlock* pParent)
  ...
  inline void removeWatchNodeListener(DtaWatchNodeListener* pListener)
  {
...
private:
  static GrfDebugExecution* getCurrentDebugger()
  {
    return (GrfDebugExecution*)GrfCommand::getCurrentExecutionContext();
  }

  bool executeCommand(GrfCommand* pCommand,
                      DtaScriptVariable& visibility,
                      ScpStream& stream);
  std::string display(GrfCommand* pCommand,
                      DtaScriptVariable& visibility,
                      int iSize,
                      bool bEchoOn = true);
  void traceStack(GrfCommand* pCommand, DtaScriptVariable& visibility);

  DtaBreakpoint* registerBreakpoint(DtaScriptVariable& visibility,
                                    const char* sFilename,
                                    int iLineNumber);
  void clearBreakpoint(DtaScriptVariable& /*visibility*/,
                       const char* sFilename,
                       int iLineNumber);
  DtaBreakpoint* stopOnBreakpoint(GrfCommand* pCommand,
                                  DtaScriptVariable& visibility);
