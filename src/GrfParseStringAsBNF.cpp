...
void
GrfParseStringAsBNF::setBNFFileName(ExprScriptScriptFile* pBNFFileName)
{
...
GrfParseStringAsBNF::executeInternal(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pClass = visibility.getExistingVariable(*_pClass);
  if (pClass == NULL)
    throw UtlException(
      "runtime error: variable '" + _pClass->toString() +
      "' doesn't exist while calling procedure 'ParseStringAsBNF()'");
  ...
  SEQUENCE_INTERRUPTION_LIST result =
    _pCachedScript->generateString(sContent, *pClass);
  switch (result) {
...
void
GrfParseStringAsBNF::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "CGRuntime::parseStringAsBNF(&Execute" << sRadical
                 << "::instance(), ";
