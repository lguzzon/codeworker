...
std::string
ExprScriptMotif::getValue(DtaScriptVariable& visibility) const
{
  throw UtlException("internal error: can't ask for the value of a motif");
}

ExprScriptMotifPath::ExprScriptMotifPath(ExprScriptMotifPath* pPrecedentPath)
  : _pPrecedentPath(pPrecedentPath)
  , _pNextPath(NULL)
{
...
ExprScriptMotifPath::ExprScriptMotifPath(ExprScriptMotifPath* pPrecedentPath,
                                         ExprScriptMotifPath* pNextPath)
  : _pPrecedentPath(pPrecedentPath)
  , _pNextPath(pNextPath)
{
  _pPrecedentPath->_pNextPath = this;
...
DtaScriptVariable*
ExprScriptMotifStep::filterNodes(
  DtaScriptVariable& visibility,
  DtaScriptVariable& currentNode,
  std::list<DtaScriptVariable*>& listOfNodes) const
{
  ...
  if (_sName == "*") {
    ...
    while (pAttributes != NULL) {
      if (endNode()) {
        ...
        DtaScriptVariable* pResult = getNextPath()->filterNodes(
          visibility, *(pAttributes->getNode()), listOfNodes);
        if (pResult != NULL)
    ...
    if (pCurrent != NULL) {
      if (endNode())
        ...
        pCurrent =
          getNextPath()->filterNodes(visibility, *pCurrent, listOfNodes);
    }
  }
  return pCurrent;
}
...
std::string
ExprScriptMotifStep::toString() const
{
...
DtaScriptVariable*
ExprScriptMotifEllipsis::filterNodes(
  DtaScriptVariable& visibility,
  DtaScriptVariable& currentNode,
  std::list<DtaScriptVariable*>& listOfNodes) const
{
  ...
  if (array != NULL) {
    for (std::list<DtaScriptVariable*>::const_iterator i = array->begin();
         i != array->end();
         i++) {
...
std::string
ExprScriptMotifEllipsis::toString() const
{
  std::string sText =
    getPrecedentPath()->toString() + "..." + getNextPath()->toString();
  return sText;
}
...
DtaScriptVariable*
ExprScriptMotifArray::filterNodes(
  DtaScriptVariable& visibility,
  DtaScriptVariable& currentNode,
  std::list<DtaScriptVariable*>& listOfNodes) const
{
    ...
    if (array != NULL) {
      for (std::list<DtaScriptVariable*>::const_iterator i = array->begin();
        ...
        if (endNode()) {
          ...
          DtaScriptVariable* pResult =
            getNextPath()->filterNodes(visibility, *(*i), listOfNodes);
          if (pResult != NULL)
            pLastCurrent = pResult;
        }
      }
...
std::string
ExprScriptMotifArray::toString() const
{
...
DtaScriptVariable*
ExprScriptMotifBoolean::filterNodes(
  DtaScriptVariable& visibility,
  DtaScriptVariable& currentNode,
  std::list<DtaScriptVariable*>& listOfNodes) const
{
  ...
  if (_cOperator == '&') {
    ...
    for (std::vector<ExprScriptMotif*>::const_iterator i = _members.begin();
         i != _members.end();
         i++) {
    ...
    if (pCurrent != NULL) {
      for (std::list<DtaScriptVariable*>::const_iterator i = list.begin();
           i != list.end();
           i++) {
    ...
    for (std::vector<ExprScriptMotif*>::const_iterator i = _members.begin();
         i != _members.end();
         i++) {
      register DtaScriptVariable* pResult =
        (*i)->filterNodes(visibility, currentNode, listOfNodes);
      if (pResult != NULL)
        pCurrent = pResult;
    }
  }
...
std::string
ExprScriptMotifBoolean::toString() const
{
  std::string sText;
  for (std::vector<ExprScriptMotif*>::const_iterator i = _members.begin();
       i != _members.end();
