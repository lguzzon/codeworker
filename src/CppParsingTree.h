...
A C++ interface is available since version 3.0, for using CodeWorker in C++
applications. This enables calling parse and generation tasks directly from C++.
Hence, the C++ application can handle the parse tree, iterating node arrays,
accessing to subtrees (the attributes) and so on. Moreover, the developer has
all native functions of the CodeWorker scripting language at his disposal.
...
Once you have downloaded CodeWorker, you'll find the directory "cpp" just below
the main directory of the setup. This directory concerns all about CodeWorker in
C++. The C++ static release library is directly available under the "cpp"
directory, in "cpp/JNICodeWorker.dll" (Windows only; for other platforms,
"libcodeworker.a" stands in the root path of CodeWorker). The C++ headers
declaring the API with the library, are delivered in "cpp/include".

From here, you have to add the C++ library to your project (Makefile, VC++
solution, ...) and to add an include path pointing to the C++ headers.

[section]Example[/section]

The following C++ application behaves like a CodeWorker's leader script. It asks
for the parsing of a file, then it generates an HTML file by exploring the parse
tree. A very classical process in Design-Specific Modeling. [code] #include
"CGCompiler.h" #include "CGRuntime.h" #include "CppParsingTree.h"
...
The class "CodeWorker::CGCompiledBNFScript" allows the precompilation of an
extended-BNF script and its execution. The execution requires a
"CodeWorker::CppParsingTree_value" object as the context to populate.

The class "CodeWorker::CGCompiledTemplateScript" precompiles a template-based
script and generates an output file, traversing the parse tree previously
populated by the BNF script.

At the end, the parse tree is displayed, up to a depth of 4, using a classical
function of the scripting language: "traceObject()". All functions of the
scripting language are accessible via the class "CodeWorker::CGRuntime".

[chapter]The C++ API of CodeWorker[/chapter]

[section]CodeWorker::CppParsingTree_value[/section]

This class represents a CodeWorker variable, able to contain an association
table, to have some attributes (branches through subtrees), to be worth a string
value or to point to another parse tree.
...
This constructor creates an internal parse tree, which will be deleted once the
garbage collector will free this instance. Note that this parse tree will have
no name (the property "getName()" will return null).

[subsection]public ParseTree(ParseTree)[/subsection]

This constructor points to another parse tree, but will never delete the
internal parse tree it refers to. It is a kind of copy-by-reference.

Example:
[code]
CodeWorker::CppParsingTree_value secondTree = new
CodeWorker::CppParsingTree_value(firstTree);
[/code]
...
This property returns the name of the node (null if the node was declared on the
stack of the Java application).
...
This property returns the association table attached to the node. If there is no
...
This property returns all attribute names (branches through subtrees) of the
node. This function introspects the node.
...
This function returns the subtree attached to an attribute of the node. If the
attribute doesn't exist, it returns null.
...
This function inserts a new attribute to the node and returns the subtree newly
created. If the attribute already exists, it returns the attached subtree.
...
The Java package of CodeWorker allows the developer to drive easily DSL parsing
and code generation from the Java platform. The parse tree can be traversed and
decorated directly in Java, rather than only in the scripting language of
CodeWorker.
   [/description]
**/
...
public:
  ...
  std::auto_ptr<std::vector<DtaScriptVariable*>> getSortedNoCaseArrayOnValue()
    const;
  ...
  CppParsingTree_var insertArrayNodeFromKey(
    const CppParsingTree_var& pKey) const;
};
