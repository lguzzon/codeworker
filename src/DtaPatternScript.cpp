...
public:
    ...
    inline const std::vector<GrfAspectAdvice*>& before() const
    {
        return _before;
    }
    inline const std::vector<GrfAspectAdvice*>& around() const
    {
        return _around;
    }
    inline const std::vector<GrfAspectAdvice*>& after() const
    {
        return _after;
    }
...
{
    if ((tcFile == NULL) || (_pPatternScript == NULL) || (_pPatternScript->getFilenamePtr() == NULL)) {
        return;
    }
...
{
    if (_pPatternScript == 0) {
        throw UtlException("internal error in 'DtaOutputFile::generate()': the pattern script is null");
    }
...
{
    if (_pPatternScript == 0) {
        throw UtlException("internal error in 'DtaOutputFile::generateString()': the pattern script is null");
    }
...
{
    if (_pPatternScript == 0) {
        throw UtlException("internal error in 'DtaOutputFile::expand()': the pattern script is null");
    }
...
DtaScriptFactory::SCRIPT_TYPE DtaPatternScript::getType() const
{
    return DtaScriptFactory::PATTERN_SCRIPT;
}
bool DtaPatternScript::isAGenerateScript() const
{
    return true;
}
...
void DtaPatternScript::traceEngine() const
{
    if (getFilenamePtr() == NULL) {
        ...
        CGRuntime::traceLine("template-based script \"" + std::string(getFilenamePtr()) + "\":");
    }
...
void DtaPatternScript::handleUnknownCommand(const std::string& sCommand, ScpStream& script, GrfBlock& block)
{
    if (sCommand == "jointpoint") {
        ...
        if (script.isEqualTo(';')) {
            ...
            if (pVariable == NULL) {
                throw UtlException(script, "variable expected on the left side of an assignment operator");
            }
...
void DtaPatternScript::parseAllFloatingLocations(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'allFloatingLocations' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'attachOutputToSocket' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'detachOutputFromSocket' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'incrementIndentLevel' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'insertText' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'insertTextOnce' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'insertTextToFloatingLocation' is available on pattern scripts only");
    }

    GrfInsertTextToFloatingLocation* pInsertTextToFloatingLocation = new GrfInsertTextToFloatingLocation;

    if (requiresParsingInformation()) {
        pInsertTextToFloatingLocation->setParsingInformation(getFilenamePtr(), script);
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'insertTextOnceToFloatingLocation' is available on pattern scripts only");
    }

    GrfInsertTextOnceToFloatingLocation* pInsertTextOnceToFloatingLocation = new GrfInsertTextOnceToFloatingLocation;

    if (requiresParsingInformation()) {
        pInsertTextOnceToFloatingLocation->setParsingInformation(getFilenamePtr(), script);
    }
    ...
    if (pMethodCaller == NULL) {
        pInsertTextOnceToFloatingLocation->setLocation(parseExpression(block, script));
    } else {
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'overwritePortion' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'populateProtectedArea' is available on pattern scripts only");
    }
    ...
    if (pMethodCaller == NULL) {
        pPopulateProtectedArea->setProtectedAreaName(parseExpression(block, script));
    } else {
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'resizeOutputStream' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'setFloatingLocation' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'setOutputLocation' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'setProtectedArea' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'writeBytes' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'writeText' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "command 'writeTextOnce' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "statement modifier 'generated_file' is available on pattern scripts only");
    }
...
{
    if (!isAGenerateScript()) {
        throw UtlException(script, "statement modifier 'generated_string' is available on pattern scripts only");
    }
    ...
    if (script.isEqualTo('(')) {
        ...
        if (!script.isEqualTo(')')) {
            throw UtlException(script, "syntax error: ')' expected after the variable");
        }
    }
