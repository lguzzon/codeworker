...
private:
  ...
  bool _bGenericKey; // means that it is a generic template function where
                     // '_sTemplateInstantiation' gives the generic key
  GrfFunction* _pTemplateFunction; // root function for templates
  std::map<std::string, GrfFunction*>
    _mapOfInstantiatedFunctions; // belongs to the root template function
  GrfFunction*
    _pGenericTemplateFunction; // belongs to the root template function
  struct PatternTemplateStruct
  {
    DtaScriptFactory::SCRIPT_TYPE _eParentScriptType;
    DtaPatternScript* _pPattern;
    PatternTemplateStruct(DtaScriptFactory::SCRIPT_TYPE eParentScriptType,
                          DtaPatternScript* pPattern)
      : _eParentScriptType(eParentScriptType)
      , _pPattern(pPattern)
    {}
    ~PatternTemplateStruct();
  };
  PatternTemplateStruct*
    _pPatternTemplateBody; // belongs to the root template function when it
                           // defines a pattern script
...
public:
  ...
  bool addParameterAndType(const char* sParameter,
                           EXPRESSION_TYPE exprType,
                           ExprScriptExpression* pDefault);
  inline const std::vector<EXPRESSION_TYPE>& getParameterTypes() const
  {
    return _parameterTypes;
  }
  inline const std::list<std::string>& getParameters() const
  {
    return _parameters;
  }
  ...
  virtual bool addBNFLocalVariable(const std::string& sVarName,
  ...
  inline unsigned int getMinArity() const
  {
    return getArity() - _defaults.size();
  }
  inline bool isATemplateInstantiation() const
  {
    return (_pTemplateFunction != NULL) ||
           !_mapOfInstantiatedFunctions.empty() ||
           (_pGenericTemplateFunction != NULL);
  }
  inline bool isATemplateDispatcher() const
  {
    return !_mapOfInstantiatedFunctions.empty();
  }
  inline const std::string& getTemplateInstantiation() const
  {
    return _sTemplateInstantiation;
  }
  inline bool isGenericKey() const { return _bGenericKey; }
  inline GrfFunction* getTemplateFunction() const { return _pTemplateFunction; }
  inline const std::map<std::string, GrfFunction*>& getInstantiatedFunctions()
    const
  {
    return _mapOfInstantiatedFunctions;
  }
  GrfFunction* getInstantiatedFunction(
    DtaScriptVariable& visibility,
    const std::string& sInstantiationKey) const;
  void setPatternTemplateBody(DtaScriptFactory::SCRIPT_TYPE eParentScriptType,
                              DtaPatternScript* pScript);

  inline bool isBodyDefined() const { return _bIsBodyDefined; }
  inline void isBodyDefined(bool bIsBodyDefined)
  {
    _bIsBodyDefined = bIsBodyDefined;
  }
