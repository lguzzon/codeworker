...
DtaScriptFactory::SCRIPT_TYPE DtaTranslateScript::getType() const
{
    return DtaScriptFactory::TRANSLATE_SCRIPT;
}
bool DtaTranslateScript::isAParseScript() const
{
    return true;
}
bool DtaTranslateScript::isAGenerateScript() const
{
    return true;
}
...
void DtaTranslateScript::traceEngine() const
{
    if (getFilenamePtr() == NULL) {
        ...
        CGRuntime::traceLine("translation script \"" + std::string(getFilenamePtr()) + "\":");
    }
...
ExprScriptFunction* DtaTranslateScript::parseImplicitCopyFunction(GrfBlock& block, ScpStream& script)
{
    ...
    if (script.isEqualTo('(')) {
        ...
        if (script.isEqualTo('<')) {
            ...
            if (!script.readString(sTemplateKey)) {
                if (script.readIdentifier(sTemplateKey)) {
                    ...
                    throw UtlException(script, "template key expected for the function of '#implicitCopy'");
                }
            }
        ...
        if (pImplicitFunction->getArity() != 1) {
            throw UtlException(script, "function '" + sFunctionName + "' should have one and only one value parameter");
        }
