...
GrfInsertText::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if ((*_pOutputCoverage != NULL) &&
      CGRuntime::getOutputStream()->insertText(sText, iLocation)) {
    int iCoverageLocation = iLocation + (*_pOutputCoverage)->getIntValue();
    DtaScriptVariable* pCoverage =
      getCoverageItem(*_pOutputCoverage, sText.size(), iCoverageLocation, "I");
...
void
GrfInsertText::prepareCoverage(DtaScriptVariable* const* pOutputCoverage)
{
  _pOutputCoverage = pOutputCoverage;
}

int
GrfInsertText::shiftCoverageItem(DtaScriptVariable* pCoverage,
                                 int iLocation,
                                 int iLength)
{
  std::list<DtaScriptVariable*>* pList =
    const_cast<std::list<DtaScriptVariable*>*>(pCoverage->getArray());
  if (pList == NULL)
    return -1;
  int iRank = pList->size();
  for (std::list<DtaScriptVariable*>::reverse_iterator i = pList->rbegin();
       i != pList->rend();
       ++i) {
...
DtaScriptVariable*
GrfInsertText::getCoverageItem(DtaScriptVariable* pCoverage,
                               int iLocation,
                               int iLength,
...
void
GrfInsertText::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
