...
BNFReadUptoIgnore::BNFReadUptoIgnore(DtaBNFScript* pBNFScript,
                                     GrfBlock* pParent,
                                     IGNORE_MODE eMode,
...
void
BNFReadUptoIgnore::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadUptoIgnore(*this, env);
}

bool
BNFReadUptoIgnore::isABNFCommand() const
{
  return true;
}

void
BNFReadUptoIgnore::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                       bool bConcat,
                                       BNFClause& theClause)
{
...
BNFReadUptoIgnore::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
  ...
  while (CGRuntime::readCharAsInt() >= 0) {
    ...
    if (iLastLocation != CGRuntime::getInputLocation())
      break;
  }
...
void
BNFReadUptoIgnore::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  CPP_COMPILER_BNF_SYMBOL_BEGIN;
  CW_BODY_INDENT << "int _compilerClauseFirstLocation_" << iCursor
                 << " = CGRuntime::getInputLocation();";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "int _compilerClauseLastLocation_" << iCursor
                 << " = _compilerClauseFirstLocation_" << iCursor << ";";
  CW_BODY_ENDL;
  BNFIgnore::compileCppRuntimeIgnore(
    theCompilerEnvironment, _eMode, _pIgnoreClause, iCursor);
  CW_BODY_INDENT << "while (CGRuntime::readCharAsInt() >= 0) {";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\t++_compilerClauseLastLocation_" << iCursor << ";";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\ttheEnvironment.skipEmptyChars();";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\tif (_compilerClauseLastLocation_" << iCursor
                 << " != CGRuntime::getInputLocation()) break;";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "}";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "_compilerClauseSuccess = (_compilerClauseFirstLocation_"
                 << iCursor << " != _compilerClauseLastLocation_" << iCursor
                 << ");";
  CW_BODY_ENDL;
  ...
  if (!_listOfConstants.empty()) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT
      << "\t_compilerClauseText_" << iCursor
      << " = CGRuntime::getLastReadChars(_compilerClauseLastLocation_"
      << iCursor << " - _compilerClauseFirstLocation_" << iCursor << ");";
    CW_BODY_ENDL;
  ...
  if (_bContinue) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tif (" << tcImplicitCopy
                   << " >= 0) CGRuntime::resizeOutputStream(" << tcImplicitCopy
                   << ");";
  }
  CW_BODY_ENDL;
  CW_BODY_INDENT << "} else if (" << tcImplicitCopy << " >= 0) {";
  CW_BODY_ENDL;
  ...
  CW_BODY_INDENT << "\tCGRuntime::writeBinaryData(_compilerClauseText_"
                 << iCursor << ".c_str(), _compilerClauseText_" << iCursor
                 << ".size());";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "}";
  CW_BODY_ENDL;
...
std::string
BNFReadUptoIgnore::toString() const
{
