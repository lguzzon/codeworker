...
public:
    CppVariableScope() : _bHasEvaluatedExpressionInScope(false)
    {
        addStack();
    }
    ~CppVariableScope()
    ...
    void addStack()
    {
        _stack.push_front(new CppStack);
    }
    void removeStack()
    {
        delete _stack.front();
...
CppCompilerEnvironment::CppCompilerEnvironment(const std::string& sCppProjectDirectory) : _bBracketsToNextBlock(true), _bCarriageReturnAfterBlock(true), _iSwitchNumber(0), _iCursorNumber(0), _bErrorEncountered(false), _iPointerToDeclarations(0), _pCurrentClause(NULL), _iBNFStepperCursor(0), _iInlineScriptCounter(0)
{
    ...
    if ((sCppProjectDirectory[0] == '/') || (!sCppProjectDirectory.empty() && (sCppProjectDirectory[1] == ':'))) {
        _sCppProjectDirectory = CGRuntime::canonizePath(sCppProjectDirectory);
    } else {
        _sCppProjectDirectory = CGRuntime::relativePath(sCppProjectDirectory, CGRuntime::getWorkingPath());
    }
    ...
    for (std::map<std::string, DynPackage*>::const_iterator i = allPackages.begin(); i != allPackages.end(); ++i) {
        ...
        (*pModuleHeader) << "#endif" << ScpStream::ENDL;
        {
            ...
            if (!pModuleHeader->equals(*pOldHeader, iPosition)) {
                pModuleHeader->saveIntoFile(_sCppProjectDirectory + i->first + ".h", true);
            }
        }
        {
            ...
            if (!pModuleBody->equals(*pOldBody, iPosition)) {
                pModuleBody->saveIntoFile(_sCppProjectDirectory + i->first + ".cpp", true);
            }
        }
    }
}
...
void CppCompilerEnvironment::generateDSP()
{
    ...
    if (pMakeFile->empty()) {
        ...
        if (!pMakeFile->findString("# Begin Source File")) {
            throw UtlException("bad format encountered into Visual C++ project \"" + sProjectName + ".dsp\": unable to find tag '# Begin Source File'");
        }
    ...
    if (!pMakeFile->equals(*pOldMakeFile, iPosition)) {
        pMakeFile->saveIntoFile(_sCppProjectDirectory + sProjectName + ".dsp", true);
    }
}
...
void CppCompilerEnvironment::translateToTargetLanguage(const std::string& sTargetLanguage)
{
    ...
    if (sParserScript.empty()) {
        throw UtlException("C++-to-" + sTargetLanguage + " translation error: unable to find the BNF-parse script \"CWcpp-parser.cwp\"");
    }
...
std::string CppCompilerEnvironment::getFunctionModule(const std::string& sFunction) const
{
    for (std::list<CppScriptSession>::const_iterator i = _listOfScriptSessions.begin(); i != _listOfScriptSessions.end(); i++) {
        if (i->_setOfProjectFunctions.find(sFunction) != i->_setOfProjectFunctions.end()) {
