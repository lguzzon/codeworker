...
bool
createDirectoriesForFile(const std::string& sFileName)
{
  bool bCreated;
...
ScpStream::ScpStream(int iCacheMemory)
  ...
  , _pPrecStream(NULL)
  , _pNextStream(NULL)
  , _pfStreamReaderCallback(NULL)
  , _pfStreamWriterCallback(NULL)
  , _iLineCounter(1)
  , _iLinePosition(0)
{
...
ScpStream::ScpStream(const std::string& sFilename,
                     int iMode,
                     int iCacheMemory,
                     int iLength)
  : _iCacheMemory(iCacheMemory)
  ...
  , _iLinePosition(0)
{
  ...
  if (loadVirtualFile(sFilename, sContent)) {
    ...
    if (iCacheMemory <= _iSize)
      _tcStream = new char[_iSize + 1];
    else
      _tcStream = new char[iCacheMemory];
    ...
    if (iMode & IN) {
      if (iMode & PATH) {
        ...
        std::list<std::string>::const_iterator i =
          ScpStream::getListOfIncludePaths().begin();
        if ((sFilename[0] != '/') &&
            ((sFilename.length() < 2) || (sFilename[1] != ':'))) {
          while ((pFile == NULL) &&
                 (i != ScpStream::getListOfIncludePaths().end())) {
        ...
        if (pFile == NULL)
          THROW_UTLEXCEPTION("impossible to find file \"" + sFilename +
                             "\" into include paths");
        pFile->close();
        delete pFile;
      }
      ...
      if (f == NULL) {
        if ((iMode & OUT) == 0)
          THROW_UTLEXCEPTION("impossible to open file \"" + _sFilename +
                             "\" for reading");
      } else {
        ...
        if ((iLoaded < 32000) && ferror(f))
          THROW_UTLEXCEPTION("an error has occurred while reading file \"" +
                             _sFilename + "\"");
        fclose(f);
      }
    }
...
ScpStream::ScpStream(const std::string& sFilename, FILE* f, int iCacheMemory)
  : _sFilename(sFilename)
  , _iCacheMemory(iCacheMemory)
  , _iSize(0)
  , _iReadCursor(0)
  ...
  , _iLinePosition(0)
{
  ...
  do {
    ...
    if ((iLoaded < 32000) && ferror(f)) {
      ...
      sMessage += tcNumber;
      sMessage +=
        "] has occurred while reading bytes of file \"" + _sFilename + "\"";
...
ScpStream::ScpStream(const std::string& sText)
  : _iSize(0)
  , _iReadCursor(0)
  , _iWriteCursor(0)
  , _bInsertText(false)
...
ScpStream::ScpStream(ScpStream& shiftedStream, int iShiftedStreamPosition)
  : _iReadCursor(0)
  , _iWriteCursor(0)
  , _bInsertText(false)
  , _iMode(0)
...
void
ScpStream::setStreamReaderCallback(END_STREAM_CALLBACK pfCBK, void* pData)
{
...
void
ScpStream::setStreamWriterCallback(END_STREAM_CALLBACK pfCBK, void* pData)
{
...
ScpStream*
ScpStream::createFile(const std::string& sFilename)
{
...
bool
ScpStream::existInputFile(const char* /*java:"const std::string&"*/ sFileName)
{
  if (existVirtualFile(sFileName))
    return true;
#ifdef WIN32
  HANDLE h = CreateFile(sFileName,
                        GENERIC_READ,
                        FILE_SHARE_READ,
                        NULL,
                        OPEN_EXISTING,
...
bool
ScpStream::existInputFileFromIncludePath(
  const char* /*java:"const std::string&"*/ tcFileName,
  std::string& sCompleteFileName)
{
  ...
  if (!bFile &&
      ((sCompleteFileName[0] == '/') || (sCompleteFileName[1] == ':')))
    return false;
  std::list<std::string>::const_iterator i =
    ScpStream::getListOfIncludePaths().begin();
...
ScpStream*
ScpStream::openInputFile(const char* /*java:"const std::string&"*/ sFileName)
{
  std::string /*java:"std::string&"*/ sContent;
  if (loadVirtualFile(sFileName, sContent)) {
    ScpStream* pStream = new ScpStream;
...
bool
ScpStream::createVirtualFile(const std::string& sHandle,
...
std::string
ScpStream::createVirtualTemporaryFile(const std::string& sContent)
{
  int iHandle = 0;
  char tcHandle[32];
  do {
...
bool
ScpStream::existVirtualFile(const std::string& sHandle)
{
  std::map<std::string, std::string>::const_iterator cursor =
    _listOfVirtualFiles.find(sHandle);
  return (cursor != _listOfVirtualFiles.end());
}

bool
ScpStream::loadVirtualFile(const std::string& sHandle, std::string& sContent)
{
  std::map<std::string, std::string>::iterator cursor =
    _listOfVirtualFiles.find(sHandle);
  if (cursor == _listOfVirtualFiles.end())
    return false;
...
bool
ScpStream::appendVirtualFile(const std::string& sHandle,
                             const std::string& sContent)
{
  std::map<std::string, std::string>::iterator cursor =
    _listOfVirtualFiles.find(sHandle);
...
bool
ScpStream::deleteVirtualFile(const std::string& sHandle)
{
  std::map<std::string, std::string>::iterator cursor =
    _listOfVirtualFiles.find(sHandle);
  if (cursor == _listOfVirtualFiles.end())
...
ScpStream::SizeAttributes
ScpStream::resize(int iNewSize)
{
  ...
  if ((iNewSize >= 0) && (iNewSize < _iSize)) {
    ...
    if (_iReadCursor > _iSize)
      _iReadCursor = _iSize;
    if (_pShiftedStream != NULL) {
      sizeAttrs._pShiftedStream = new SizeAttributes(
        _pShiftedStream->resize(iNewSize + _iShiftedStreamPosition));
    }
  }
  return sizeAttrs;
}
...
void
ScpStream::restoreSize(const ScpStream::SizeAttributes& sizeAttrs)
{
  if (!sizeAttrs.empty()) {
...
void
ScpStream::incrementIndentation(int iLevel)
{
  while (iLevel > 0) {
    _sIndentation += "\t";
    --iLevel;
...
bool
ScpStream::decrementIndentation(int iLevel)
{
...
std::string
ScpStream::getMessagePrefix(bool bCountCols) const
{
  ...
  char tcNumber[64];
  if (bCountCols)
    sprintf(tcNumber, "(%d): ", getLineCount());
  else
    sprintf(tcNumber, "(%d,%d): ", getLineCount(), getColCount());
  return sFilename + tcNumber;
}

void
ScpStream::setLineDirective(int iLine)
{
  _iLineCounter = iLine;
...
int
ScpStream::getOutputLineCount() const
{
  int iLine = 1;
  int iCursor = 0;
...
int
ScpStream::getOutputColCount() const
{
...
bool
ScpStream::skipSpaces()
{
  int iChar = readChar();
  while ((iChar >= (int)'\0') && (iChar == (int)' ' || iChar == (int)'\t'))
    iChar = readChar();
  if (iChar < 0)
    return false;
  goBack();
...
bool
ScpStream::skipLineBlanks()
{
  int iChar = readChar();
  while ((iChar >= (int)'\0') && (iChar <= (int)' ') && (iChar != '\r') &&
         (iChar != '\n'))
    iChar = readChar();
  if (iChar < 0)
    return false;
...
bool
ScpStream::skipCppExceptDoxygenComments()
{
  int iCursor = getInputLocation();
  int iChar = readChar();
  if (iChar == (int)'/') {
...
bool
ScpStream::skipEmptyCppExceptDoxygen()
{
  bool bSuccess;
  do
    bSuccess = skipBlanks();
...
bool
ScpStream::readChars(int iLength, std::string& sText)
{
...
bool
ScpStream::readWord(std::string& sWord)
{
  int iIndex = 0;
  int iChar;
  ...
  do {
    iChar = readChar();
    ++iIndex;
  } while (((iChar >= (int)'a') && (iChar <= (int)'z')) ||
           ((iChar >= (int)'A') && (iChar <= (int)'Z')) ||
           ((iChar >= (int)'0') && (iChar <= (int)'9')) ||
           (iChar == (int)'_') || (iChar == (unsigned char)'é') ||
           (iChar == (unsigned char)'è') || (iChar == (unsigned char)'ê') ||
           (iChar == (unsigned char)'à') || (iChar == (unsigned char)'î') ||
           (iChar == (unsigned char)'ù') || (iChar == (unsigned char)'ô') ||
           (iChar == (unsigned char)'û') || (iChar == (unsigned char)'â') ||
           (iChar == (unsigned char)'ç'));
  if (iChar >= 0)
    goBack();
  --iIndex;
  if (iIndex == 0)
    ...
    sWord = std::string(_tcStream + iPos, iIndex);
  return (iIndex > 0);
}

bool
ScpStream::readIdentifier(std::string& sIdentifier)
{
  ...
  if (((iChar >= (int)'a') && (iChar <= (int)'z')) ||
      ((iChar >= (int)'A') && (iChar <= (int)'Z')) || (iChar == (int)'_')) {
...
bool
ScpStream::readHexadecimal(int& iResult)
{
  int iChar = readChar();
  if (((iChar < (int)'0') || (iChar > (int)'9')) &&
      ((iChar < (int)'A') || (iChar > (int)'F')) &&
      ((iChar < (int)'a') || (iChar > (int)'f'))) {
    if (iChar >= 0)
      goBack();
    return false;
  ...
  for (;;) {
    ...
    if ((iChar >= (int)'0') && (iChar <= (int)'9'))
      iDigit = iChar - (int)'0';
    else if ((iChar >= (int)'A') && (iChar <= (int)'F'))
...
bool
ScpStream::readDouble(double& dValue)
{
...
bool
ScpStream::readStringOrCharLiteral(std::string& sText)
{
  if (readString(sText))
    return true;
  int iChar;
...
bool
ScpStream::readCharLiteral(int& iChar)
{
  int iPosition = getInputLocation();
  if (!isEqualTo('\''))
    return false;
...
bool
ScpStream::readString(std::string& sText)
{
  ...
  while ((iChar >= 0) && (iChar != (int)'\"')) {
    if (iIndex > 32760) {
      THROW_UTLEXCEPTION(
        *this, "cannot define a string of more than 32760 characters");
    }
    if (iChar == '\\') {
      iChar = readChar();
      switch (iChar) {
...
bool
ScpStream::readPythonString(std::string& sText)
{
  unsigned char tcReadString[32766];
  int iIndex = 0;
  int iLocation = _iReadCursor;
...
bool
ScpStream::readAdaString(std::string& sText)
{
...
bool
ScpStream::readLine(std::string& sLine)
{
...
bool
ScpStream::readLastChars(int iLength, std::string& sLastChars)
{
  if (iLength < 0)
    return false;
  ...
  if (_iReadCursor - iLength > 0)
    sLastChars = _tcStream + (_iReadCursor - iLength);
  else
    sLastChars = _tcStream;
  _tcStream[_iReadCursor] = a;
  return true;
}

bool
ScpStream::readUptoChar(char cEnd, std::string& sText)
{
...
bool
ScpStream::readUptoChar(const std::string& sEnd, std::string& sText)
{
  int iLocation = getInputLocation();
  std::string sLocalText;
  int iChar = readChar();
...
bool
ScpStream::isEqualTo(unsigned char ch)
{
  int iChar = readChar();
  if (iChar == (int)ch)
    return true;
...
bool
ScpStream::isEqualTo(const char* sText)
{
  int iChar;
  int iPosition = getInputLocation();
...
bool
ScpStream::isEqualToIgnoreCase(unsigned char ch)
{
  int iChar = readChar();
  if ((ch >= 'A') && (ch <= 'Z'))
    ch += ' ';
...
bool
ScpStream::isEqualToIgnoreCase(const char* sText)
{
  int iChar;
  int iPosition = getInputLocation();
  for (int i = 0; sText[i] != '\0'; ++i) {
...
bool
ScpStream::isEqualToIdentifier(const char* /*java:"const std::string&"*/ sText)
{
  int iPosition = getInputLocation();
...
bool
ScpStream::findString(const char* sText)
{
  int iChar = -1;
  int iPosition = getInputLocation();
...
bool
ScpStream::findString(const char* sText, int iBoundary)
{
  int iChar = -1;
  int iPosition = getInputLocation();
  int iCurrent = iPosition;
...
bool
ScpStream::newFloatingLocation(const std::string& sKey)
{
...
void
ScpStream::setFloatingLocation(const std::string& sKey, int iLocation)
{
  _mapOfFloatingLocations[sKey] = iLocation;
}

int
ScpStream::getFloatingLocation(const std::string& sKey,
                               ScpStream*& pOwner) const
{
  std::map<std::string, int>::const_iterator cursor =
    _mapOfFloatingLocations.find(sKey);
...
int
ScpStream::removeFloatingLocation(const std::string& sKey, ScpStream*& pOwner)
{
...
ScpStream::operator<<(const ScpStream& theStream)
{
  for (std::map<std::string, int>::iterator i =
         theStream._mapOfFloatingLocations.begin();
       i != theStream._mapOfFloatingLocations.end();
       ++i) {
...
std::string
ScpStream::getLastWrittenChars(int iNbChars) const
{
...
bool
ScpStream::equals(const ScpStream& theStream, int& iPosition) const
{
...
bool
ScpStream::equalsFromInputLocations(const ScpStream& theStream,
                                    int& iPosition) const
{
...
bool
ScpStream::insertTextOnce(const std::string& sText,
                          int iLocation,
                          int iAreaToRecover)
{
...
bool
ScpStream::insertText(const std::string& sText,
                      int iLocation,
                      int iAreaToRecover)
{
  if (iLocation < 0)
  ...
  if (iBytesToAdd != 0) {
    memmove(_tcStream + (iLocation + sText.length()),
            _tcStream + iEndPortion,
            _iSize - iEndPortion);
    _iSize += iBytesToAdd;
    if (iEndPortion < _iWriteCursor)
      _iWriteCursor += iBytesToAdd;
    //==        if (iEndPortion < _iWriteCursor || _iSize > _iWriteCursor)
    //_iWriteCursor += iBytesToAdd;
    for (std::map<std::string, int>::iterator i =
           _mapOfFloatingLocations.begin();
         i != _mapOfFloatingLocations.end();
         ++i) {
      if (i->second >= iEndPortion)
        _mapOfFloatingLocations[i->first] += iBytesToAdd;
    }
  }
...
bool
ScpStream::insertStream(const ScpStream& theStream,
                        int iLocation,
                        int iAreaToRecover)
{
  if (iLocation < 0)
  ...
  if (iBytesToAdd != 0) {
    memmove(_tcStream + (iLocation + theStream.size()),
            _tcStream + iEndPortion,
            _iSize - iEndPortion);
    _iSize += iBytesToAdd;
    if (iEndPortion < _iWriteCursor)
...
bool
ScpStream::copy(const ScpStream& theStream, int iLocation, int iLength)
{
  if (iLength < 0)
    iLength = theStream.size() - iLocation;
  else if (iLength + iLocation > theStream.size())
    return false;
  ...
  memcpy(
    _tcStream + _iWriteCursor, theStream.readBuffer() + iLocation, iLength);
...
void
ScpStream::saveIntoFile(const std::string& sFilename,
                        bool bCreateFileIfUnknown) const
{
  if (existVirtualFile(sFilename)) {
    ...
    if (f == NULL) {
      ...
      if (f == NULL)
        THROW_UTLEXCEPTION("unable to copy a stream into file '" + sFilename +
                           "'");
    }
    int iCursor = 0;
    ...
    do {
      ...
      if (iWritten != iBlock) {
        char tcNumber[32];
        sprintf(tcNumber, "%d", ferror(f));
        THROW_UTLEXCEPTION(std::string("error [") + tcNumber +
                           "] has occurred while saving bytes into file \"" +
                           sFilename + "\"");
      }
      iCursor += iBlock;
...
void
ScpStream::appendFile(const std::string& sFilename,
                      bool bCreateFileIfUnknown) const
{
  if (existVirtualFile(sFilename)) {
    std::string /*java:"std::string&"*/ sContent;
    ...
    if (f == NULL) {
      ...
      if (f == NULL)
        THROW_UTLEXCEPTION("unable to append a stream into file '" + sFilename +
                           "'");
    }
    int iCursor = 0;
    int iBlock = (_iSize >= 32000) ? 32000 : _iSize;
    do {
      ...
      if (iWritten != iBlock) {
        ...
        sprintf(tcNumber, "%d", ferror(f));
        THROW_UTLEXCEPTION(std::string("error [") + tcNumber +
                           "] has occurred while appending bytes into file \"" +
                           sFilename + "\"");
      }
      iCursor += iBlock;
      iBlock = (iCursor + 32000 <= _iSize) ? 32000 : _iSize - iCursor;
...
void
ScpStream::writeBinaryData(const char* tcBinary, int iLength)
{
  int iFinalLength = iLength;
  bool bStartIndents = false; // if indent-mode active, do we have to start the
                              // writing by an indentation?
  if (!_sIndentation.empty()) {
    ...
    if ((iStartCursor < 0) || (_tcStream[iStartCursor] == '\n')) {
      ...
      // If indent-mode active, number of tabs for reaching the beginning of
      // line
      int iStartIndents = _iWriteCursor - iStartCursor - 1;
      int iExtraStartingTabs =
        _sIndentation.length() - iStartIndents; // how many tabs to add?
      if (iExtraStartingTabs > 0) {
        bStartIndents = true;
        _iWriteCursor -=
          iStartIndents; // we'll have to write at the beginning of the line
        iFinalLength +=
          iExtraStartingTabs; // forecast of extra starting tabulation
      }
    }
    // in the total number of bytes to reserve to the data, add as many
    // indentation as carriage return, ignoring the last one, if at the end of
    // the data
  ...
  if (_bInsertText) {
    memmove(
      _tcStream + iNeededSize, _tcStream + _iWriteCursor, _iSize - iNeededSize);
    for (std::map<std::string, int>::iterator i =
           _mapOfFloatingLocations.begin();
         i != _mapOfFloatingLocations.end();
         ++i) {
      if (i->second >= _iWriteCursor)
        _mapOfFloatingLocations[i->first] += iFinalLength;
    }
  }
  if (_sIndentation.empty()) {
    ...
    // indent-mode active
    char* tcIndentStart =
      (char*)(_tcStream + _iWriteCursor); // where to copy the data
    if (bStartIndents) {
    ...
    *tcIndentStart = *u; // independent copy of the last byte, in case of
                         // encountering an end of line
  }
...
void
ScpStream::writeText(const char* tcText)
{
  writeBinaryData(tcText, strlen(tcText));
}

bool
ScpStream::writeTextOnce(const std::string& sText)
{
  if (_setOfTextsToInsertOnce.find(sText) == _setOfTextsToInsertOnce.end()) {
...
void
ScpStream::writeString(const std::string& sString)
{
  ...
  if (_bInsertText) {
    ...
    for (std::map<std::string, int>::iterator i =
         ...
         ++i) {
      if (i->second >= _iWriteCursor)
        _mapOfFloatingLocations[i->first] += iRequiredSpace;
    }
  }
...
void
ScpStream::writeQuotedChar(char c)
{
  WRITECHAR('\'');
  switch (c) {
    case '\\':
  ...
  while (iChar >= 0) {
    ...
    if (iChar == (int)'\r')
      ...
      do {
        switch (iChar) {
          case ':':
            if ((iPreviousChar != (int)':') && (peekChar() != ':') &&
                (iPreviousNotEmptyChar != (int)')')) {
              if ((sLastIdentifier == "public") ||
                  (sLastIdentifier == "protected") ||
                  (sLastIdentifier == "private") ||
                  (sLastIdentifier == "case")) {
                if (sSequence.empty() ||
                    (sSequence[sSequence.length() - 1] != ':')) {
                  sSequence += ':';
                  sIndentation += '\t';
                } else {
                  --_iWriteCursor;
                  if (sLocalIndentation.length() == sIndentation.length())
                    sLocalIndentation = sLocalIndentation.substr(
                      0, sLocalIndentation.length() - 1);
                }
              }
            }
            sLine += ':';
          ...
          case '}':
            ...
            if (sIndentation.length() <= iOffset) {
              ...
              if (iPreviousChar < 0)
                sLocalIndentation = sLocalIndentation.substr(
                  0, sLocalIndentation.length() - iOffset);
              sSequence = sSequence.substr(0, sSequence.length() - iOffset);
              sIndentation =
                sIndentation.substr(0, sIndentation.length() - iOffset);
            }
            sLine += '}';
            iPreviousNotEmptyChar = iChar;
            break;
...
std::fstream*
openIOFile(const char* sFile, bool bCreateDirectories)
{
  std::fstream* pStream = new std::fstream(
    sFile, std::ios::in | std::ios::out | std::ios::trunc | std::ios::binary);
  if (pStream->fail()) {
    delete pStream;
    pStream = NULL;
    if (bCreateDirectories) {
      createDirectoriesForFile(sFile);
      pStream = new std::fstream(sFile,
...
std::ifstream*
ScpStream::openSTLInputFile(
  const char* /*java:"const std::string&"*/ sInputFile)
{
  if ((sInputFile == NULL) || (sInputFile[0] == '\0'))
    return NULL;
  std::ifstream* pStream =
    new std::ifstream(sInputFile, std::ios::in | std::ios::binary);
  if (pStream->fail()) {
    delete pStream;
    pStream = NULL;
  }
...
std::ifstream*
openInputFileFromIncludePath(const char* tcFileName,
                             std::string& sCompleteFileName)
{
  sCompleteFileName = tcFileName;
...
int
getLocation(std::istream& stream)
{
  int iPos = stream.tellg();
  if ((iPos < 0) && !stream.fail())
    return 0;
  return iPos;
}

void
setLocation(std::istream& stream, int iCursor)
{
  stream.seekg(iCursor);
}

void
setLocationAtXLines(std::istream& stream, int iLines)
{
  if (iLines <= 0) {
...
bool
getColEmptyHeader(std::istream& stream, std::string& sHeader)
{
...
int
getLineCount(std::istream& stream)
{
  int iLine = 1;
  int iPos = getLocation(stream);
...
int
getColCount(std::istream& stream)
{
...
bool
previousLine(std::istream& stream)
{
...
bool
skipLineBlanks(std::istream& stream)
{
  int iChar = readChar(stream);
  while ((iChar >= (int)'\0') && (iChar <= (int)' ') && (iChar != '\r') &&
         (iChar != '\n'))
...
bool
skipCppComments(std::istream& stream)
{
...
bool
skipEmpty(std::istream& stream)
{
  bool bSuccess;
  do
    bSuccess = skipBlanks(stream);
  while (bSuccess && skipCppComments(stream));
  return bSuccess;
}

bool
skipEmptyHTML(std::istream& stream)
{
  int iChar;
  for (;;) {
    do
...
bool
skipEmptyLaTeX(std::istream& stream)
{
  int iChar = readChar(stream);
  if (iChar == (int)'%') {
    do
...
bool
readWord(std::istream& stream, std::string& sWord)
{
  ...
  do {
    ...
    tcReadWord[iIndex++] = (unsigned char)iChar;
           ...
           ((iChar >= (int)'A') && (iChar <= (int)'Z')) ||
           ((iChar >= (int)'0') && (iChar <= (int)'9')) ||
           (iChar == (int)'_') || (iChar == (unsigned char)'°') ||
           (iChar == (int)'\'') || (iChar == (int)'^') ||
           (iChar == (unsigned char)'é') || (iChar == (unsigned char)'è') ||
           (iChar == (unsigned char)'ê') || (iChar == (unsigned char)'à') ||
           (iChar == (unsigned char)'î') || (iChar == (unsigned char)'ù') ||
           (iChar == (unsigned char)'ô') || (iChar == (unsigned char)'û') ||
           (iChar == (unsigned char)'â') || (iChar == (unsigned char)'ç'));
  if (iChar >= 0)
    goBack(stream);
  iIndex--;
  tcReadWord[iIndex] = '\0';
...
bool
readIdentifier(std::istream& stream, std::string& sWord)
{
  unsigned char tcText[1000];
  int iIndex = 0;
  int iChar;
...
bool
readLine(std::istream& stream, std::string& sLine)
{
...
bool
readInt(std::istream& stream, int& iResult)
{
  ...
  if (iChar >= 0)
    setLocation(stream, getLocation(stream) - 1);
  tcText[iIndex] = '\0';
...
bool
readLong(std::istream& stream, long& lResult)
{
  unsigned char tcText[32];
  int iIndex = 0;
  int iChar = readChar(stream);
...
bool
readBoolean(std::istream& stream, bool& bResult)
{
  int iChar = readChar(stream);
  if ((iChar == (int)'f') || (iChar == (int)'F')) {
    iChar = readChar(stream);
    if ((iChar != (int)'a') && (iChar != (int)'A'))
      return false;
    iChar = readChar(stream);
    if ((iChar != (int)'l') && (iChar != (int)'L'))
      return false;
    iChar = readChar(stream);
    if ((iChar != (int)'s') && (iChar != (int)'S'))
      return false;
    iChar = readChar(stream);
    if ((iChar != (int)'e') && (iChar != (int)'E'))
      return false;
    ...
    iChar = readChar(stream);
    if ((iChar != (int)'r') && (iChar != (int)'R'))
      return false;
    iChar = readChar(stream);
    if ((iChar != (int)'u') && (iChar != (int)'U'))
      return false;
    iChar = readChar(stream);
    if ((iChar != (int)'e') && (iChar != (int)'E'))
      return false;
...
bool
readStringOrCharLiteral(std::istream& stream, std::string& sText)
{
  if (readString(stream, sText))
    return true;
  int iChar;
  if (!readCharLiteral(stream, iChar))
    return false;
  sText.assign(1, (char)iChar);
  return true;
}
...
bool
readCharLiteral(std::istream& stream, int& iChar)
{
  int iPosition = getLocation(stream);
  if (!isEqualTo(stream, '\''))
    return false;
...
bool
readString(std::istream& stream, std::string& sResult)
{
...
bool
readDouble(std::istream& stream, double& dResult)
{
  unsigned char tcText[32];
  int iIndex = 0;
  int iChar = readChar(stream);
...
bool
readDate(std::istream& stream, tm& myDate)
{
  int iChar = readChar(stream);
  if ((iChar < (int)'0') || (iChar > (int)'9')) {
    std::string sWord;
    if ((iChar == (int)'t') && !readWord(stream, sWord))
      return false;
    if (sWord != "oday")
      return false;
    myDate.tm_mday = 0;
...
bool
readFormattedText(std::istream& stream,
...
bool
readPointer(std::istream& theStream, unsigned long& iPointer)
{
...
bool
readUptoChar(std::istream& myStream, char cEnd, std::string& sText)
{
...
bool
readUptoChar(std::istream& myStream,
...
bool
isEqualTo(std::istream& stream, unsigned char ch)
{
  int iChar = readChar(stream);
  if (iChar == (int)ch)
    return true;
...
bool
isEqualTo(std::istream& stream, const char* sText)
{
  int iChar;
  int iPosition = getLocation(stream);
  for (int i = 0; sText[i] != '\0'; ++i) {
...
bool
isEqualToIdentifier(std::istream& stream, const char* sText)
{
  int iPosition = getLocation(stream);
  std::string sIdentifier;
  if (!readIdentifier(stream, sIdentifier))
    return false;
  if (sIdentifier != sText) {
    setLocation(stream, iPosition);
    return false;
...
bool
findString(std::istream& stream, const char* sText)
{
  int iChar = -1;
  int iPosition = getLocation(stream);
  int iCurrent = iPosition;
...
std::string
getLastReadChars(std::istream& stream, int iNbChars)
{
  int iPosition = getLocation(stream);
  if (iPosition < iNbChars)
    iNbChars = iPosition;
...
std::ofstream*
openOutputFile(const char* sOutputFile, bool bCreateDirectories)
{
  std::ofstream* pStream =
    new std::ofstream(sOutputFile, std::ios::out | std::ios::binary);
  if (pStream->fail()) {
    delete pStream;
    pStream = NULL;
    if (bCreateDirectories) {
...
std::ofstream*
openAppendFile(const char* sAppendFile)
{
  std::ofstream* pStream =
    new std::ofstream(sAppendFile, std::ios::app | std::ios::binary);
  if (pStream->fail()) {
    delete pStream;
    pStream = NULL;
  }
...
void
writeChar(std::ostream& stream, char c)
{
  stream.put(c);
}

void
writeText(std::ostream& stream, const char* sText)
{
  if (*sText != '\0')
    stream.write(sText, strlen(sText));
}

void
writeComments(std::ostream& stream, const char* sComment)
{
#ifdef WIN32
...
void
writeBorderedComments(std::ostream& stream, const char* sComment)
{
#ifdef WIN32
  char* tLines = ::_strdup(sComment);
#else
  ...
  writeText(stream, "///");
  for (iIndex = 0; iIndex < iMaxLength; iIndex++)
    writeChar(stream, '/');
  writeText(stream, "///\n");
  sCursor = tLines;
  for (int iLine = 0; iLine < iNbLines; iLine++) {
    ...
    writeText(stream, "// ");
    writeText(stream, sCursor);
    for (iIndex = iLength; iIndex < iMaxLength; iIndex++)
      writeChar(stream, ' ');
    writeText(stream, " //\n");
    sCursor += (iLength + 1);
  }
...
void
carriageReturn(std::ostream& stream)
{
  writeChar(stream, '\n');
}

void
writeInt(std::ostream& stream, int iValue)
{
  char sNumber[32];
...
void
writeBoolean(std::ostream& stream, bool bValue)
{
  if (bValue)
...
void
writeDouble(std::ostream& stream, double dValue)
{
  char sNumber[320];
...
void
writeString(std::ostream& stream, const char* sText)
{
  writeChar(stream, '\"');
  const char* u = sText;
  while (*u != '\0') {
...
void
writeDate(std::ostream& stream, const tm& myDate)
{
  if ((myDate.tm_mday == 0) && (myDate.tm_mon == 0) && (myDate.tm_year == 0))
    writeText(stream, "today");
  else {
    if (myDate.tm_mday < 10)
      writeChar(stream, '0');
...
void
writePointer(std::ostream& theStream, unsigned long iPointer)
{
#ifdef WIN32
...
void
copyToStream(std::istream& inputStream,
...
std::string
canonizeUNC_(const char* tcPathname, char* dest1)
{
  char dest2[4096];
  ...
  { // == Fix path (remove ./ and ../) to dest2
    ...
    *dst = *src;
    dst++;
    src++; // Copy drive letter [or first '\' Cedric 12/04/2007]
    if (*src != '\\' && *src != '/') {
      *dst = *src;
      dst++;
      src++; // Copy ':' [but not  second '\' Cedric 14/05/2007]
    }
    *dst = '\0';
#endif
    char* dstMin = dst + 1;
...
std::string
canonize_(const char* tcPathname, char* dest1)
{
  char dest2[4096];
