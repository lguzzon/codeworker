    ...
    if (loadVirtualFile(sFilename, sContent)) {
        ...
        if (iMode & IN) {
            if (iMode & PATH) {
                ...
                if (pFile == NULL) {
                    THROW_UTLEXCEPTION("impossible to find file \"" + sFilename + "\" into include paths");
                }
            ...
            if (f == NULL) {
                if ((iMode & OUT) == 0) {
                    THROW_UTLEXCEPTION("impossible to open file \"" + _sFilename + "\" for reading");
                }
            } else {
                ...
                if ((iLoaded < 32000) && ferror(f)) {
                    THROW_UTLEXCEPTION("an error has occurred while reading file \"" + _sFilename + "\"");
                }
...
bool ScpStream::existInputFileFromIncludePath(const char* /*java:"const std::string&"*/ tcFileName, std::string& sCompleteFileName)
{
    ...
    if (!bFile && ((sCompleteFileName[0] == '/') || (sCompleteFileName[1] == ':'))) {
        return false;
    }
...
ScpStream* ScpStream::openInputFileFromIncludePath(const char* /*java:"const std::string&"*/ tcFileName, std::string& sCompleteFileName)
{
    ...
    if ((pFile == NULL) && ((sCompleteFileName[0] == '/') || (sCompleteFileName[1] == ':'))) {
        return NULL;
    }
            ...
            do {
                        ...
                        if ((sLastIdentifier == "public") || (sLastIdentifier == "protected") || (sLastIdentifier == "private") || (sLastIdentifier == "case")) {
                            if (sSequence.empty() || (sSequence[sSequence.length() - 1] != ':')) {
                                ...
                                if (sLocalIndentation.length() == sIndentation.length()) {
                                    sLocalIndentation = sLocalIndentation.substr(0, sLocalIndentation.length() - 1);
                                }
                            }
                        }
                    }
                ...
                case '}':
                    ...
                    if (sIndentation.length() <= iOffset) {
                        ...
                        if (iPreviousChar < 0) {
                            sLocalIndentation = sLocalIndentation.substr(0, sLocalIndentation.length() - iOffset);
                        }
