...
void
GrfParseFree::setDesignFileName(ExprScriptScriptFile* pDesignFileName)
{
  if (pDesignFileName->isFileName())
    _pDesignFileName = pDesignFileName->getFileName();
  _pCachedScript = dynamic_cast<DtaDesignScript*>(pDesignFileName->getBody());
  pDesignFileName->release();
}

SEQUENCE_INTERRUPTION_LIST
GrfParseFree::executeInternal(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pClass = visibility.getVariable(*_pClass);
  if (pClass == NULL)
    throw UtlException("runtime error: variable '" + _pClass->toString() +
                       "' doesn't exist while calling procedure 'parseFree()'");
  ...
  if (_pDesignFileName != NULL) {
    std::string sDesignFileName = _pDesignFileName->getValue(visibility);
    if ((_pCachedScript == NULL) || (_sCachedDesignFile != sDesignFileName) ||
        ScpStream::existVirtualFile(sDesignFileName)) {
  ...
  SEQUENCE_INTERRUPTION_LIST result =
    executeScript(sInputFile.c_str(), pClass, executeFunction);
  switch (result) {
...
SEQUENCE_INTERRUPTION_LIST
GrfParseFree::executeScript(const char* sInputFile,
                            DtaScriptVariable* pThisContext,
                            EXECUTE_FUNCTION* executeFunction)
{
  if (executeFunction != NULL) {
    CGRuntime::parseFree(
      executeFunction, CppParsingTree_var(pThisContext), sInputFile);
...
void
GrfParseFree::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ExprScriptConstant* pConstantFileName =
    dynamic_cast<ExprScriptConstant*>(_pDesignFileName);
  ...
  CW_BODY_INDENT << "CGRuntime::parseFree(&Execute" << sRadical
                 << "::instance(), ";
...
void
GrfParseFree::compileCppForQuiet(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  std::string sRadical =
    DtaScript::convertFilenameAsIdentifier(CppCompilerEnvironment::getRadical(
      CppCompilerEnvironment::filename2Module(sScriptFilename)));
  CW_BODY_STREAM << "\tCGRuntime::parseFree(&Execute" << sRadical
                 << "::instance(), pThisTree, sFilename);";
  CW_BODY_ENDL;
  if (_pCachedScript == NULL) {
    _pCachedScript = new DtaDesignScript(getParent());
    _sCachedDesignFile = sScriptFilename;
