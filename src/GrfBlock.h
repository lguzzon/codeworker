...
public:
  ...
  inline const std::vector<GrfCommand*>& getCommands() const
  {
    return _commands;
  }
  ...
  DtaTargetLanguageTypeSpecifier* addTargetLanguageTypeSpecifier(
    const std::string& sTypeSpec);
  DtaTargetLanguageTypeSpecifier* getTargetLanguageTypeSpecifier(
    const std::string& sTypeSpec);
  ...
  virtual void compileCppBNFSequence(
    CppCompilerEnvironment& theCompilerEnvironment) const;

protected:
  virtual SEQUENCE_INTERRUPTION_LIST executeInternal(
    DtaScriptVariable& visibility);
  SEQUENCE_INTERRUPTION_LIST executeAt(DtaScriptVariable& visibility,
                                       int iPosition);
  SEQUENCE_INTERRUPTION_LIST executeInto(DtaScriptVariable& visibility,
                                         int iStart,
                                         int iEnd);

  void compileCppBetween(CppCompilerEnvironment& theCompilerEnvironment,
                         int iStart,
                         int iEnd) const;
  void compileCppAt(CppCompilerEnvironment& theCompilerEnvironment,
                    int iStart) const;
};
}
