...
const char* CGRuntime::getApplicationName()
{
    return EXECUTABLE_NAME;
}
const char* CGRuntime::getVersionNumber()
{
...
void CGRuntime::generateString(EXECUTE_FUNCTION* executeFunction, CppParsingTree_var pThisTree, CppParsingTree_var pOutput)
{
    ...
    if (pPatternScript->generateString(sOutput, *(pThisTree._pInternalNode)) == NO_INTERRUPTION) {
...
std::string CGRuntime::readNumeric()
{
    ...
    if (!sNumeric.empty()) {
        ...
        if ((iChar == (int) 'e') || (iChar == (int) 'E')) {
            ...
            if (sExponent.empty()) {
                _pInputStream->setInputLocation(_pInputStream->getInputLocation() - sPrefix.size());
            } else {
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::loadProject(const std::string& sXMLorTXTFileName, DtaScriptVariable* pNodeToLoad)
{
    ...
    if (pFile == NULL) {
        throw UtlException("unable to open file \"" + sXMLorTXTFileName + "\" for reading");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::removeVariable(DtaScriptVariable* pNode)
{
    ...
    if (pParent == NULL) {
        throw UtlException("It is forbidden to remove the main tree or a local variable");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::saveBinaryToFile(const std::string& sFilename, const std::string& sContent)
{
    ...
    if ((iLength % 2) != 0) {
        throw UtlException("saveBinaryToFile(\"" + sFilename + "\"): a binary content cannot have an odd length");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::saveProject(const std::string& sXMLorTXTFileName, DtaScriptVariable* pNodeToSave)
{
    ...
    if (pFile == NULL) {
        throw UtlException("unable to open file \"" + sXMLorTXTFileName + "\" for writing");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::saveProjectTypes(const std::string& sXMLFileName)
{
    ...
    if (pXMLFile == NULL) {
        throw UtlException("unable to open file \"" + sXMLFileName + "\" for writing");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::setTextMode(const std::string& sTextMode)
{
    ...
    if (stricmp(sTextMode.c_str(), "dos") == 0) {
        ...
        throw UtlException("setTextMode(<text-mode>) doesn't recognize \"" + sTextMode + "\" (\"DOS\" or \"UNIX\" or \"BINARY\" expected)");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::setWriteMode(const std::string& sMode)
{
    ...
    if (_pOutputStream == NULL) {
        throw UtlException("no output stream on which to apply the function 'setWriteMode(\"" + sMode + "\")'!");
    }
    ...
    if (stricmp(sMode.c_str(), "insert") == 0) {
        ...
        throw UtlException("function 'setWriteMode()': \"insert\" or \"overwrite\" mode expected, instead of \"" + sMode + "\"");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::insertTextToFloatingLocation(const std::string& sLocation, const std::string& sText)
{
    ...
    if (pOwner == NULL) {
        throw UtlException("the floating location '" + sLocation + "' doesn't exist");
    }
...
SEQUENCE_INTERRUPTION_LIST CGRuntime::writeBytes(const std::string& sBytes)
{
    ...
    if ((iLength % 2) != 0) {
        throw UtlException("writeBytes(\"" + sBytes + "\"): a binary content cannot have an odd length");
    }
...
std::string CGRuntime::byteToChar(const std::string& sByte)
{
    //##protect##"byteToChar"
    if (sByte.size() != 2) {
        throw UtlException("'" + composeCLikeString(sByte) + "' isn't recognized as a byte");
    }
    ...
    if (a <= '9') {
        if (a < '0') {
            throw UtlException("byteToChar(\"" + sByte + "\"): invalid hexadecimal digit '" + composeCLikeString(std::string(1, a)) + "' encountered");
        }
...
unsigned long CGRuntime::bytesToLong(const std::string& sBytes)
{
    //##protect##"bytesToLong"
    if (sBytes.size() != 8) {
        throw UtlException("'" + composeCLikeString(sBytes) + "' isn't recognized as a 4-bytes sequence");
    }

    unsigned long iLong;

    if (!convertBytesToChars(sBytes, (unsigned char*) &iLong, 4)) {
        throw UtlException("invalid hexadecimal digit in the sequence of bytes '" + composeCLikeString(sBytes) + "'");
    }
...
std::string CGRuntime::charToByte(const std::string& sChar)
{
    //##protect##"charToByte"
    if (sChar.size() > 1) {
        throw UtlException("charToByte('" + composeCLikeString(sChar) + "'): the parameter is bigger than a character");
    }
...
bool CGRuntime::chmod(const std::string& sFilename, const std::string& sMode)
{
    ...
    for (std::string::size_type i = 0; i < sMode.size(); i++) {
        ...
        if ((a == 'R') || (a == 'r')) {
            ...
            throw UtlException("invalid access mode '" + sMode + "' encountered by function 'chmod'");
        }
    }
...
int CGRuntime::findFirstChar(const std::string& sText, const std::string& sSomeChars)
{
    ...
    for (std::string::size_type i = 0; i < sSomeChars.size(); i++) {
        ...
        if ((iPos != std::string::npos) && ((iFirstPos == std::string::npos) || (iFirstPos > iPos))) {
            iFirstPos = iPos;
        }
    }
...
std::string CGRuntime::getEnv(const std::string& sVariable)
{
    ...
    if (tcVariable == NULL) {
        throw UtlException("variable '" + sVariable + "' not found in the environment table");
    }
...
int CGRuntime::hexaToDecimal(const std::string& sHexaNumber)
{
    ...
    for (std::string::size_type i = 0; i < sHexaNumber.size(); i++) {
        ...
        if (a <= '9') {
            if (a < '0') {
                throw UtlException("hexaToDecimal(\"" + composeCLikeString(sHexaNumber) + "\"): invalid hexadecimal digit '" + composeCLikeString(std::string(1, a)) + "' encountered");
            }
...
std::string CGRuntime::hostToNetworkLong(const std::string& sBytes)
{
    ...
    if (!convertBytesToChars(sBytes, (unsigned char*) tcLong, 4)) {
        throw UtlException("invalid hexadecimal representation in '" + composeCLikeString(sBytes) + "'");
    }
...
std::string CGRuntime::hostToNetworkShort(const std::string& sBytes)
{
    ...
    if (sBytes.size() != 4) {
        throw UtlException("'" + composeCLikeString(sBytes) + "' isn't recognized as a 2-bytes sequence");
    }
...
bool CGRuntime::indentFile(const std::string& sFile, const std::string& sMode)
{
    ...
    if ((iIndex == std::string::npos) || (iIndex == sFile.size())) {
        throw UtlException("function 'indentFile(\"" + sFile + "\")' expects a file extension to determine its type for indentation");
    }

    std::string sExtension = sFile.substr(iIndex + 1);
    bool bSuccess;

    if ((stricmp(sMode.c_str(), "c++") == 0) || (stricmp(sExtension.c_str(), "cpp") == 0) || (stricmp(sExtension.c_str(), "cxx") == 0) || (stricmp(sExtension.c_str(), "hxx") == 0) || (stricmp(sExtension.c_str(), "h") == 0)) {
        bSuccess = pFile->indentAsCpp();
    } else if ((stricmp(sMode.c_str(), "java") == 0) || stricmp(sExtension.c_str(), "java") == 0) {
        bSuccess = pFile->indentAsCpp();
    } else {
        throw UtlException("function 'indentFile(\"" + sFile + "\")' works on C++ and JAVA only at the present time, so '" + sExtension + "' extension is refused");
    }
...
double CGRuntime::log(double dX)
{
    //##protect##"log"
    if (dX < 0.0) {
        throw UtlException("the logarithm of a negative floating-point doesn't exist");
    }
...
int CGRuntime::octalToDecimal(const std::string& sOctalNumber)
{
    ...
    for (std::string::size_type i = 0; i < sOctalNumber.size(); i++) {
        ...
        if ((a < '0') || (a > '7')) {
            throw UtlException("octalToDecimal(\"" + composeCLikeString(sOctalNumber) + "\"): invalid hexadecimal digit '" + composeCLikeString(std::string(1, a)) + "' encountered");
        }
...
std::string CGRuntime::pathFromPackage(const std::string& sPackage)
{
    ...
    if (sPath.empty() || ((sPath[sPath.size() - 1] != '\\') && (sPath[sPath.size() - 1] != '/'))) {
        sPath += "/";
    }
...
std::string CGRuntime::resolveFilePath(const std::string& sFilename)
{
    ...
    if (openInputFileFromIncludePath(sFilename.c_str(), sCompleteFileName) == NULL) {
        return "";
    }
...
std::string CGRuntime::system(const std::string& sCommand)
{
    ...
    if (iResult == -1) {
        ...
        case ENOEXEC:
            return "Command interpreter file has invalid format and is not executable";

        case ENOMEM:
            return "Not enough memory is available to execute command; or available memory has been corrupted; or invalid block exists, indicating that process making call was not allocated properly";
...
int CGRuntime::getFloatingLocation(const std::string& sKey)
{
    ...
    if (pOwner != _pOutputStream) {
        throw UtlException("the floating area '" + sKey + "' was put in a precedent markup area, but not in this one.");
    }
