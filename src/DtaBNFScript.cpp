...
public:
    ...
    inline ExprScriptExpression* getFilter() const
    {
        return _pFilter;
    }
    inline void setFilter(ExprScriptExpression* pFilter)
    {
        _pFilter = pFilter;
    }
    ...
    virtual bool applyRuleTransformer(ScpStream& script, GrfBlock& block, int iStatementBeginning, int iProductionRuleBeginning, const std::string& sClauseName, const std::string& sPrototype, const std::string& sProductionRule)
    {
        ...
        if (_pPrototypeScript != NULL) {
            ...
            if (sNewPrototype != sPrototype) {
                script.insertText(sNewPrototype, iStatementBeginning, sPrototype.size());
            }
        }
...
DtaScriptFactory::SCRIPT_TYPE DtaBNFScript::getType() const
{
    return DtaScriptFactory::BNF_SCRIPT;
}
bool DtaBNFScript::isAParseScript() const
{
    return true;
}
bool DtaBNFScript::isAGenerateScript() const
{
    return false;
}
...
void DtaBNFScript::traceEngine() const
{
    if (getFilenamePtr() == NULL) {
        ...
        CGRuntime::traceLine("Extended-BNF script \"" + std::string(getFilenamePtr()) + "\":");
    }
...
void DtaBNFScript::traceInternalEngine() const
{
    ...
    if (_eIgnoreMode == NOT_IGNORE) {
        ...
        CGRuntime::traceLine("\tignore insignificant chars = " + IGNORE_MODEtoString(_eIgnoreMode));
    } else {
...
BNFClause& DtaBNFScript::buildClause(ScpStream& script, GrfBlock& parent, const std::string& sName, const std::string& sTemplateKey, bool bGenericKey, int iReturnType, const std::vector<std::string>& listOfParameters, const std::vector<EXPRESSION_TYPE>& listOfParameterTypes, bool bOverload)
{
            ...
            for (std::string::size_type j = 0; j < listOfParameters.size(); j++) {
                ...
                if (listOfParameters[j] != sClauseParameter) {
                    if (sClauseParameter.empty()) {
                        ...
                        throw UtlException(script, "parameter '" + listOfParameters[j] + "' must be called '" + sClauseParameter + "' into BNF clause declaration");
                    }
                }
            }
    ...
    if (pClause == NULL) {
        if (bOverload) {
            throw UtlException(script, "cannot overload a clause that doesn't exist yet!");
        }

        BNFClause* pTemplateClause = NULL;

        if (!sTemplateKey.empty()) {
            pTemplateClause = &buildClause(script, parent, sName, "", false, iReturnType, listOfParameters, listOfParameterTypes, false);
        }
...
void DtaBNFScript::parsePreprocessorDirective(const std::string& sDirective, ScpStream& script, GrfBlock& block)
{
    if (sDirective == "#applyBNFRule") {
        ...
        if (!script.readIdentifier(sIdentifier)) {
            throw UtlException(script, "the directive #applyBNFRule should be followed by a non-terminal");
        }
        ...
        if (!script.isEqualTo(';')) {
            throw UtlException(script, "syntax error, ';' expected to close the directive #applyBNFRule");
        }
    } else {
...
void DtaBNFScript::parseStepintoHook(GrfBlock& block, ScpStream& script)
{
    if (getStepintoHook() != NULL) {
        throw UtlException(script, "function 'stepintoHook(<clause_signature>, <parameters>)' has already been defined and can't be implemented twice");
    }
    ...
    if (!script.readIdentifier(sIdentifier)) {
        throw UtlException(script, "syntax error: argument name expected for <parameters>");
    }
...
void DtaBNFScript::parseStepoutHook(GrfBlock& block, ScpStream& script)
{
    if (getStepoutHook() != NULL) {
        throw UtlException(script, "function 'StepoutHook(<clause_signature>, <parameters>, <success>)' has already been defined and can't be implemented twice");
    }
    ...
    if (!script.readIdentifier(sIdentifier)) {
        throw UtlException(script, "syntax error: argument name expected for <success>");
    }
...
void DtaBNFScript::parseAndPropagateParameters(ScpStream& script, GrfBlock& block)
{
    ...
    if (!script.isEqualTo(')')) {
        ...
        do {
            ...
            if (!script.isEqualTo(':')) {
                throw UtlException(script, "':' expected after parameter '" + sParameter + "'");
            }
...
BNFClause& DtaBNFScript::parseBNFClause(ScpStream& script, GrfBlock& block, int iStatementBeginning, const std::string& sClauseName, bool bOverload)
{
        ...
        if (script.findString("::=")) {
            ...
            if (!bFail) {
                ...
                for (std::list<BNFTransformRules*>::const_iterator i = _listOfTransformRules.begin(); i != _listOfTransformRules.end(); ++i) {
                    ...
                    if ((*i)->applyRuleTransformer(script, block, iStatementBeginning, iStartProductionRule, sCompleteClauseName, sPrototype, sProductionRule)) {
                        break;
                    }
                }
            }
    ...
    if (script.isEqualTo('<')) {
        ...
        if (!script.readPythonString(sTemplateKey)) {
            if (script.readIdentifier(sTemplateKey)) {
                ...
                throw UtlException(script, "template key expected for clause '" + sClauseName + "'");
            }
        }
        ...
        if (!script.isEqualTo('>')) {
            throw UtlException(script, "'>' to end the template key '" + sTemplateKey + "' for clause '" + sClauseName + "'");
        }
    ...
    if (script.isEqualTo('(')) {
        do {
            ...
            if (!script.readIdentifier(sParameter)) {
                throw UtlException(script, "parameter expected while parsing a BNF clause for declaration");
            }

            if (sParameter == sClauseName) {
                throw UtlException(script, "the parameter '" + sParameter + "' cannot hold the same name as the BNF rule");
            }
            ...
            if (!script.isEqualTo(':')) {
                throw UtlException(script, "':' expected after parameter '" + sParameter + "' while parsing a BNF clause for declaration");
            }
            ...
            if (!script.readIdentifier(sType)) {
                throw UtlException(script, "parameter type expected for '" + sParameter + "' while parsing a BNF clause for declaration");
            }
            ...
            if (sType == "node") {
                ...
                throw UtlException(script, "unknown type '" + sType + "' for parameter '" + sParameter + "' while parsing a BNF clause for declaration");
            }
        ...
        if (!script.isEqualTo(')')) {
            throw UtlException(script, "')' expected while parsing a BNF clause declaration");
        }
    ...
    if (!script.isEqualTo(':')) {
        throw UtlException(script, "'::=' or ':' expected after parsing a BNF clause declaration");
    }
    ...
    if (!script.isEqualTo(':')) {
        ...
        if (script.readIdentifier(sReturnType)) {
            if (sReturnType == "list") {
                ...
                throw UtlException(script, "'list' or 'node' expected instead of '" + sReturnType + "', while parsing return type of a BNF clause for declaration");
            }
            ...
            if (script.isEqualTo("::=")) {
                ...
                throw UtlException(script, "'::=' expected after parsing a BNF clause declaration");
            }
        }
        ...
        if (!bDefinitionExpected) {
            if (script.isEqualTo('#')) {
                if (script.isEqualTo('!')) {
                    if (!script.isEqualToIdentifier("ignore")) {
                        throw UtlException(script, "'#!ignore' expected as preprocessing of a BNF clause");
                    }
    ...
    if (myClause.getNbCommands() > 0) {
        throw UtlException(script, "clause '" + myClause.getSignature() + "' has already been described");
    }
    ...
    if (!script.isEqualTo(';')) {
        throw UtlException(script, "';' expected at the end of a BNF clause definition");
    }
...
ExprScriptVariable* DtaBNFScript::parseLiteralAssignment(ScpStream& script, GrfBlock& block, std::vector<std::string>& listOfConstants, bool& bConcatVariable, bool bVariableAllowed)
{
    ...
    if (script.isEqualTo(':')) {
        ...
        if (script.isEqualTo('\"')) {
            ...
            if (!script.isEqualTo('}')) {
                throw UtlException(script, "'}' expected to close a set of constant strings");
            }
...
bool DtaBNFScript::parseBNFLitteral(ScpStream& script, BNFClause& rule, GrfBlock& block, bool& bContinue, bool& bNoCase, bool bLiteralOnly, BNFStepper* pStepperRE)
{
    ...
    if (script.isEqualTo('\"')) {
        ...
        if (!script.readPythonString(sText)) {
            throw UtlException(script, "string literal expected into BNF clause definition");
        }
        ...
        if (!listOfConstants.empty()) {
            throw UtlException(script, "it has no sense to match a string literal with a constant string");
        }
    } else if (script.isEqualTo('\'')) {
        script.goBack();
        int iChar;
        ...
        if (script.isEqualTo("..")) {
            ...
            if (!script.readCharLiteral(iEndChar)) {
                throw UtlException(script, "char literal expected (between quotes) after '..'");
            }

            if (iChar > iEndChar) {
                throw UtlException(script, "range expression '..' must be ordered correctly");
            }
            ...
            if (!listOfConstants.empty()) {
                throw UtlException(script, "a char literal shouldn't expect a set of constant values");
            }
        }
        ...
        if (script.isEqualTo('(')) {
            ...
            if (!listOfIntermediateConstants.empty()) {
                pFindToken->setIntermediateConstantsToMatch(listOfIntermediateConstants);
            }
        ...
        if (!script.readIdentifier(sIdentifier)) {
            ...
            if (script.isEqualTo('(')) {
                ...
                if (!script.readIdentifier(sVariableName)) {
                    throw UtlException(script, "variable name expected to catch the original syntax error");
                }
                ...
                if (!script.isEqualTo(')')) {
                    throw UtlException(script, "')' expected to close the expression of '#continue'");
                }
            ...
            do {
                ...
                if (script.isEqualTo('#')) {
                    ...
                    if (script.isEqualTo("catch")) {
                        ...
                        if (!script.isEqualTo('(')) {
                            throw UtlException(script, "syntax error, directive '#catch' expects a '('");
                        }
                        ...
                        if (!script.readIdentifier(sVariableName)) {
                            throw UtlException(script, "variable name expected to catch the error message");
                        }
            ...
            if (pBNFTryCatch->getErrorVariable() == NULL) {
                throw UtlException(script, "directive '#catch' expected before the end of the sequence");
            }
        } else if (sIdentifier == "parsedFile") {
            script.skipEmpty();

            if (!isAParseScript()) {
                throw UtlException(script, "directive '#parsedFile' is available on parsing scripts only");
            }
            ...
            if (!isAParseScript()) {
                throw UtlException(script, "directive '#parsedString' is available on parsing scripts only");
            }
            ...
            if (!isAGenerateScript()) {
                throw UtlException(script, "directive '#generatedFile' is available on pattern scripts only");
            }
            ...
            if (!isAGenerateScript()) {
                throw UtlException(script, "directive '#generatedString' is available on pattern scripts only");
            }
            ...
            if (!isAGenerateScript()) {
                throw UtlException(script, "statement modifier 'append_file' is available on pattern scripts only");
            }
            ...
            if (pStepperRE == NULL) {
                throw UtlException(script, "'#nextStep' must be declared into a jump ('->') or repeat ('[...]*') statement");
            }
            ...
            if (script.isEqualTo('(')) {
                ...
                if (!script.isEqualTo(')')) {
                    throw UtlException(script, "')' expected to close the expression of '#break'");
                }
            }
            ...
            if (!script.isEqualTo('(')) {
                throw UtlException(script, "'(' expected after '#pushItem' to pass the array");
            }

            pPushItem->setVariable(parseVariableExpression(block, script));

            if (!script.isEqualTo(')')) {
                throw UtlException(script, "')' expected after passing the array to '#pushItem'");
            }
            ...
            if (!script.isEqualTo('(')) {
                throw UtlException(script, "'(' expected after '#insert' to pass the array");
            }

            pInsert->setVariable(parseVariableExpression(block, script));

            if (!script.isEqualTo(')')) {
                throw UtlException(script, "')' expected after passing the array to '#insert'");
            }
            ...
            if (requiresParsingInformation()) {
                pReadCompleteIdentifier->setParsingInformation(getFilenamePtr(), script);
            }
            ...
            for (;;) {
                ...
                if (script.isEqualTo('#')) {
                    ...
                    if (!script.readIdentifier(sIdentifier)) {
                        throw UtlException(script, "non-terminal call expected by directive '#super'");
                    }
...
BNFClauseCall* DtaBNFScript::parseBNFClauseCall(ScpStream& script, GrfBlock& block, const std::string& sClauseCallName, bool bContinue, bool bNoCase)
{
    ...
    if (script.isEqualTo('<')) {
        ...
        if (!script.isEqualTo('>')) {
            throw UtlException(script, "'>' expected to end the template expression of clause call '" + sClauseCallName + "'");
        }
...
BNFMultiplicityBoundaries* DtaBNFScript::parseMultiplicity(ScpStream& script, GrfBlock& block, BNFMultiplicityBoundaries* boundaries)
{
    ...
    if (script.isEqualTo('*')) {
        ...
        if (script.readInt(iBegin)) {
            if (iBegin < 0) {
                throw UtlException(script, "multiplicity 'm..n' or 'm' doesn't allow negative boundaries");
            }
            ...
            if (script.isEqualTo("..")) {
                ...
                if (script.isEqualTo('*') || script.isEqualTo('n')) {
                    ...
                    if (!script.readInt(iEnd)) {
                        throw UtlException(script, "multiplicity expected as 'm..n' format");
                    }

                    if (iEnd < iBegin) {
                        throw UtlException(script, "multiplicity 'm..n' doesn't allow 'm' being greater than 'n'");
                    }
                }
            } else {
                iEnd = iBegin;
...
std::string DtaBNFScript::assignmentToString(ExprScriptVariable* pVariableToAssign, bool bConcatenateVariable)
{
    if (pVariableToAssign != NULL) {
        return ((bConcatenateVariable) ? ":+" : ":") + pVariableToAssign->toString();
    }
...
IGNORE_MODE DtaBNFScript::parseIgnoreMode(ScpStream& script, BNFClause*& pPreprocessingIgnoreClause)
{
    ...
    if (script.isEqualTo('(')) {
        ...
        if (!script.isEqualTo(')')) {
            throw UtlException(script, "')' or correct mode expected by clause '#ignore'");
        }
    } else {
        eMode = IGNORE_CLAUSE;
