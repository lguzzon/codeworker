...
BNFTryCatch::BNFTryCatch(DtaBNFScript* pBNFScript)
  : _pBNFScript(pBNFScript)
{}

BNFTryCatch::BNFTryCatch(DtaBNFScript* pBNFScript, GrfBlock* pParent)
  : _pBNFScript(pBNFScript)
{
...
void
BNFTryCatch::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFTryCatch(*this, env);
}

bool
BNFTryCatch::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFTryCatch::executeInternal(DtaScriptVariable& visibility)
{
  ...
  try {
    ...
    result = executeCatchStatement(
      visibility, iLocation, iImplicitCopyPosition, exception);
  } catch (std::exception& exceptSTL) {
    result = executeCatchStatement(
      visibility, iLocation, iImplicitCopyPosition, exceptSTL.what());
  } catch (...) {
    result = executeCatchStatement(visibility,
                                   iLocation,
                                   iImplicitCopyPosition,
                                   "ellipsis exception (internal error)");
  }
  return result;
}

SEQUENCE_INTERRUPTION_LIST
BNFTryCatch::executeCatchStatement(DtaScriptVariable& visibility,
                                   int iLocation,
                                   int iImplicitCopyPosition,
                                   UtlException& exception)
{
  return executeCatchStatement(visibility,
                               iLocation,
                               iImplicitCopyPosition,
...
BNFTryCatch::executeCatchStatement(DtaScriptVariable& visibility,
                                   int iLocation,
                                   int iImplicitCopyPosition,
                                   const std::string& sCatchedMessage,
                                   const std::string& sCatchedStack)
{
...
void
BNFTryCatch::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_STREAM << " catch(UtlException& _compilerException_"
                 << getErrorVariable()->getName() << ") {";
  CW_BODY_ENDL;
  theCompilerEnvironment.incrementIndentation();
  if (_pBNFScript->hasCoverageRecording()) {
    // coverage recording required
    CW_BODY_INDENT << "_compilerClauseMatching.purgeChildsAfterPosition("
                   << tcLocation << ");";
    CW_BODY_ENDL;
  }
  CW_BODY_INDENT;
  getErrorVariable()->compileCppForSet(theCompilerEnvironment);
  CW_BODY_STREAM << ".setValue(_compilerException_"
                 << getErrorVariable()->getName() << ".getMessage());";
  CW_BODY_ENDL;
...
BNFContinue::BNFContinue(DtaBNFScript* pBNFScript,
                         GrfBlock* pParent,
                         ExprScriptVariable* pVariable,
                         ExprScriptExpression* pMessage)
  : BNFTryCatch(pBNFScript)
  , _pErrorMessage(pMessage)
{
  setTryBlock(new GrfBlock(pParent));
  setCatchBlock(new GrfBlock(pParent));
...
void
BNFContinue::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFContinue(*this, env);
}

SEQUENCE_INTERRUPTION_LIST
BNFContinue::executeCatchStatement(DtaScriptVariable& visibility,
                                   int iLocation,
                                   int iImplicitCopyPosition,
                                   UtlException& exception)
{
