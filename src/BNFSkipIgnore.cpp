...
BNFSkipIgnore::BNFSkipIgnore(DtaBNFScript* pBNFScript,
                             GrfBlock* pParent,
                             IGNORE_MODE eMode,
...
void
BNFSkipIgnore::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFSkipIgnore(*this, env);
}

bool
BNFSkipIgnore::isABNFCommand() const
{
  return true;
}

void
BNFSkipIgnore::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                   bool bConcat,
                                   BNFClause& theClause)
{
...
BNFSkipIgnore::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if (!_listOfConstants.empty()) {
    sValue =
      CGRuntime::getLastReadChars(CGRuntime::getInputLocation() - iLocation);
    bool bMatch = false;
...
void
BNFSkipIgnore::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
