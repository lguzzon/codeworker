...
BNFConjunction::BNFConjunction(DtaBNFScript* pBNFScript, GrfBlock* pParent)
...
void
BNFConjunction::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFConjunction(*this, env);
}

bool
BNFConjunction::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFConjunction::execute(DtaScriptVariable& visibility)
{
  ...
  if (result == BREAK_INTERRUPTION) {
    if (_pBNFScript->getParentClauseMatching() != NULL) {
      ...
      _pBNFScript->getParentClauseMatching()->purgeChildsAfterPosition(
        iLocation);
    }
...
void
BNFConjunction::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (i != getCommands().end()) {
    ...
    CW_BODY_INDENT << "int " << tcLocation
                   << " = CGRuntime::getInputLocation();";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "int " << tcImplicitCopy
                   << " = ((theEnvironment.implicitCopy()) ? "
                      "CGRuntime::getOutputLocation() : -1);";
    CW_BODY_ENDL;
