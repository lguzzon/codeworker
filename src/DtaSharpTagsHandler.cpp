...
public:
    DtaClassicalSharpTagsHandler(ScpStream* pInputStream) : DtaSharpTagsHandlerImpl(pInputStream)
    {
        if ((DtaProject::getInstance().getTextMode() == DtaProject::DOS_MODE) && (DtaProject::getInstance().getCommentEnd() == "\n")) {
            ...
            _sCommentEnd = DtaProject::getInstance().getCommentEnd();
        }
    }
    ...
    virtual bool findExpansionMarkup()
    {
        ...
        if (!_pInputStream->findString(sMarker)) {
            return false;
        }

        if (!_pInputStream->isEqualTo("er##") && !_pInputStream->isEqualTo("up##")) {
            throw UtlException(*_pInputStream, "unrecognized markup format: '##markup##' or '##marker##' expected");
        }

        _pInputStream->skipEmpty();
        std::string sMarkerKey;

        if (_pInputStream->readString(sMarkerKey)) {
            _bKeyIsIdentifier = false;
        } else if (_pInputStream->readIdentifier(sMarkerKey)) {
            _bKeyIsIdentifier = true;
        } else {
        ...
        while (!_pInputStream->isEqualTo(DtaProject::getInstance().getCommentEnd())) {
            ...
            if ((a < 0) || (a > (int) ' ')) {
                throw UtlException(*_pInputStream, "end of comment expected after '##markup##\"" + DtaProject::getInstance().getMarkupKey() + "\"'");
            }
        }
    ...
    virtual bool readMarkupScript(std::string& sScript)
    {
        ...
        if (!_pInputStream->isEqualTo(sScriptMarker)) {
            return false;
        }
        ...
        while (!_pInputStream->isEqualTo(DtaProject::getInstance().getCommentEnd())) {
            ...
            if ((a < 0) || (a > (int) ' ')) {
                throw UtlException(*_pInputStream, "end of comment expected after '##script##'");
            }
        }

        int iStartScript = _pInputStream->getInputLocation();

        if (!_pInputStream->findString(sScriptMarker)) {
            throw UtlException(*_pInputStream, "'##script##' expected to close the template-based script block of markup '" + DtaProject::getInstance().getMarkupKey() + "'");
        }
    ...
    virtual bool readMarkupData(std::string& sData)
    {
        ...
        if (!_pInputStream->isEqualTo(sDataMarker)) {
            return false;
        }
        ...
        while (!_pInputStream->isEqualTo(DtaProject::getInstance().getCommentEnd())) {
            ...
            if ((a < 0) || (a > (int) ' ')) {
                throw UtlException(*_pInputStream, "end of comment expected after '##data##'");
            }
        }

        int iStartData = _pInputStream->getInputLocation();

        if (!_pInputStream->findString(sDataMarker)) {
            throw UtlException(*_pInputStream, "'##data##' expected to close the data block of markup '" + DtaProject::getInstance().getMarkupKey() + "'");
        }
    ...
    {
        if (DtaProject::getInstance().getCommentBegin().empty() || !_pInputStream->isEqualTo(DtaProject::getInstance().getCommentBegin() + "##begin##")) {
            return false;
        }

        std::string sStringMarkerKey = "\"" + CGRuntime::composeCLikeString(DtaProject::getInstance().getMarkupKey()) + "\"";

        if (!_pInputStream->isEqualTo(sStringMarkerKey)) {
            throw UtlException(*_pInputStream, "'##begin##' tag is expected to concern marker called '" + sStringMarkerKey + "'");
        }

        if (!_pInputStream->findString(DtaProject::getInstance().getCommentEnd())) {
            throw UtlException(*_pInputStream, "'##begin##' tag is expected to end with an end of comment marker");
        }
    ...
    virtual int locateMarkupEnd()
    {
        ...
        if (!_pInputStream->findString(sMarkupBegin)) {
            throw UtlException(*_pInputStream, "'##begin##' tag is expected to have an associated '##end##' tag");
        }
        ...
        if (!_pInputStream->isEqualTo(sStringMarkerKey)) {
            throw UtlException(*_pInputStream, "'##end##' tag is expected to concern marker called " + sStringMarkerKey);
        }

        if (!_pInputStream->findString(DtaProject::getInstance().getCommentEnd())) {
            throw UtlException(*_pInputStream, "'##end##' tag is expected to end with an end of comment marker");
        }
    ...
    virtual void writeMarkupBegin(ScpStream& outputStream)
    {
        ...
        if (_bKeyIsIdentifier) {
            outputStream << DtaProject::getInstance().getMarkupKey();
        } else {
            outputStream.writeString(DtaProject::getInstance().getMarkupKey());
        }
    ...
    virtual bool findGenerationProtectBegin(int iEnd)
    {
        ...
        if (iEnd < 0) {
            if (!_pInputStream->findString(sBeginning.c_str())) {
                return false;
            }
        } else {
            if (!_pInputStream->findString(sBeginning.c_str(), iEnd)) {
                return false;
            }
        }
        ...
        if (_pInputStream->readIdentifier(_sProtectDefine)) {
            if (bNotProtectDefine) {
                _sProtectDefine = "!" + _sProtectDefine;
            }
        ...
        if (!_pInputStream->isEqualTo("##")) {
            throw UtlException(*_pInputStream, "wrong syntax for announcing protected area, '##' expected");
        }

        if (!_pInputStream->readString(_sProtectKey)) {
            throw UtlException(*_pInputStream, "wrong syntax for announcing protected area");
        }

        if (!_pInputStream->findString(DtaProject::getInstance().getCommentEnd().c_str())) {
            throw UtlException(*_pInputStream, "end of comment expected after announcing protected area \"" + _sProtectKey + "\"");
        }
    ...
    virtual int locateGenerationProtectEnd()
    {
        ...
        if (!_pInputStream->findString(sBeginning.c_str())) {
            throw UtlException(*_pInputStream, "end of protected area \"" + _sProtectKey + "\" expected");
        }
        ...
        if (sProtectDefine != _sProtectDefine) {
            throw UtlException(*_pInputStream, "wrong syntax for finishing protected area \"" + _sProtectKey + "\", defines are different");
        }

        if (!_pInputStream->isEqualTo("##")) {
            throw UtlException(*_pInputStream, "wrong syntax for finishing protected area \"" + _sProtectKey + "\", '##' expected");
        }

        std::string sProtectKey;

        if (!_pInputStream->readString(sProtectKey)) {
            throw UtlException(*_pInputStream, "wrong syntax for finishing protected area \"" + _sProtectKey + "\"");
        }

        if (sProtectKey != _sProtectKey) {
            throw UtlException(*_pInputStream, "the key \"" + sProtectKey + "\" of the preserved area's end doesn't match with key \"" + _sProtectKey + "\", announced at the beginning");
        }

        if (!_pInputStream->findString(DtaProject::getInstance().getCommentEnd().c_str())) {
            throw UtlException(*_pInputStream, "end of comment expected after finishing protected area \"" + _sProtectKey + "\"");
        }
    ...
    virtual void writeProtectBegin(ScpStream& outputStream, const std::string& sKey, const std::string& sDefine)
    {
        ...
        if ((DtaProject::getInstance().getTextMode() == DtaProject::DOS_MODE) && (DtaProject::getInstance().getCommentEnd() == "\n")) {
            ...
            outputStream << DtaProject::getInstance().getCommentEnd();
        }
    }
...
public:
    virtual bool findExpansionMarkup()
    {
        ...
        if (pVariable == NULL) {
            throw UtlException("field 'this.markup_key' should have been populated by the 'find_expansion_markup' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual bool readMarkupScript(std::string& sScript)
    {
        ...
        if (pVariable == NULL) {
            throw UtlException("field 'this.script' should have been populated by the 'read_markup_script' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual bool readMarkupData(std::string& sData)
    {
        ...
        if (pVariable == NULL) {
            throw UtlException("field 'this.data' should have been populated by the 'read_markup_data' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual int locateMarkupEnd()
    {
        ...
        if (!executeReader()) {
            throw UtlException(*_pInputStream, "unable to locate the end of the markup '" + DtaProject::getInstance().getMarkupKey() + "'");
        }

        DtaScriptVariable* pVariable = _context.getNode("beginning_position");

        if (pVariable == NULL) {
            throw UtlException("field 'this.beginning_position' should have been populated by the 'locate_markup_end' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual bool findGenerationProtectBegin(int iEnd)
    {
        ...
        if (iEnd >= 0) {
            _context.insertNode("end_position")->setValue(iEnd);
        } else {
            _context.remove("end_position");
        }
        ...
        if (pVariable == NULL) {
            throw UtlException("field 'this.id' should have been populated by the 'find_generation_protect_begin' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual int locateGenerationProtectEnd()
    {
        ...
        if (!executeReader()) {
            throw UtlException(*_pInputStream, "unable to locate the end of the protected area '" + getProtectKey() + "'");
        }

        DtaScriptVariable* pVariable = _context.getNode("beginning_position");

        if (pVariable == NULL) {
            throw UtlException("field 'this.beginning_position' should have been populated by the 'locate_generation_protect_end' reader of the generation tags handler designated by key '" + DtaProject::getInstance().getCurrentGenerationTagsHandler() + "'");
        }
    ...
    virtual void writeProtectBegin(ScpStream& outputStream, const std::string& sKey, const std::string& sDefine)
    {
        ...
        if (!sDefine.empty()) {
            _context.insertNode("property")->setValue(sDefine.c_str());
        } else {
...
DtaSharpTagsHandler::DtaSharpTagsHandler(ScpStream* pInputStream)
{
    ...
    if (sKey.empty()) {
        ...
        if (!DtaProject::getInstance().getGenerationTagsHandler(sKey, pReader, pWriter)) {
            throw UtlException(*pInputStream, "internal error: no generation tags handler registered to the key '" + sKey + "'");
        }
