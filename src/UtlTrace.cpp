...
void
UtlTrace::traceFormat(int iLevel, const char* sFormat, ...)
{
...
#ifdef WIN32
          case 'c':
            *sDestination++ = va_arg(marker, char);
            break;
#else
          case 'c':
            *sDestination++ = va_arg(marker, int);
            break;
#endif
          case 'd':
...
void
UtlTrace::traceText(const char* sSourceFile, int iLineCode, const char* sText)
{
  char sBuffer[1024];
  sprintf(sBuffer,
          "%s%s(%d):\t%s (%d)",
...
void
UtlTrace::writeText(const char* sText)
{
  if (!_sTraceFile.empty()) {
    if (_bPersistent) {
      std::auto_ptr<std::ofstream> f(
        CodeWorker::openAppendFile(_sTraceFile.c_str()));
      if (f.get() != NULL) {
        (*f) << sText << std::endl;
        f->close();
      }
...
void
UtlTrace::initialize(const char* sTraceFile,
                     bool bPersistent,
                     int iLowerLevel,
                     int iUpperLevel)
{
...
void
UtlTrace::setConfiguration(const char* sTraceFile,
                           bool bPersistent,
                           int iLowerLevel,
                           int iUpperLevel)
{
  ...
  if ((sTraceFile == NULL) || (sTraceFile[0] == '\0'))
    _sTraceFile = "";
  else {
    _sTraceFile = sTraceFile;
    std::auto_ptr<std::ofstream> pTraceFile(
      CodeWorker::openOutputFile(_sTraceFile.c_str()));
    _pFile = pTraceFile;
...
void
UtlTrace::pushTraceMethod(const char* sTraceMethod)
{
  if (!_sTraceFile.empty()) {
    char sBuffer[1024];
...
void
UtlTrace::traceStackInstruction(const char* sFileName, int iLine)
{
  if (_iTraceSize == 0)
    traceStackFunction(sFileName, "main scope", iLine);
  _tsTraceFileStack[_iTraceSize - 1] = sFileName;
  _tiTraceLocationStack[_iTraceSize - 1] = iLine;
}

void
UtlTrace::traceStackFunction(const char* sFileName,
                             ...
                             int iLine)
{
  if (_iTraceSize >= _iMaxTraceSize) {
    ...
    memcpy(_tsTraceFileStack,
           tsTraceFileStack,
           sizeof(const char*) * _iMaxTraceSize);
    delete[] tsTraceFileStack;
    int* tiTraceLocationStack = _tiTraceLocationStack;
    _tiTraceLocationStack = new int[iNextMaxTraceSize];
    memcpy(_tiTraceLocationStack,
           tiTraceLocationStack,
           sizeof(int*) * _iMaxTraceSize);
...
UtlTrace::getTraceStack()
{
  ...
  for (int i = 0; i < _iTraceSize; i++) {
    if (_tsTraceFileStack[i] != NULL) {
      ...
      std::auto_ptr<std::ifstream> pFile(
        openInputFileFromIncludePath(_tsTraceFileStack[i], sCompleteFileName));
      if (pFile.get() == NULL) {
        sprintf(tcNumber,
                "\"%s\" offset[%d]: ",
                _tsTraceFileStack[i],
                _tiTraceLocationStack[i]);
      } else {
        ...
        if (iIndex != std::string::npos)
          sShortFilename = sShortFilename.substr(iIndex + 1);
...
int*
UtlTrace::copyTraceLocationStack()
{
  int* tiTraceLocationStack = new int[_iTraceSize];
  memcpy(
    tiTraceLocationStack, _tiTraceLocationStack, _iTraceSize * sizeof(int));
  return tiTraceLocationStack;
}

void
UtlTrace::writeTraceText(const char* sText)
{
  if (!_sTraceFile.empty()) {
    std::auto_ptr<std::ofstream> f(
      CodeWorker::openAppendFile(_sTraceFile.c_str()));
...
UtlTraceStackFunction::UtlTraceStackFunction(const char* sFileName,
                                             const char* sFunctionName,
                                             int iLine)
{
