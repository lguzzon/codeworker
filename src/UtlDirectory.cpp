...
UtlFile::UtlFile(UtlDirectory& myParent, const std::string& sFileName)
  : _pParent(&myParent)
  , _sFileName(sFileName)
{}

UtlFile::UtlFile(const std::string& sFileName)
  : _pParent(NULL)
  , _sFileName(sFileName)
{}
...
std::string
UtlFile::getFileNameExtension() const
{
  std::string::size_type iIndex = _sFileName.find(".");
  if (iIndex == std::string::npos)
    return "";
  iIndex++;
  if (_sFileName.size() == iIndex)
    return "";
...
UtlDirectory::UtlDirectory(UtlDirectory& myParent,
                           const std::string& sDirectoryName)
  : _bScanned(false)
  , _pParent(&myParent)
...
UtlDirectory::UtlDirectory(const std::string& sDirectoryName)
  : _bScanned(false)
  , _pParent(NULL)
  , _sDirectoryName(sDirectoryName)
{}
...
UtlDirectory::~UtlDirectory()
{
  for (std::list<UtlFile*>::const_iterator iterateFiles = _listOfFiles.begin();
       iterateFiles != _listOfFiles.end();
       iterateFiles++) {
    delete (*iterateFiles);
  }
  for (std::list<UtlDirectory*>::const_iterator iterateDirectories =
         _listOfDirectories.begin();
       iterateDirectories != _listOfDirectories.end();
       iterateDirectories++) {
...
UtlDirectory::getSubdirectory(const std::string& sDirectoryName) const
{
  if (!_bScanned)
    throw UtlException(
      "can't call 'UtlDirectory(\"" + getFullPath() + "\").getSubdirectory(\"" +
      sDirectoryName +
      "\")': its files and directories must be scanned before");
  for (std::list<UtlDirectory*>::const_iterator iterateDirectories =
         _listOfDirectories.begin();
       iterateDirectories != _listOfDirectories.end();
       iterateDirectories++) {
    if ((*iterateDirectories)->getDirectoryName() == sDirectoryName)
      return (*iterateDirectories);
  }
  return NULL;
}

std::string
UtlDirectory::getFullPath() const
{
...
std::string
UtlDirectory::getRelativePath() const
{
  std::string sRelativePath;
  if (_pParent != NULL)
    sRelativePath = _pParent->getRelativePath();
...
bool
UtlDirectory::createDirectory(const std::string& sDirectory)
{
...
bool
UtlDirectory::scan(const std::string& sExtendedPattern)
{
  bool bSuccess = true;
  if (_bScanned) {
    _bScanned = false;
    ...
    for (std::list<UtlFile*>::const_iterator iterateFiles =
           _listOfFiles.begin();
         iterateFiles != _listOfFiles.end();
         iterateFiles++) {
      delete (*iterateFiles);
  ...
  if (iIndex != iTempIndex) {
    if ((iIndex == std::string::npos) ||
        ((iIndex < iTempIndex) && (iTempIndex != std::string::npos)))
      iIndex = iTempIndex;
    iIndex++;
    if (iIndex < sPattern.size())
      sSuffix = sPattern.substr(iIndex);
  }
  while (bMore) {
    if ((myFindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) {
      ...
      if (!sSuffix.empty() && (iLength > sSuffix.size()) &&
          (strcmp(tcName + (iLength - sSuffix.size()), sSuffix.c_str()) != 0)) {
  ...
  bSuccess = bMore;
  while (bMore) {
    if ((myDirectoriesFindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) !=
        0) {
  ...
  if (glob(sDirectoryPath.c_str(), GLOB_DOOFFS | GLOB_MARK, NULL, &globbuf) ==
      0) {
    for (int i = 0; i < globbuf.gl_pathc; ++i) {
      ...
      if (sDirectoryName[iIndex] == '/') {
        ...
        if (iIndex != std::string::npos)
          sDirectoryName = sDirectoryName.substr(iIndex + 1);
...
bool
UtlDirectory::matchPatternDirectory(const std::string& sPatternDirectory)
{
  if (sPatternDirectory.empty())
    return true;
  std::string::size_type iIndex = sPatternDirectory.find_last_of("/\\");
  if (iIndex == std::string::npos) {
    return (_sDirectoryName == sPatternDirectory);
  }
  if (_sDirectoryName != sPatternDirectory.substr(iIndex + 1))
    return false;
  return matchPatternDirectory(sPatternDirectory.substr(0, iIndex));
}

bool
UtlDirectory::scanRecursively(const std::string& sPattern)
{
...
UtlDirectory::getTmpDirectory()
{
  ...
  if (((pRepTmp1 == NULL) || (pRepTmp1[0] == '\0')) &&
      ((pRepTmp2 == NULL) || (pRepTmp2[0] == '\0'))) {
    THROW_UTLEXCEPTION("Environnement variable TMP or TEMP must be defined for "
                       "method UtlDirectory::getTmpDirectory");
  }
  ...
  if ((pRepTmp1 != NULL) && (pRepTmp1[0] != '\0')) {
    ...
    if (pRepTmp1[iSize] == '\\')
      sRepTmp = sRepTmp.substr(0, iSize) + "/";
    else if (pRepTmp1[iSize] != '/')
      sRepTmp += "/";
