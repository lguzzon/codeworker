...
{
    if (!_bScanned) {
        throw UtlException("can't call 'UtlDirectory(\"" + getFullPath() + "\").getSubdirectory(\"" + sDirectoryName + "\")': its files and directories must be scanned before");
    }
...
bool UtlDirectory::scan(const std::string& sExtendedPattern)
{
    ...
    if (iIndex != iTempIndex) {
        if ((iIndex == std::string::npos) || ((iIndex < iTempIndex) && (iTempIndex != std::string::npos))) {
            iIndex = iTempIndex;
        }
