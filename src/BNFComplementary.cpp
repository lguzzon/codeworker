...
BNFComplementary::BNFComplementary(DtaBNFScript* pBNFScript,
                                   GrfBlock* pParent,
                                   bool bContinue)
...
void
BNFComplementary::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFComplementary(*this, env);
}

SEQUENCE_INTERRUPTION_LIST
BNFComplementary::executeInternal(DtaScriptVariable& visibility)
{
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (i != getCommands().end()) {
    ...
    if (_pBNFScript->hasCoverageRecording()) {
      ...
      CW_BODY_INDENT << "_compilerClauseMatching.purgeChildsAfterPosition(_"
                        "compilerClauseLocation_"
                     << iCursor << ");";
      CW_BODY_ENDL;
    }
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT
      << "\tCGRuntime::setInputLocation(_compilerClauseNotEmptyLocation_"
      << iCursor << ");";
    CW_BODY_ENDL;
    ...
    if (!_listOfConstants.empty()) {
      ...
      for (std::vector<std::string>::const_iterator i =
           ...
           i != _listOfConstants.end();
           i++) {
        CW_BODY_INDENT << "\t\tcase " << (int)((unsigned char)((*i)[0]))
                       << ": // '" << CGRuntime::composeCLikeString(*i) << "'";
        CW_BODY_ENDL;
      }
    ...
    CW_BODY_INDENT << "if (_compilerClauseSuccess) {";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tif (_compilerClauseImplicitCopy_" << iCursor
                   << " >= 0) {";
    CW_BODY_ENDL;
...
std::string
BNFComplementary::toString() const
{
  std::string sText =
