...
void BNFComplementary::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (i != getCommands().end()) {
        ...
        if (_pBNFScript->hasCoverageRecording()) {
            ...
            CW_BODY_INDENT << "_compilerClauseMatching.purgeChildsAfterPosition(_compilerClauseLocation_" << iCursor << ");";
            CW_BODY_ENDL;
        }
        ...
        CW_BODY_INDENT << "if (_compilerClauseSuccess) {";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\tif (_compilerClauseImplicitCopy_" << iCursor << " >= 0) {";
        CW_BODY_ENDL;
