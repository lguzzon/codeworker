...
void DtaProtectedAreasBag::setProtection(const char* sProtection, const char* sContent)
{
    ...
    if (i != _codes.end()) {
        if (i->second->isAlreadyGenerated()) {
            throw UtlException(std::string("protected area called \"") + sProtection + "\" has already been generated");
        }
...
bool DtaProtectedAreasBag::recoverProtectedCode(DtaSharpTagsHandler& tagsHandler, int iEnd)
{
    ...
    if (_codes.find(tagsHandler.getProtectKey()) != _codes.end()) {
        throw UtlException(inputFile, "a protected area called \"" + tagsHandler.getProtectKey() + "\" already exists");
    }

    int iBeginCode = inputFile.getInputLocation();
    int iEndCode = tagsHandler.locateGenerationProtectEnd();

    if ((iBeginCode < 0) || (iBeginCode > iEndCode) || (iEndCode > inputFile.size())) {
        throw UtlException(inputFile, "inconsistence in boundaries of the protected area called \"" + tagsHandler.getProtectKey() + "\"");
    }
