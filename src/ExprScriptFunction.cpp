...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'first(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an index");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'first(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an index\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : index' in the prototype");
    if (pIteratorData->first())
      return "true";
    return "";
  }

  virtual bool compileCppBoolean(CppCompilerEnvironment& theCompilerEnvironment,
                                 bool bNegative) const
  {
    ...
    if (pForeach->getSorted()) {
      sModifier = "Sorted";
      if (pForeach->getSortedNoCase())
        sModifier += "NoCase";
    }
    if (bNegative) {
      CW_BODY_STREAM << "_compilerIndex" << sModifier << "Foreach_"
                     << ((ExprScriptVariable*)_parameters.front())->getName()
                     << " != _compilerMap" << sModifier << "Foreach_"
                     << ((ExprScriptVariable*)_parameters.front())->getName()
                     << "->begin()";
    } else {
      CW_BODY_STREAM << "_compilerIndex" << sModifier << "Foreach_"
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptFirst; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'last(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an index");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'last(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an index\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : index' in the prototype");
    if (pIteratorData->last())
      return "true";
    return "";
  }
                                 ...
                                 bool bNegative) const
  {
    ...
    if (bNegative) {
      CW_BODY_STREAM << "CGRuntime::nextIteration(_compilerIndex" << sModifier
                     << "Foreach_"
                     << ((ExprScriptVariable*)_parameters.front())->getName()
                     << ") != _compilerMap" << sModifier << "Foreach_"
                     << ((ExprScriptVariable*)_parameters.front())->getName()
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptLast; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'key(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an index");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'key(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an index\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : index' in the prototype");
  ...
  virtual EXPRESSION_RETURN_TYPE compileCpp(
  ...
  virtual bool compileCppString(
    CppCompilerEnvironment& theCompilerEnvironment) const
  {
    CW_BODY_STREAM << "_compilerKeyForeach_"
                   << ((ExprScriptVariable*)_parameters.front())->getName();
    return true;
  }

  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptKey; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'index(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an iterator");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'index(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an iterator\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : index' in the prototype");
    char tcResult[32];
  ...
  virtual bool compileCppInt(CppCompilerEnvironment& theCompilerEnvironment,
                             bool bNegative) const
  {
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptIndex; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'prec(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an iterator");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'prec(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an iterator\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : Prec' in the prototype");
    if (pIteratorData->prec())
      return "true";
    return "";
  }
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptPrec; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    DtaScriptVariable* pIndexExpr =
      visibility.getIterator(pIndexParam->getName().c_str());
    if (pIndexExpr == NULL)
      throw UtlException("'next(" + pIndexParam->getName() +
                         ")' has no sense: '" + pIndexParam->getName() +
                         "' isn't declared as an iterator");
    DtaArrayIterator* pIteratorData = pIndexExpr->getIteratorData();
    if (pIteratorData == NULL)
      throw UtlException(
        "'next(" + pIndexParam->getName() + ")' can't work here: '" +
        pIndexParam->getName() + "' is a reference to an iterator\nif '" +
        pIndexParam->getName() + "' is a function parameter, write '" +
        pIndexParam->getName() + " : Next' in the prototype");
    if (pIteratorData->next())
      return "true";
    return "";
  }
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptNext; }
...
public:
  ...
  virtual bool compileCppBoolean(CppCompilerEnvironment& theCompilerEnvironment,
                                 bool bNegative) const
  {
    return _parameters.front()->compileCppBoolean(theCompilerEnvironment,
                                                  !bNegative);
  }
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptNot; }
...
public:
  ...
  {
    _iCounter++;
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptAdd; }
...
public:
  virtual ~ExprScriptAddGenerationTagsHandler()
  {
    ...
    if (_pReaderCache != NULL)
      delete _pReaderCache;
    if (_pWriterCache != NULL)
      delete _pWriterCache;
    //##protect##"addGenerationTagsHandler::destructor"
  }
  ...
  virtual EXPRESSION_TYPE getParameterType(unsigned int iIndex) const
  {
    if (iIndex == 0)
      return VALUE_EXPRTYPE;
    if (iIndex == 1)
      return SCRIPTFILE_BNF_EXPRTYPE;
    if (iIndex == 2)
      return SCRIPTFILE_PATTERN_EXPRTYPE;
    return UNKNOWN_EXPRTYPE;
  }
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    if (returnValue) {
      ...
      ExprScriptScriptFile* pBNFFileName =
        dynamic_cast<ExprScriptScriptFile*>(_parameters[1]);
      if (pBNFFileName->isFileName()) {
        if (_pReaderCache == NULL) {
          std::string sScriptFile =
            pBNFFileName->getFileName()->getValue(visibility);
      ...
      ExprScriptScriptFile* pPatternFileName =
        dynamic_cast<ExprScriptScriptFile*>(_parameters[2]);
      if (pPatternFileName->isFileName()) {
        if (_pWriterCache == NULL) {
          std::string sScriptFile =
            pPatternFileName->getFileName()->getValue(visibility);
      ...
      returnValue = DtaProject::getInstance().addGenerationTagsHandler(
        sKey, pReader, pWriter);
    }
    //##protect##"addGenerationTagsHandler::execute"
    return (returnValue ? "true" : "");
  }
...
public:
  ...
  virtual bool compileCppUnsignedLong(
    CppCompilerEnvironment& theCompilerEnvironment) const
  {
...
public:
  ...
  virtual bool compileCppUnsignedShort(
    CppCompilerEnvironment& theCompilerEnvironment) const
  {
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    int returnValue =
      CGRuntime::changeFileTime(sFilename, sAccessTime, sModificationTime);
    char tcNumber[16];
    sprintf(tcNumber, "%d", returnValue);
    return tcNumber;
  }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptCharAt; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptChmod; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptCeil; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    bool returnValue =
      CGRuntime::copySmartFile(sSourceFileName, sDestinationFileName);
    return (returnValue ? "true" : "");
  }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    DtaScriptVariable* pI = visibility.getIterator(
      dynamic_cast<ExprScriptVariable*>(*cursor)->getName().c_str());
    if (pI == NULL)
      pI = visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    ++cursor;
    DtaScriptVariable* pList = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    DtaScriptVariable* pNumber =
      visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    ...
    CppCompilerEnvironment& theCompilerEnvironment) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    dynamic_cast<ExprScriptVariable*>(*cursor)->compileCppForSet(
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptDiv; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    DtaScriptVariable* pOldIt = visibility.getIterator(
      dynamic_cast<ExprScriptVariable*>(*cursor)->getName().c_str());
    if (pOldIt == NULL)
      pOldIt =
        visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    ++cursor;
    DtaScriptVariable* pNewIt = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptEndl; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptEqual; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    DtaScriptVariable* pFirstTree = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
    ++cursor;
    DtaScriptVariable* pSecondTree = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::vector<ExprScriptExpression*>::const_iterator cursor =
      _parameters.begin();
    DtaScriptVariable* pVariable = visibility.getNoWarningExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptExp; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    bool returnValue =
      CGRuntime::exploreDirectory(pDirectory, sPath, bSubfolders);
    return (returnValue ? "true" : "");
  }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::string sFilename = (*cursor)->getValue(visibility);
    ++cursor;
    DtaScriptVariable* pGenerator =
      visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    std::string sGenerator;
    const char* tsGenerator = pGenerator->getValue();
    if (tsGenerator != NULL)
      sGenerator = tsGenerator;
    ++cursor;
    DtaScriptVariable* pVersion =
      visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    std::string sVersion;
    ...
    if (tsVersion != NULL)
      sVersion = tsVersion;
    ++cursor;
    DtaScriptVariable* pDate =
      visibility.getVariable(*dynamic_cast<ExprScriptVariable*>(*cursor));
    std::string sDate;
    const char* tsDate = pDate->getValue();
    if (tsDate != NULL)
      sDate = tsDate;
    std::string returnValue = CGRuntime::extractGenerationHeader(
      sFilename, sGenerator, sVersion, sDate);
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    ++cursor;
    DtaScriptVariable* pVariable = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    ++cursor;
    DtaScriptVariable* pArray = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    int returnValue =
      CGRuntime::findNextSubstringIntoKeys(sSubstring, pArray, iNext);
    char tcNumber[16];
    sprintf(tcNumber, "%d", returnValue);
    return tcNumber;
  }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptFloor; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptGetEnv; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::string returnValue =
      CGRuntime::getHTTPRequest(sURL, pHTTPSession, pArguments);
    return returnValue;
  }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptGetNow; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptInf; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptIsEmpty; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptLog; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptMod; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptMult; }
...
public:
    ...
    CppCompilerEnvironment& theCompilerEnvironment) const
  {
    ...
    sDefinition +=
      "(CppParsingTree_var pThisTree, const std::string& sFilename) {\n";
    sDefinition += "\tCGQuietOutput quiet;\n";
    ScpStream* pOwner;
    CW_BODY_STREAM.insertText(
      sDefinition, CW_BODY_STREAM.getFloatingLocation("INSERT AREA", pOwner));
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    std::string sURL = (*cursor)->getValue(visibility);
    ++cursor;
    DtaScriptVariable* pHTTPSession = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
    ++cursor;
    DtaScriptVariable* pArguments = visibility.getExistingVariable(
      *dynamic_cast<ExprScriptVariable*>(*cursor));
    std::string returnValue =
      CGRuntime::postHTTPRequest(sURL, pHTTPSession, pArguments);
    return returnValue;
  }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptPow; }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    int iLength = atoi(sLength.c_str());
    std::string returnValue =
      CGRuntime::receiveBinaryFromSocket(iSocket, iLength);
    return returnValue;
  }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    int iLength = atoi(sLength.c_str());
    std::string returnValue =
      CGRuntime::receiveTextFromSocket(iSocket, iLength);
    return returnValue;
  }
...
public:
  ...
  virtual std::string getValue(DtaScriptVariable& visibility) const
  {
    ...
    bool bSubfolders = !sSubfolders.empty();
    bool returnValue =
      CGRuntime::scanFiles(pFiles, sPath, sPattern, bSubfolders);
    return (returnValue ? "true" : "");
  }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptSqrt; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptSub; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptSup; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptSystem; }
...
public:
  ...
  {
    if (iIndex == 0)
      return SCRIPTFILE_TRANSLATE_EXPRTYPE;
    if (iIndex == 1)
      return NODE_EXPRTYPE;
    if (iIndex == 2)
      return VALUE_EXPRTYPE;
  ...
  virtual void initializationDone()
  {
    ...
    _pTranslateString->setPatternFileName(
      (ExprScriptScriptFile*)_parameters[0]);
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptTrim; }
...
public:
  ...
  static ExprScriptFunction* create(GrfBlock&) { return new ExprScriptUUID; }
...
ExprScriptFunction::ExprScriptFunction(GrfFunction* pPrototype)
...
void
ExprScriptFunction::clearParameters()
{
  for (std::vector<ExprScriptExpression*>::iterator i = _parameters.begin();
       i != _parameters.end();
       i++)
    delete (*i);
  _parameters = std::vector<ExprScriptExpression*>();
...
std::map<std::string, DtaFunctionInfo>&
ExprScriptFunction::getFunctionRegister()
{
  ...
  if (!_bInitialization) {
    ...
    //##markup##"registration"
    //##begin##"registration"
    ExprScriptFlushOutputToSocket::registerFunction(
      _functionRegister["flushOutputToSocket"]);
    ...
    ExprScriptAddGenerationTagsHandler::registerFunction(
      _functionRegister["addGenerationTagsHandler"]);
    ...
    ExprScriptChangeDirectory::registerFunction(
      _functionRegister["changeDirectory"]);
    ExprScriptChangeFileTime::registerFunction(
      _functionRegister["changeFileTime"]);
    ...
    ExprScriptCompleteLeftSpaces::registerFunction(
      _functionRegister["completeLeftSpaces"]);
    ExprScriptCompleteRightSpaces::registerFunction(
      _functionRegister["completeRightSpaces"]);
    ExprScriptComposeAdaLikeString::registerFunction(
      _functionRegister["composeAdaLikeString"]);
    ExprScriptComposeCLikeString::registerFunction(
      _functionRegister["composeCLikeString"]);
    ExprScriptComposeHTMLLikeString::registerFunction(
      _functionRegister["composeHTMLLikeString"]);
    ExprScriptComposeSQLLikeString::registerFunction(
      _functionRegister["composeSQLLikeString"]);
    ExprScriptComputeMD5::registerFunction(_functionRegister["computeMD5"]);
    ExprScriptCopySmartFile::registerFunction(
      _functionRegister["copySmartFile"]);
    ExprScriptCoreString::registerFunction(_functionRegister["coreString"]);
    ExprScriptCountStringOccurences::registerFunction(
      _functionRegister["countStringOccurences"]);
    ExprScriptCreateDirectory::registerFunction(
      _functionRegister["createDirectory"]);
    ExprScriptCreateINETClientSocket::registerFunction(
      _functionRegister["createINETClientSocket"]);
    ExprScriptCreateINETServerSocket::registerFunction(
      _functionRegister["createINETServerSocket"]);
    ExprScriptCreateIterator::registerFunction(
      _functionRegister["createIterator"]);
    ExprScriptCreateReverseIterator::registerFunction(
      _functionRegister["createReverseIterator"]);
    ExprScriptCreateVirtualFile::registerFunction(
      _functionRegister["createVirtualFile"]);
    ExprScriptCreateVirtualTemporaryFile::registerFunction(
      _functionRegister["createVirtualTemporaryFile"]);
    ...
    ExprScriptDeleteVirtualFile::registerFunction(
      _functionRegister["deleteVirtualFile"]);
    ExprScriptDiv::registerFunction(_functionRegister["div"]);
    ExprScriptDuplicateIterator::registerFunction(
      _functionRegister["duplicateIterator"]);
    ...
    ExprScriptEqualsIgnoreCase::registerFunction(
      _functionRegister["equalsIgnoreCase"]);
    ExprScriptEqualTrees::registerFunction(_functionRegister["equalTrees"]);
    ExprScriptExecuteStringQuiet::registerFunction(
      _functionRegister["executeStringQuiet"]);
    ExprScriptExistDirectory::registerFunction(
      _functionRegister["existDirectory"]);
    ...
    ExprScriptExistVirtualFile::registerFunction(
      _functionRegister["existVirtualFile"]);
    ExprScriptExistVariable::registerFunction(
      _functionRegister["existVariable"]);
    ExprScriptExp::registerFunction(_functionRegister["exp"]);
    ExprScriptExploreDirectory::registerFunction(
      _functionRegister["exploreDirectory"]);
    ExprScriptExtractGenerationHeader::registerFunction(
      _functionRegister["extractGenerationHeader"]);
    ExprScriptFileCreation::registerFunction(_functionRegister["fileCreation"]);
    ExprScriptFileLastAccess::registerFunction(
      _functionRegister["fileLastAccess"]);
    ExprScriptFileLastModification::registerFunction(
      _functionRegister["fileLastModification"]);
    ...
    ExprScriptFindFirstChar::registerFunction(
      _functionRegister["findFirstChar"]);
    ExprScriptFindFirstSubstringIntoKeys::registerFunction(
      _functionRegister["findFirstSubstringIntoKeys"]);
    ExprScriptFindLastString::registerFunction(
      _functionRegister["findLastString"]);
    ExprScriptFindNextString::registerFunction(
      _functionRegister["findNextString"]);
    ExprScriptFindNextSubstringIntoKeys::registerFunction(
      _functionRegister["findNextSubstringIntoKeys"]);
    ...
    ExprScriptGetCommentBegin::registerFunction(
      _functionRegister["getCommentBegin"]);
    ExprScriptGetCommentEnd::registerFunction(
      _functionRegister["getCommentEnd"]);
    ExprScriptGetCurrentDirectory::registerFunction(
      _functionRegister["getCurrentDirectory"]);
    ExprScriptGetEnv::registerFunction(_functionRegister["getEnv"]);
    ExprScriptGetGenerationHeader::registerFunction(
      _functionRegister["getGenerationHeader"]);
    ExprScriptGetHTTPRequest::registerFunction(
      _functionRegister["getHTTPRequest"]);
    ExprScriptGetIncludePath::registerFunction(
      _functionRegister["getIncludePath"]);
    ...
    ExprScriptGetShortFilename::registerFunction(
      _functionRegister["getShortFilename"]);
    ExprScriptGetTextMode::registerFunction(_functionRegister["getTextMode"]);
    ExprScriptGetVariableAttributes::registerFunction(
      _functionRegister["getVariableAttributes"]);
    ExprScriptGetVersion::registerFunction(_functionRegister["getVersion"]);
    ExprScriptGetWorkingPath::registerFunction(
      _functionRegister["getWorkingPath"]);
    ExprScriptGetWriteMode::registerFunction(_functionRegister["getWriteMode"]);
    ExprScriptHexaToDecimal::registerFunction(
      _functionRegister["hexaToDecimal"]);
    ExprScriptHostToNetworkLong::registerFunction(
      _functionRegister["hostToNetworkLong"]);
    ExprScriptHostToNetworkShort::registerFunction(
      _functionRegister["hostToNetworkShort"]);
    ...
    ExprScriptLoadBinaryFile::registerFunction(
      _functionRegister["loadBinaryFile"]);
    ExprScriptLoadFile::registerFunction(_functionRegister["loadFile"]);
    ExprScriptLoadVirtualFile::registerFunction(
      _functionRegister["loadVirtualFile"]);
    ...
    ExprScriptNetworkLongToHost::registerFunction(
      _functionRegister["networkLongToHost"]);
    ExprScriptNetworkShortToHost::registerFunction(
      _functionRegister["networkShortToHost"]);
    ExprScriptNext::registerFunction(_functionRegister["next"]);
    ExprScriptNot::registerFunction(_functionRegister["not"]);
    ExprScriptOctalToDecimal::registerFunction(
      _functionRegister["octalToDecimal"]);
    ExprScriptParseFreeQuiet::registerFunction(
      _functionRegister["parseFreeQuiet"]);
    ExprScriptPathFromPackage::registerFunction(
      _functionRegister["pathFromPackage"]);
    ExprScriptPostHTTPRequest::registerFunction(
      _functionRegister["postHTTPRequest"]);
    ExprScriptPow::registerFunction(_functionRegister["pow"]);
    ExprScriptPrec::registerFunction(_functionRegister["prec"]);
    ExprScriptRandomInteger::registerFunction(
      _functionRegister["randomInteger"]);
    ExprScriptReceiveBinaryFromSocket::registerFunction(
      _functionRegister["receiveBinaryFromSocket"]);
    ExprScriptReceiveFromSocket::registerFunction(
      _functionRegister["receiveFromSocket"]);
    ExprScriptReceiveTextFromSocket::registerFunction(
      _functionRegister["receiveTextFromSocket"]);
    ExprScriptRelativePath::registerFunction(_functionRegister["relativePath"]);
    ExprScriptRemoveDirectory::registerFunction(
      _functionRegister["removeDirectory"]);
    ExprScriptRemoveGenerationTagsHandler::registerFunction(
      _functionRegister["removeGenerationTagsHandler"]);
    ExprScriptRepeatString::registerFunction(_functionRegister["repeatString"]);
    ExprScriptReplaceString::registerFunction(
      _functionRegister["replaceString"]);
    ExprScriptReplaceTabulations::registerFunction(
      _functionRegister["replaceTabulations"]);
    ExprScriptResolveFilePath::registerFunction(
      _functionRegister["resolveFilePath"]);
    ...
    ExprScriptScanDirectories::registerFunction(
      _functionRegister["scanDirectories"]);
    ExprScriptScanFiles::registerFunction(_functionRegister["scanFiles"]);
    ExprScriptSendBinaryToSocket::registerFunction(
      _functionRegister["sendBinaryToSocket"]);
    ExprScriptSendHTTPRequest::registerFunction(
      _functionRegister["sendHTTPRequest"]);
    ExprScriptSendTextToSocket::registerFunction(
      _functionRegister["sendTextToSocket"]);
    ExprScriptSelectGenerationTagsHandler::registerFunction(
      _functionRegister["selectGenerationTagsHandler"]);
    ...
    ExprScriptToLowerString::registerFunction(
      _functionRegister["toLowerString"]);
    ExprScriptToUpperString::registerFunction(
      _functionRegister["toUpperString"]);
    ExprScriptTranslateString::registerFunction(
      _functionRegister["translateString"]);
    ...
    ExprScriptTruncateAfterString::registerFunction(
      _functionRegister["truncateAfterString"]);
    ExprScriptTruncateBeforeString::registerFunction(
      _functionRegister["truncateBeforeString"]);
    ExprScriptUUID::registerFunction(_functionRegister["UUID"]);
    ExprScriptCountInputCols::registerFunction(
      _functionRegister["countInputCols"]);
    ExprScriptCountInputLines::registerFunction(
      _functionRegister["countInputLines"]);
    ExprScriptGetInputFilename::registerFunction(
      _functionRegister["getInputFilename"]);
    ExprScriptGetLastReadChars::registerFunction(
      _functionRegister["getLastReadChars"]);
    ExprScriptGetInputLocation::registerFunction(
      _functionRegister["getInputLocation"]);
    ...
    ExprScriptReadAdaString::registerFunction(
      _functionRegister["readAdaString"]);
    ...
    ExprScriptReadCharAsInt::registerFunction(
      _functionRegister["readCharAsInt"]);
    ExprScriptReadChars::registerFunction(_functionRegister["readChars"]);
    ExprScriptReadIdentifier::registerFunction(
      _functionRegister["readIdentifier"]);
    ExprScriptReadIfEqualTo::registerFunction(
      _functionRegister["readIfEqualTo"]);
    ExprScriptReadIfEqualToIgnoreCase::registerFunction(
      _functionRegister["readIfEqualToIgnoreCase"]);
    ExprScriptReadIfEqualToIdentifier::registerFunction(
      _functionRegister["readIfEqualToIdentifier"]);
    ...
    ExprScriptReadPythonString::registerFunction(
      _functionRegister["readPythonString"]);
    ExprScriptReadString::registerFunction(_functionRegister["readString"]);
    ExprScriptReadUptoJustOneChar::registerFunction(
      _functionRegister["readUptoJustOneChar"]);
    ...
    ExprScriptSkipEmptyCppExceptDoxygen::registerFunction(
      _functionRegister["skipEmptyCppExceptDoxygen"]);
    ExprScriptSkipEmptyHTML::registerFunction(
      _functionRegister["skipEmptyHTML"]);
    ExprScriptSkipEmptyLaTeX::registerFunction(
      _functionRegister["skipEmptyLaTeX"]);
    ExprScriptCountOutputCols::registerFunction(
      _functionRegister["countOutputCols"]);
    ExprScriptCountOutputLines::registerFunction(
      _functionRegister["countOutputLines"]);
    ExprScriptDecrementIndentLevel::registerFunction(
      _functionRegister["decrementIndentLevel"]);
    ExprScriptEqualLastWrittenChars::registerFunction(
      _functionRegister["equalLastWrittenChars"]);
    ExprScriptExistFloatingLocation::registerFunction(
      _functionRegister["existFloatingLocation"]);
    ExprScriptGetFloatingLocation::registerFunction(
      _functionRegister["getFloatingLocation"]);
    ExprScriptGetLastWrittenChars::registerFunction(
      _functionRegister["getLastWrittenChars"]);
    ExprScriptGetMarkupKey::registerFunction(_functionRegister["getMarkupKey"]);
    ExprScriptGetMarkupValue::registerFunction(
      _functionRegister["getMarkupValue"]);
    ExprScriptGetOutputFilename::registerFunction(
      _functionRegister["getOutputFilename"]);
    ExprScriptGetOutputLocation::registerFunction(
      _functionRegister["getOutputLocation"]);
    ExprScriptGetProtectedArea::registerFunction(
      _functionRegister["getProtectedArea"]);
    ExprScriptGetProtectedAreaKeys::registerFunction(
      _functionRegister["getProtectedAreaKeys"]);
    ExprScriptIndentText::registerFunction(_functionRegister["indentText"]);
    ExprScriptNewFloatingLocation::registerFunction(
      _functionRegister["newFloatingLocation"]);
    ExprScriptRemainingProtectedAreas::registerFunction(
      _functionRegister["remainingProtectedAreas"]);
    ExprScriptRemoveFloatingLocation::registerFunction(
      _functionRegister["removeFloatingLocation"]);
    ExprScriptRemoveProtectedArea::registerFunction(
      _functionRegister["removeProtectedArea"]);
    //##end##"registration"
  }
...
void
ExprScriptFunction::addParameter(ExprScriptExpression* pParameter)
{
  _parameters.push_back(pParameter);
}

EXPRESSION_TYPE
ExprScriptFunction::getParameterType(unsigned int iIndex) const
{
  if (_pPrototype != NULL)
    return _pPrototype->getParameterType(iIndex);
  return UNKNOWN_EXPRTYPE;
}

ExprScriptExpression*
ExprScriptFunction::getDefaultParameter(unsigned int iIndex) const
{
  if (_pPrototype != NULL)
    return _pPrototype->getDefaultParameter(iIndex);
  return NULL;
}

unsigned int
ExprScriptFunction::getArity() const
{
  if (_pPrototype != NULL)
    return _pPrototype->getArity();
  throw UtlException("internal error in ExprScriptFunction::getArity(): this "
                     "case shouldn't occur");
}

unsigned int
ExprScriptFunction::getMinArity() const
{
  if (_pPrototype != NULL)
    return _pPrototype->getMinArity();
  return getArity();
}

const char*
ExprScriptFunction::getName() const
{
  if (_pPrototype != NULL)
    return _pPrototype->getFunctionName();
  throw UtlException("internal error in ExprScriptFunction::getName(): this "
                     "case shouldn't occur");
}

int
ExprScriptFunction::getThisPosition() const
{
  return 0;
}
void
ExprScriptFunction::initializationDone()
{}

std::string
ExprScriptFunction::getValue(DtaScriptVariable& visibility) const
{
  if (_pTemplate != NULL) {
    std::string sInstantiationKey = _pTemplate->getValue(visibility);
    GrfFunction* pFunction =
      _pPrototype->getInstantiatedFunction(visibility, sInstantiationKey);
    if (pFunction != NULL)
      return pFunction->launchExecution(visibility, *this, sInstantiationKey);
    throw UtlException("template function '" + _pPrototype->getName() + "<\"" +
                       sInstantiationKey + "\">' hasn't been implemented");
  }
  if (_pPrototype == NULL)
    throw UtlException(
      "ExprScriptFunction::getValue() -> must be implemented into derivated "
      "classes\nFor custom functions, see class GrfFunction");
  return _pPrototype->launchExecution(visibility, *this);
}

ExprScriptFunction*
ExprScriptFunction::create(GrfBlock& block,
                           ScpStream& script,
                           ...
                           bool bGenericKey)
{
  ...
  if (sFunction == "getVariableSize") {
    std::string sErrorMessage =
      "warning: function 'getVariableSize' has been deprecated since version "
      "1.30 -> replace it by function 'getArraySize'" +
      CGRuntime::endl();
    ...
    std::string sErrorMessage =
      "warning: function 'today' has been deprecated since version 2.09 -> "
      "replace it by function 'getNow'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'getDefineTarget' has been deprecated since version "
      "1.30 -> replace it by function 'getProperty'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'trimLeftString' has been deprecated since version "
      "1.40 -> replace it by function 'trimLeft'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'trimRightString' has been deprecated since version "
      "1.40 -> replace it by function 'trimRight'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'trimString' has been deprecated since version 1.40 "
      "-> replace it by function 'trim'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'readLastChars' has been deprecated since version "
      "1.30 -> replace it by function 'getLastReadChars'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'getLocation' has been deprecated since version 3.7.1 "
      "-> replace it by function 'getInputLocation'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
    ...
    std::string sErrorMessage =
      "warning: function 'getMarkerKey' has been deprecated since version 2.14 "
      "-> replace it by function 'getMarkupKey'" +
      CGRuntime::endl();
    sErrorMessage += DtaProject::getInstance().getTraceStack(script);
    if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
      CGRuntime::traceLine(sErrorMessage);
  ...
  if (getFunctionRegister().find(sFunctionName) !=
      getFunctionRegister().end()) {
    if (!sTemplate.empty())
      throw UtlException("predefined function '" + sFunction +
                         "' cannot instantiate template function '" +
                         sFunction + "<" + sTemplate + ">'");
    pFunction = getFunctionRegister()[sFunctionName].constructor(block);
  } else {
    GrfFunction* pPrototype =
      block.getFunction(sFunctionName, sTemplate, bGenericKey);
    if (pPrototype != NULL) {
...
ExprScriptFunction*
ExprScriptFunction::createMethod(GrfBlock& block,
                                 ScpStream& script,
                                 const std::string& sFunction,
                                 const std::string& sTemplate,
                                 bool bGenericKey)
{
  ...
  //##begin##"createMethod"
  if (sFunction == "size")
    return create(block, script, "getArraySize", sTemplate, bGenericKey);
  if (sFunction == "length")
    return create(block, script, "lengthString", sTemplate, bGenericKey);
  if (sFunction == "empty")
    return create(block, script, "isEmpty", sTemplate, bGenericKey);
  //##end##"createMethod"
...
{
  for (std::map<std::string, DtaFunctionInfo>::iterator i =
         getFunctionRegister().begin();
       i != getFunctionRegister().end();
       i++) {
    *(i->second.pCounter) = 0;
  }
}
...
ExprScriptFunction::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  bool bDispatchedYet = true;
  if (_pPrototype != NULL) {
    if (_pPrototype->isATemplateInstantiation()) {
      CW_BODY_STREAM << DtaScript::convertFilenameAsIdentifier(
                          CppCompilerEnvironment::getRadical(
                            theCompilerEnvironment.getFunctionModule(
                              _pPrototype->getFunctionName())))
                     << "::_compilerTemplateFunction_"
                     << _pPrototype->getFunctionName()
                     << "_compilerInstantiation_";
      ExprScriptConstant* pConstant =
        dynamic_cast<ExprScriptConstant*>(_pTemplate);
      DtaScriptVariable visibility;
      if ((pConstant != NULL) && !pConstant->getConstant().empty() &&
          (_pPrototype->getInstantiatedFunction(
             visibility, pConstant->getConstant()) != NULL)) {
        CW_BODY_STREAM << theCompilerEnvironment.convertTemplateKey(
                            pConstant->getConstant())
                       << "(";
      } else {
        CW_BODY_STREAM << "(";
    ...
    CW_BODY_STREAM << "CGRuntime::" << getName() << "(";
  int iIndex = 0;
  for (std::vector<ExprScriptExpression*>::const_iterator i =
         _parameters.begin();
       i != _parameters.end();
       i++) {
    if ((i != _parameters.begin()) || !bDispatchedYet)
      CW_BODY_STREAM << ", ";
    if ((getParameterType(iIndex) & 0x00FF) == VALUE_EXPRTYPE)
      (*i)->compileCpp(theCompilerEnvironment);
    else
      dynamic_cast<ExprScriptVariable*>(*i)->compileCppForSet(
        theCompilerEnvironment);
    iIndex++;
  }
  CW_BODY_STREAM << ")";
...
bool
ExprScriptFunction::compileCppBoolean(
...
bool
ExprScriptFunction::compileCppString(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ScpStream* pOwner;
  int iInsertAreaLocation =
  ...
  if (returnType != STRING_TYPE) {
    CW_BODY_STREAM.insertText(
      "CGRuntime::toString(",
      iLocation + (CW_BODY_STREAM.getFloatingLocation("INSERT AREA", pOwner) -
                   iInsertAreaLocation));
    CW_BODY_STREAM << ")";
  }
  return (_pPrototype != NULL);
...
std::string
ExprScriptFunction::toString() const
{
  std::string sSerialize;
  if (_pPrototype != NULL)
    sSerialize = _pPrototype->getFunctionName();
  else
    sSerialize = getName();
  if (_pTemplate != NULL)
    sSerialize += "<" + _pTemplate->toString() + ">";
  sSerialize += "(";
  int iIndex = 0;
  for (std::vector<ExprScriptExpression*>::const_iterator i =
         _parameters.begin();
