...
bool BNFClauseCall::isABNFCommand() const
{
    return true;
}
...
BNFClause& BNFClauseCall::getInstantiatedClause(DtaScriptVariable& visibility, std::string& sInstantiationKey) const
{
    ...
    if (_pTemplateExpression != NULL) {
        ...
        if (pClause == NULL) {
            throw UtlException("template clause '" +  _pClause->getName() + "<\"" + sInstantiationKey + "\">' hasn't been implemented");
        }
    } else {
        pClause = _pClause;
    }
...
SEQUENCE_INTERRUPTION_LIST BNFClauseCall::executeInternal(DtaScriptVariable& visibility)
{
    ...
    if (!sInstantiationKey.empty()) {
        if (myClause.isGenericKey()) {
            sClauseName += "<" + myClause.getTemplateKey() + "=\"" + sInstantiationKey + "\">";
        } else {
    ...
    if (result == NO_INTERRUPTION) {
        ...
        if (!_listOfConstants.empty()) {
            ...
            if (myClause.getReturnType() == BNFClause::VALUE_RETURN_TYPE) {
                ...
                sTextToMatch = CGRuntime::getLastReadChars(CGRuntime::getInputLocation() - iNotEmptyLocation);
            }
...
void BNFClauseCall::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    for (std::vector<ExprScriptExpression*>::const_iterator i = _listOfParameters.begin(); i != _listOfParameters.end(); i++) {
        ...
        if ((exprType == VALUE_EXPRTYPE) || (dynamic_cast<ExprScriptAlien*>(*i) != NULL)) {
            (*i)->compileCpp(theCompilerEnvironment);
        } else {
            dynamic_cast<ExprScriptVariable*>(*i)->compileCppForGetOrCreateLocal(theCompilerEnvironment);
        }
...
void BNFRootClauseCall::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (getBNFScript().getIgnoreClause() != NULL) {
        ...
        if (sIgnoreClauseName[0] == '#') {
            sIgnoreClauseName = "_compilerDirectiveClause_" + sIgnoreClauseName.substr(1);
        }
    ...
    if (getClause().isATemplateInstantiation()) {
        sClauseName = "_compilerTemplateClause_" + sClauseName + "_compilerInstantiation_" + theCompilerEnvironment.convertTemplateKey(getClause().getTemplateKey());
    } else if (sClauseName[0] == '#') {
        sClauseName = "_compilerDirectiveClause_" + sClauseName.substr(1);
    } else {
        sClauseName = "_compilerClause_" + sClauseName;
    ...
    CW_BODY_INDENT << "CGBNFRuntimeEnvironment theEnvironment(" << sIgnoreClauseName << ", " << getBNFScript().getIgnoreMode() << ", " << ((getBNFScript().implicitCopy()) ? "true" : "false") << ");";
    CW_BODY_ENDL;
    ...
    if (getBNFScript().hasCoverageRecording()) {
        ...
        for (std::map<std::string, std::list<BNFClause*> >::const_iterator i = allClauses.begin(); i != allClauses.end(); ++i) {
            for (std::list<BNFClause*>::const_iterator j = i->second.begin(); j != i->second.end(); ++j) {
                CW_BODY_INDENT << "theEnvironment.addClauseSignature(\"" << CGRuntime::composeCLikeString((*j)->getSignature()) << "\");";
                CW_BODY_ENDL;
            }
        }
    }
