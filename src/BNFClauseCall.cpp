...
public:
  ...
  inline ~GrfClauseDepth() { GrfFunction::_iCurrentStackDepth--; }
};

BNFClauseCall::BNFClauseCall(DtaBNFScript* pBNFScript,
                             GrfBlock* pParent,
                             bool bContinue,
                             bool bNoCase)
...
{
  for (std::vector<ExprScriptExpression*>::iterator i =
         _listOfParameters.begin();
       i != _listOfParameters.end();
       i++) {
    delete *i;
  }
...
void
BNFClauseCall::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFClauseCall(*this, env);
}

bool
BNFClauseCall::isABNFCommand() const
{
  return true;
}

void
BNFClauseCall::addASTCommand(ASTCommand* pCommand)
{
...
void
BNFClauseCall::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                   bool bConcat,
                                   BNFClause& theClause)
{
...
void
BNFClauseCall::callBeforeExecutionCBK(DtaScriptVariable& visibility)
{
  callRecursiveBeforeExecutionCBK(getCurrentExecutionContext(), visibility);
}

void
BNFClauseCall::callRecursiveBeforeExecutionCBK(GrfExecutionContext* pContext,
...
void
BNFClauseCall::callAfterExecutionCBK(DtaScriptVariable& visibility)
{
...
BNFClause&
BNFClauseCall::getInstantiatedClause(DtaScriptVariable& visibility,
                                     std::string& sInstantiationKey) const
{
  ...
  if (_pTemplateExpression != NULL) {
    ...
    if (pClause == NULL)
      throw UtlException("template clause '" + _pClause->getName() + "<\"" +
...
BNFClauseCall::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if (!sInstantiationKey.empty()) {
    if (myClause.isGenericKey())
      sClauseName +=
        "<" + myClause.getTemplateKey() + "=\"" + sInstantiationKey + "\">";
    else
      sClauseName += "<\"" + sInstantiationKey + "\">";
  }
  if (_pBNFScript->trace()) {
    char tcMessage[80];
    sprintf(tcMessage, "' [%d]", CGRuntime::getInputLocation());
    CGRuntime::traceLine(_pBNFScript->traceIndentation() + "IN '" +
                         sClauseName + tcMessage);
    _pBNFScript->incrementTraceIndentation();
    if (!CGRuntime::inputKey(false).empty()) {
      char tcMessage[512];
      std::string sFilename = CGRuntime::getInputFilename();
      sprintf(tcMessage,
              "%s(%d): trace mode suspended",
              sFilename.c_str(),
  ...
  UTLTRACE_STACK_FUNCTION(
    _sParsingFilePtr, sClauseName.c_str(), _iFileLocation);
  GrfClauseDepth newClauseCall;
  if (myClause.getNbCommands() == 0) {
    throw UtlException("clause '" + myClause.getSignature() +
                       "' has never been implemented");
  }
  std::string sName = "##stack## clause '";
  sName += sClauseName + "'";
  DtaScriptVariable stackClause(&visibility, sName.c_str());
  DtaScriptVariable* pReturnValue =
    stackClause.getNonRecursiveVariable(myClause.getName().c_str());
  if (!sInstantiationKey.empty() && myClause.isGenericKey()) {
    stackClause.setValueAtVariable(myClause.getTemplateKey().c_str(),
                                   sInstantiationKey.c_str());
  }
  int iLocation = CGRuntime::getInputLocation();
  int iNotEmptyLocation =
    0; // used only if variable to assign or constant to check
  ...
  if (myClause.isPropagatedParameter()) {
    if (myClause.getParameters().size() < _listOfParameters.size()) {
      ...
      sprintf(tcExpectedNumber,
              "' requires less than %d parameters (%d parameters, propagated)",
              _listOfParameters.size(),
              myClause.getParameters().size());
      throw UtlException("call to non-terminal '" + myClause.getSignature() +
                         tcExpectedNumber);
    }
  } else {
    if (myClause.getParameters().size() != _listOfParameters.size()) {
  ...
  std::vector<EXPRESSION_TYPE>::const_iterator k =
    myClause.getParameterTypes().begin();
      ...
      case NODE_EXPRTYPE: {
        ...
        if (pVarExpr == NULL) {
          ...
          if (pResolvedVariable == NULL) {
            ...
            throw UtlException(
              "in clause '" + myClause.getSignature() + "', parameter '" + *j +
        ...
        DtaScriptVariable* pParameter =
          stackClause.createNodeArgument(j->c_str(), pVariable);
      } break;
    }
    j++;
    k++;
  ...
  if (_pBNFScript->getStepintoHook() != NULL) {
    _pBNFScript->getStepintoHook()->executeHook(
      CGRuntime::getThisInternalNode(), sClauseName, stackClause);
  }
  SEQUENCE_INTERRUPTION_LIST result =
    myClause.executeClause(stackClause, _iSuperCallDepth);
  if (_pBNFScript->getStepoutHook() != NULL) {
    _pBNFScript->getStepoutHook()->executeHook(CGRuntime::getThisInternalNode(),
                                               sClauseName,
                                               stackClause,
  ...
  if (result == NO_INTERRUPTION) {
    ...
    for (std::vector<ExprScriptExpression*>::const_iterator i =
      ...
      if ((*k & 0x00FF) == REFERENCE_EXPRTYPE) {
        DtaScriptVariable* pVariable =
          visibility.getVariableForReferenceAssignment(
            (const ExprScriptVariable&)(*(*i)));
        DtaScriptVariable* pReference =
          stackClause.getNodeArgument(j->c_str())->getReferencedVariable();
    ...
    if (myClause.isPropagatedParameter()) {
      while (j != myClause.getParameters().end()) {
        if ((*k & 0x00FF) == REFERENCE_EXPRTYPE) {
          DtaScriptVariable* pVariable =
            visibility.getVariable(j->c_str(), true, true, false);
          DtaScriptVariable* pReference =
            stackClause.getNodeArgument(j->c_str())->getReferencedVariable();
          pVariable->setValue(pReference);
        }
    ...
    if (!_listOfConstants.empty()) {
      ...
      if (myClause.getReturnType() == BNFClause::VALUE_RETURN_TYPE) {
        ...
        if (tcReturnValue != NULL)
          sTextToMatch = tcReturnValue;
      } else
        sTextToMatch = CGRuntime::getLastReadChars(
          CGRuntime::getInputLocation() - iNotEmptyLocation);
      for (std::vector<std::string>::const_iterator i =
             _listOfConstants.begin();
           i != _listOfConstants.end();
           i++) {
        if (_bNoCase)
          bMatch = (stricmp(i->c_str(), sTextToMatch.c_str()) == 0);
        else
    ...
    if ((result != BREAK_INTERRUPTION) && (_pVariableToAssign != NULL)) {
      DtaScriptVariable* pVariable =
        visibility.getVariable(*_pVariableToAssign);
      if (myClause.getReturnType() == BNFClause::LIST_RETURN_TYPE) {
        ...
        std::string sValue;
        if (tcReturnValue != NULL)
          sValue = tcReturnValue;
        if (_iClauseReturnType == BNFClause::LIST_RETURN_TYPE) {
          pVariable->pushItem(sValue);
        } else {
          if (_bConcatVariable)
            pVariable->concatValue(sValue.c_str());
          else
            pVariable->setValue(sValue.c_str());
        }
      } else {
        std::string sText = CGRuntime::getLastReadChars(
          CGRuntime::getInputLocation() - iNotEmptyLocation);
        if (_iClauseReturnType == BNFClause::LIST_RETURN_TYPE) {
          pVariable->pushItem(sText);
        } else {
          if (_bConcatVariable)
            pVariable->concatValue(sText.c_str());
          else
            pVariable->setValue(sText.c_str());
        }
      }
    }
  } else {
...
void
BNFClauseCall::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "int " << tcLocation << " = CGRuntime::getInputLocation();";
  CW_BODY_ENDL;
  if (myClause.getPreprocessingIgnoreMode() == UNDEFINED_IGNORE) {
    CW_BODY_INDENT << "int " << tcImplicitCopy
                   << " = theEnvironment.skipEmptyChars();";
    CW_BODY_ENDL;
  } else {
    CW_BODY_INDENT << "int " << tcImplicitCopy << " = -1;";
    CW_BODY_ENDL;
  }
  if ((_pVariableToAssign != NULL) || !_listOfConstants.empty()) {
    CW_BODY_INDENT << "int _compilerClauseNotEmptyLocation_" << iCursor
  ...
  if (_pTemplateExpression != NULL) {
    ...
    sClauseName =
      "_compilerTemplateClause_" + sClauseName + "_compilerInstantiation_";
    ExprScriptConstant* pConstant =
      dynamic_cast<ExprScriptConstant*>(_pTemplateExpression);
    if (pConstant != NULL) {
      BNFClause* pTemplateClause =
        myClause.getInstantiatedClause(pConstant->getConstant());
      if ((pTemplateClause != 0) &&
          (pTemplateClause->getTemplateKey() == pConstant->getConstant())) {
        // it exists a clause instantiation for the given template key
        sClauseName +=
          theCompilerEnvironment.convertTemplateKey(pConstant->getConstant());
      } else {
  ...
  bool bReturnedValue =
    (!_listOfConstants.empty() &&
     (myClause.getReturnType() == BNFClause::VALUE_RETURN_TYPE)) ||
    ((_pVariableToAssign != NULL) &&
     (myClause.getReturnType() != BNFClause::NO_RETURN_TYPE));
  if (bReturnedValue) {
  ...
  CW_BODY_INDENT << "_compilerClauseSuccess = "
                 << DtaScript::convertFilenameAsIdentifier(
                      CppCompilerEnvironment::getRadical(
                        theCompilerEnvironment.getClauseModule(
                          myClause.getName())))
                 << "::" << sClauseName << "(";
  ...
  for (std::vector<ExprScriptExpression*>::const_iterator i =
    ...
    if ((exprType == VALUE_EXPRTYPE) ||
        (dynamic_cast<ExprScriptAlien*>(*i) != NULL))
      (*i)->compileCpp(theCompilerEnvironment);
    else
      dynamic_cast<ExprScriptVariable*>(*i)->compileCppForGetOrCreateLocal(
  ...
  if ((_pVariableToAssign != NULL) || !_listOfConstants.empty()) {
    ...
    if (!_listOfConstants.empty()) {
      ...
      if (myClause.getReturnType() == BNFClause::VALUE_RETURN_TYPE) {
        ...
        CW_BODY_STREAM
          << "CGRuntime::getLastReadChars(CGRuntime::getInputLocation() - "
             "_compilerClauseNotEmptyLocation_"
          << iCursor << ");";
      }
      CW_BODY_ENDL;
      CW_BODY_INDENT << "_compilerClauseSuccess = ";
      for (std::vector<std::string>::const_iterator i =
           ...
           i != _listOfConstants.end();
           i++) {
        if (i != _listOfConstants.begin())
          CW_BODY_STREAM << " || ";
        CW_BODY_STREAM << "(";
        if (_bNoCase) {
          CW_BODY_STREAM << "stricmp(_compilerClauseText_" << iCursor
                         << ".c_str(), ";
    ...
    if (_pVariableToAssign != NULL) {
      ...
      if (myClause.getReturnType() == BNFClause::NODE_RETURN_TYPE) {
        ...
        CW_BODY_STREAM << ".setAll(_compilerClauseReturnedValue);";
        CW_BODY_ENDL;
      } else {
        if (_listOfConstants.empty()) {
          CW_BODY_INDENT << "std::string _compilerClauseText_" << iCursor
        ...
        _pBNFScript->compileCppBNFAssignment(theCompilerEnvironment,
                                             _iClauseReturnType,
                                             *_pVariableToAssign,
                                             _bConcatVariable,
                                             tcText);
...
std::string
BNFClauseCall::toString() const
{
  std::string sString;
  if (_bContinue)
    sString = "#continue ";
  sString += getClause().getName();
  if (!getClause().getTemplateKey().empty())
    sString += "<\"" + getClause().getTemplateKey() + "\">";
  if (!_listOfParameters.empty()) {
    if (_pTemplateExpression != NULL)
      sString += "<" + _pTemplateExpression->toString() + ">";
    sString += "(";
    for (std::vector<ExprScriptExpression*>::const_iterator i =
           _listOfParameters.begin();
         i != _listOfParameters.end();
         i++) {
      if (i != _listOfParameters.begin())
        sString += ", ";
      sString += (*i)->toString();
    }
    sString += ")";
  }
  return sString + DtaBNFScript::constantsToString(_listOfConstants) +
         DtaBNFScript::assignmentToString(_pVariableToAssign, _bConcatVariable);
}

BNFRootClauseCall::BNFRootClauseCall(DtaBNFScript* pBNFScript,
                                     GrfBlock* pParent,
                                     bool bNoCase)
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (getBNFScript().getIgnoreClause() != NULL) {
    ...
    if (sIgnoreClauseName[0] == '#')
      sIgnoreClauseName =
        "_compilerDirectiveClause_" + sIgnoreClauseName.substr(1);
    sIgnoreClauseName =
      DtaScript::convertFilenameAsIdentifier(CppCompilerEnvironment::getRadical(
        theCompilerEnvironment.getClauseModule(
          getBNFScript().getIgnoreClause()->getName()))) +
      "::" + sIgnoreClauseName;
    ...
    if (eMode != NOT_IGNORE) {
      if (eMode == IGNORE_CLAUSE)
        sIgnoreClauseName =
          "_compilerDirectiveClause_" + sIgnoreClauseName.substr(1);
      sIgnoreClauseName = DtaScript::convertFilenameAsIdentifier(
                            CppCompilerEnvironment::getRadical(
                              theCompilerEnvironment.getClauseModule(
                                getBNFScript().getIgnoreClause()->getName()))) +
                          "::" + sIgnoreClauseName;
  ...
  if (sIgnoreClauseName.empty()) {
    ...
    sIgnoreClauseName =
      "new CodeWorker::DEFAULT_EXECUTE_CLAUSE(" + sIgnoreClauseName + ")";
  }
  std::string sClauseName = getClause().getName();
  if (getClause().isATemplateInstantiation())
    sClauseName =
      "_compilerTemplateClause_" + sClauseName + "_compilerInstantiation_" +
      theCompilerEnvironment.convertTemplateKey(getClause().getTemplateKey());
  else if (sClauseName[0] == '#')
    sClauseName = "_compilerDirectiveClause_" + sClauseName.substr(1);
  else
    sClauseName = "_compilerClause_" + sClauseName;
  CW_BODY_INDENT << "CGBNFRuntimeEnvironment theEnvironment("
                 << sIgnoreClauseName << ", " << getBNFScript().getIgnoreMode()
                 << ", " << ((getBNFScript().implicitCopy()) ? "true" : "false")
                 << ");";
  CW_BODY_ENDL;
  if (getBNFScript().hasCoverageRecording()) {
    CW_BODY_INDENT << "theEnvironment.activateMatchingAreas();";
    CW_BODY_ENDL;
    const std::map<std::string, std::list<BNFClause*>>& allClauses =
      getBNFScript().getClauses();
    for (std::map<std::string, std::list<BNFClause*>>::const_iterator i =
      ...
      for (std::list<BNFClause*>::const_iterator j = i->second.begin();
           j != i->second.end();
           ++j) {
        CW_BODY_INDENT << "theEnvironment.addClauseSignature(\""
                       << CGRuntime::composeCLikeString((*j)->getSignature())
                       << "\");";
        CW_BODY_ENDL;
      }
    }
  }
  CW_BODY_INDENT << DtaScript::convertFilenameAsIdentifier(
                      CppCompilerEnvironment::getRadical(
                        theCompilerEnvironment.getClauseModule(
                          getClause().getName())))
                 << "::" << sClauseName << "(";
  ...
  if (getBNFScript().hasCoverageRecording()) {
    ...
    getBNFScript().getMatchingAreasContainer()->compileCppForSet(
      theCompilerEnvironment);
    CW_BODY_STREAM << ");";
    CW_BODY_ENDL;
  }
