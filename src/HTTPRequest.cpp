        ...
        if (pCookies != NULL) {
            ...
            if (cookies != NULL) {
                for (std::list<DtaScriptVariable*>::const_iterator i = cookies->begin(); i != cookies->end(); i++) {
                    ...
                    if (pName == NULL) {
                        throw UtlException("attribute 'name' of a cookie expected into the HTTP session");
                    }

                    const char* tcName = pName->getValue();

                    if (tcName == NULL) {
                        throw UtlException("attribute 'name' of a cookie should be populated into the HTTP session");
                    }
...
HTTPRequest::HTTPRequest()
{
    ...
    if (curl_escape == NULL) {
        ...
        if (curl_escape == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_escape' in dynamic library \"" + sLibrary + "\"");
        }

        curl_free = (CURL_FREE) DynPackage::findFunction(hHandle, "curl_free");

        if (curl_free == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_free' in dynamic library \"" + sLibrary + "\"");
        }

        curl_easy_init = (CURL_EASY_INIT) DynPackage::findFunction(hHandle, "curl_easy_init");

        if (curl_easy_init == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_easy_init' in dynamic library \"" + sLibrary + "\"");
        }

        curl_easy_setopt = (CURL_EASY_SETOPT) DynPackage::findFunction(hHandle, "curl_easy_setopt");

        if (curl_easy_setopt == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_easy_setopt' in dynamic library \"" + sLibrary + "\"");
        }

        curl_easy_perform = (CURL_EASY_PERFORM) DynPackage::findFunction(hHandle, "curl_easy_perform");

        if (curl_easy_perform == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_easy_perform' in dynamic library \"" + sLibrary + "\"");
        }

        curl_easy_cleanup = (CURL_EASY_CLEANUP) DynPackage::findFunction(hHandle, "curl_easy_cleanup");

        if (curl_easy_cleanup == NULL) {
            throw UtlException("libcURL error: unable to find function 'curl_easy_cleanup' in dynamic library \"" + sLibrary + "\"");
        }
    }
