...
struct CurlOutputDataSection
{
  ...
  CurlOutputDataSection(const char* tcSection, int iLength)
    : _iLength(iLength)
    , next(NULL)
  {
...
struct CurlOutputData
{
  ...
  CurlOutputData(const std::string& sURL)
    : bText(false)
    , first(NULL)
    , last(NULL)
  {
    ...
    if (iIndex != std::string::npos) {
      ...
      bText =
        (sExtension == ".html" || sExtension == ".htm" || sExtension == ".txt");
    }
  }
      ...
      for (int i = 0; i < pCurrent->_iLength; ++i) {
        ...
        if ((a == 255) || (a <= 1)) {
          if ((a == 0) && bText) {
            ...
            if (iSize > 0) {
              memmove(
                pCurrent->_tcSection + i, pCurrent->_tcSection + i + 1, iSize);
            }
            pCurrent->_iLength--;
            --i;
            continue;
...
size_t
writeHeaderLineFunction(void* ptr,
                        size_t iSize,
                        size_t iNmemb,
                        void* pCurlOutput)
{
...
size_t
writeCurlFunction(void* ptr, size_t iSize, size_t iNmemb, void* pCurlOutput)
{
  int iLength = iSize * iNmemb;
  CurlOutputData* pData = (CurlOutputData*)pCurlOutput;
  CurlOutputDataSection* pSection =
    new CurlOutputDataSection((const char*)ptr, iLength);
  if (pData->last == NULL)
    pData->first = pSection;
  else
    pData->last->next = pSection;
  pData->last = pSection;
  return iLength;
}

std::string
getCurlErrorCode(CURLcode aCode)
{
  switch (aCode) {
    case CURLE_OK:
      return "";
    ...
    case CURLE_URL_MALFORMAT:
      return "URL malformated";
    case CURLE_URL_MALFORMAT_USER:
      return "URL malformated - user";
    ...
    case CURLE_FTP_COULDNT_SET_BINARY:
      return "FTP can't set binary";
    case CURLE_PARTIAL_FILE:
      return "FTP partial file";
    ...
    case CURLE_BAD_PASSWORD_ENTERED:
      return "Bad password entered";
    case CURLE_TOO_MANY_REDIRECTS:
      return "Catch endless re-direct loops";
    case CURLE_UNKNOWN_TELNET_OPTION:
      return "Unknown TELNET option";
    case CURLE_TELNET_OPTION_SYNTAX:
      return "Malformed TELNET option";
    case CURLE_OBSOLETE:
      return "Obsolete cURL error (50), removed after version 7.7.3";
    case CURLE_SSL_PEER_CERTIFICATE:
      return "SSL peer's certificate wasn't ok";
    ...
    case CURLE_SHARE_IN_USE:
      return "Share is in use";
    case CURLE_SSL_CERTPROBLEM:
      return "Problem with the local certificate";
    case CURLE_SSL_CIPHER:
      return "Couldn't use specified cipher";
    case CURLE_SSL_CACERT:
      return "Problem with the CA cert (path?)";
    case CURLE_BAD_CONTENT_ENCODING:
      return "Unrecognized transfer encoding";
  }
  char tcMessage[80];
  sprintf(tcMessage,
          "internal error: unrecognized 'libcURL' error code (%d)",
          (int)aCode);
  throw UtlException(tcMessage);
}

CURL*
fillCurlSession(DtaScriptVariable* pSession, CurlOutputData& theCurlOutput)
{
  CURL* pCURL = curl_easy_init();
  if (pCURL != NULL) {
    DtaScriptVariable* pAgent =
      (pSession != NULL) ? pSession->getNode("agent") : NULL;
    const char* tcAgent = ((pAgent != NULL) ? pAgent->getValue() : NULL);
    curl_easy_setopt(
      pCURL, CURLOPT_USERAGENT, ((tcAgent != NULL) ? tcAgent : "CodeWorker"));
    DtaScriptVariable* pReferer =
      (pSession != NULL) ? pSession->getNode("referer") : NULL;
    if (pReferer != NULL) {
      const char* tcReferer = pReferer->getValue();
      curl_easy_setopt(
        pCURL, CURLOPT_REFERER, ((tcReferer == NULL) ? "" : tcReferer));
    }
    DtaScriptVariable* pProxy =
      (pSession != NULL) ? pSession->getNode("proxy") : NULL;
    if (pProxy != NULL) {
      DtaScriptVariable* pHost = pProxy->getNode("host");
      DtaScriptVariable* pPort = pProxy->getNode("port");
      if ((pHost == NULL) || (pPort == NULL)) {
        throw UtlException(
          "'proxy.host' and 'proxy.port' expected into the HTTP session");
      }
      const char* tcHost = pHost->getValue();
      if (tcHost == NULL) {
        throw UtlException(
          "non empty 'proxy.host' expected into the HTTP session");
      }
      ...
      if (pUser != NULL) {
        ...
        if (strchr(tcUserPwd, ':') == NULL) {
          throw UtlException("'proxy.userpwd' must write the user and the "
                             "password like 'user:pwd'");
        }
        curl_easy_setopt(pCURL, CURLOPT_PROXYUSERPWD, tcUserPwd);
      }
    }
    DtaScriptVariable* pCookies =
      (pSession != NULL) ? pSession->getNode("cookies") : NULL;
    if (pCookies != NULL) {
      std::string sCookies;
      const std::list<DtaScriptVariable*>* cookies = pCookies->getArray();
      if (cookies != NULL) {
        for (std::list<DtaScriptVariable*>::const_iterator i = cookies->begin();
             i != cookies->end();
             i++) {
          DtaScriptVariable* pName = (*i)->getNode("name");
          if (pName == NULL)
            throw UtlException(
              "attribute 'name' of a cookie expected into the HTTP session");
          const char* tcName = pName->getValue();
          if (tcName == NULL)
            throw UtlException("attribute 'name' of a cookie should be "
                               "populated into the HTTP session");
          std::string sValue;
...
void
populateCookie(DtaScriptVariable* pSession, const std::string& sCookie)
{
  ...
  DtaScriptVariable* pCookie =
    pSession->insertNode("cookies")->getArrayElement(sName);
  if (pCookie == NULL) {
...
std::string
handleCurlOutput(DtaScriptVariable* pSession, CurlOutputData& theCurlOutput)
{
  ...
  if (!theCurlOutput.headers.empty()) {
    ...
    pHeaders->clearArray();
    for (std::map<std::string, std::list<std::string>>::const_iterator i =
           theCurlOutput.headers.begin();
         i != theCurlOutput.headers.end();
         ++i) {
      DtaScriptVariable* pLine = pHeaders->addElement(i->first);
      bool bCookie = (i->first == "Set-Cookie");
      for (std::list<std::string>::const_iterator j = i->second.begin();
           j != i->second.end();
           ++j) {
        pLine->pushItem(*j);
        if (bCookie)
          populateCookie(pSession, *j);
      }
    }
  } else {
...
HTTPRequest::HTTPRequest()
{
  ...
  if (curl_escape == NULL) {
    ...
    if (hHandle == NULL) {
      throw UtlException(
        "libcURL error: unable to locate the dynamic library \"" + sLibrary +
        "\"");
    }
#else
    ...
    if (hHandle == NULL) {
      ...
      if (hHandle == NULL) {
        ...
        if (hHandle == NULL) {
          throw UtlException(
            "libcURL error: unable to locate the dynamic library \"" +
            sLibrary + "\"");
        }
      }
    }
    ...
    if (curl_escape == NULL)
      throw UtlException("libcURL error: unable to find function 'curl_escape' "
                         "in dynamic library \"" +
                         sLibrary + "\"");
    curl_free = (CURL_FREE)DynPackage::findFunction(hHandle, "curl_free");
    if (curl_free == NULL)
      throw UtlException("libcURL error: unable to find function 'curl_free' "
                         "in dynamic library \"" +
                         sLibrary + "\"");
    curl_easy_init =
      (CURL_EASY_INIT)DynPackage::findFunction(hHandle, "curl_easy_init");
    if (curl_easy_init == NULL)
      throw UtlException("libcURL error: unable to find function "
                         "'curl_easy_init' in dynamic library \"" +
                         sLibrary + "\"");
    curl_easy_setopt =
      (CURL_EASY_SETOPT)DynPackage::findFunction(hHandle, "curl_easy_setopt");
    if (curl_easy_setopt == NULL)
      throw UtlException("libcURL error: unable to find function "
                         "'curl_easy_setopt' in dynamic library \"" +
                         sLibrary + "\"");
    curl_easy_perform =
      (CURL_EASY_PERFORM)DynPackage::findFunction(hHandle, "curl_easy_perform");
    if (curl_easy_perform == NULL)
      throw UtlException("libcURL error: unable to find function "
                         "'curl_easy_perform' in dynamic library \"" +
                         sLibrary + "\"");
    curl_easy_cleanup =
      (CURL_EASY_CLEANUP)DynPackage::findFunction(hHandle, "curl_easy_cleanup");
    if (curl_easy_cleanup == NULL)
      throw UtlException("libcURL error: unable to find function "
                         "'curl_easy_cleanup' in dynamic library \"" +
                         sLibrary + "\"");
  }
#endif
}
...
std::string
HTTPRequest::get(const std::string& sURL,
                 DtaScriptVariable* pSession,
                 DtaScriptVariable* pArguments)
{
  ...
  if (pCURL != NULL) {
      ...
      if (arguments != NULL) {
        for (std::list<DtaScriptVariable*>::const_iterator i =
          ...
          if (i == arguments->begin())
            ...
            sGetURL += "&";
          char* tcName =
            curl_escape((*i)->getName(), ::strlen((*i)->getName()));
          sGetURL += tcName;
          sGetURL += "=";
    ...
    if (!sErrorCode.empty())
      throw UtlException("libcURL error: " + sErrorCode);
  }
  return handleCurlOutput(pSession, theCurlOutput);
#else
  throw UtlException(
    "HTTPRequest::get(" + sURL +
    ") cannot work, because 'CodeWorker' doesn't link libCURL!");
#endif
}

std::string
HTTPRequest::post(const std::string& sURL,
                  DtaScriptVariable* pSession,
                  DtaScriptVariable* pArguments)
{
#ifdef CODEWORKER_CURL_LIB
...
#else
  throw UtlException(
    "HTTPRequest::post(" + sURL +
    ") cannot work, because 'CodeWorker' doesn't link libCURL!");
#endif
}

std::string
HTTPRequest::send(const std::string& sURL, DtaScriptVariable* pSession)
{
#ifdef CODEWORKER_CURL_LIB
  CurlOutputData theCurlOutput(sURL);
  CURL* pCURL = fillCurlSession(pSession, theCurlOutput);
...
HTTPRequest::UUID()
{
  ...
  if (aCreateStatus != RPC_S_OK)
    throw UtlException("Cannot create a unique Windows UUID");
  unsigned char* aStringUuid;
  RPC_STATUS aConversionStatus = ::UuidToString(&aUuid, &aStringUuid);
  if (aConversionStatus != RPC_S_OK)
    throw UtlException("Cannot convert a unique Windows UUID to a string");
  std::string ssUuid((char*)aStringUuid);
  RPC_STATUS aDeallocationStatus = ::RpcStringFree(&aStringUuid);
  if (aDeallocationStatus != RPC_S_OK)
    throw UtlException("Cannot deallocate UUID memory");
  return ssUuid;
#else
  throw UtlException(
    "UUID() not implemented yet. Use 'uuid.h' and 'libuuid.so' under Linux");
#endif
}
}
