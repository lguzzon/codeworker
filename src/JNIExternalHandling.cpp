...
JNIExternalHandling::JNIExternalHandling(JNIEnv& theEnv, jobject pJNIObject)
  : _theEnv(theEnv)
  , _pJNIObject(pJNIObject)
{
#ifdef CODEWORKER_JNI
  _pClass = _theEnv.GetObjectClass(_pJNIObject);
  _pTraceLineMethod =
    _theEnv.GetMethodID(_pClass, "traceLine", "(Ljava/lang/String;)V");
  _pTraceTextMethod =
    _theEnv.GetMethodID(_pClass, "traceText", "(Ljava/lang/String;)V");
#endif
}
...
jstring
JNIExternalHandling::executeScript(jobjectArray listOfCommands)
{
#ifdef CODEWORKER_JNI
  ...
  try {
    if (!CGRuntime::executeScript(iLength + 1, tsCommands, NULL)) {
      sErrorMessage = "Bad command line passed to 'CodeWorker' interpreter: "
                      "shouldn't occur, please call the authors";
    }
    ...
    sErrorMessage =
      "unexpected exception encountered while running CodeWorker interpreter";
  }
...
std::string
JNIExternalHandling::inputLine(bool bEcho)
{
#ifdef CODEWORKER_JNI
  std::string sLine;
  if (_pInputLineMethod != 0) {
    jclass pBooleanClass = _theEnv.FindClass("java/lang/Boolean");
    jmethodID pBooleanConstructor =
      _theEnv.GetMethodID(pBooleanClass, "<init>", "(B)V");
    jobject pEcho =
      _theEnv.NewObject(pBooleanClass, pBooleanConstructor, bEcho);
    jstring pLine =
      (jstring)_theEnv.CallObjectMethod(_pJNIObject, _pInputLineMethod, pEcho);
...
std::string
JNIExternalHandling::inputKey(bool bEcho)
{
  return "JNIExternalHandling::inputKey() NOT IMPLEMENTED YET";
}

void
JNIExternalHandling::traceLine(const std::string& sLine)
{
#ifdef CODEWORKER_JNI
  if (_pTraceLineMethod == 0)
    return;
  _theEnv.CallVoidMethod(
    _pJNIObject, _pTraceLineMethod, _theEnv.NewStringUTF(sLine.c_str()));
#endif
}

void
JNIExternalHandling::traceText(const std::string& sText)
{
#ifdef CODEWORKER_JNI
  if (_pTraceTextMethod == 0)
    return;
  _theEnv.CallVoidMethod(
    _pJNIObject, _pTraceTextMethod, _theEnv.NewStringUTF(sText.c_str()));
#endif
}
}
