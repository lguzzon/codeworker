...
bool
DtaConsoleScript::betweenCommands(ScpStream& script, GrfBlock&)
{
  script.skipEmpty();
  if (script.isEqualTo('@') && (_sCursor.size() > 1))
    waitForCommand(script);
  return false;
}

void
DtaConsoleScript::beforeParsingABlock(ScpStream&, GrfBlock&)
{
  _sCursor += ">";
}

void
DtaConsoleScript::afterParsingABlock(ScpStream&, GrfBlock&)
{
  _sCursor = _sCursor.substr(1);
}

void
DtaConsoleScript::handleNotAWordCommand(ScpStream& script, GrfBlock& block)
{
  if (script.isEqualTo('?'))
...
void
DtaConsoleScript::handleUnknownCommand(const std::string& sCommand,
                                       ScpStream& script,
                                       GrfBlock& block)
{
...
void
DtaConsoleScript::waitForCommand(ScpStream& stream)
{
  std::string sText;
  ...
  do {
    ...
    for (int iIndex = sText.size() - 1; iIndex >= 0; iIndex--) {
      if ((sText[iIndex] < '\0') || (sText[iIndex] > ' ')) {
        bContinue = (sText[iIndex] != ';') && (sText[iIndex] != '{') &&
                    (sText[iIndex] != '}');
        break;
      }
    }
    if (bContinue) {
      stream.skipEmpty();
      bContinue = !(stream.isEqualToIdentifier("help") ||
                    stream.isEqualToIdentifier("quit") ||
                    stream.isEqualToIdentifier("exit"));
...
void
DtaConsoleScript::traceEngine() const
{
...
DtaConsoleScript::execute(DtaScriptVariable& thisContext)
{
  ...
  while (bContinue) {
    ...
    try {
      ...
      if (strcmp(sExit, "exit") == 0)
        bContinue = false;
      else
        CGRuntime::traceLine(sExit);
    }
