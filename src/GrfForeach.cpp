...
public:
    ...
    virtual DtaArrayIterator* clone() const
    {
        return new DtaMapIterator(*this);
    }
    virtual int size() const
    {
        return _map.size();
...
void GrfForeach::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (_bSorted) {
        ...
        if (_iCascading != NO_CASCADING) {
            ...
            CW_BODY_ENDL;
            CW_BODY_INDENT << "\t_COMPILER_INDEX_FOREACH_STACK(std::list<DtaScriptVariable*>::const_iterator& i, const std::list<DtaScriptVariable*>* pList, _COMPILER_INDEX_FOREACH_STACK* pParent) : i_(i), pList_(pList), pParent_(pParent) {}";
            CW_BODY_ENDL;
            CW_BODY_INDENT << "} *_compilerIndexForeach_stack = NULL;";
            CW_BODY_ENDL;
            CW_BODY_INDENT << "if (_compilerMapForeach_" << sIndex << " != NULL) _compilerIndexForeach_" << sIndex << " = _compilerMapForeach_" << sIndex << "->begin();";
            CW_BODY_ENDL;
            ...
            CW_BODY_ENDL;
            CW_BODY_INDENT << "if (_compilerMapForeach_" << sIndex << " != 0) for (; _compilerIndexForeach_" << sIndex << " != _compilerMapForeach_" << sIndex << "->end(); ++_compilerIndexForeach_" << sIndex << ") {";
            CW_BODY_ENDL;
            theCompilerEnvironment.incrementIndentation();

            if (_iCascading == CASCADING_FIRST) {
                CW_BODY_INDENT << "CppParsingTree_var _compilerForeachNext(CppParsingTree_var(*_compilerIndexForeach_" << sIndex << ").getNode(\"" << _sCascadedName << "\"));";
                CW_BODY_ENDL;
                CW_BODY_INDENT << "if (!_compilerForeach_firstDone && !_compilerForeachNext.isNull() && (_compilerForeachNext.getArray() != NULL)) {";
                CW_BODY_ENDL;
                CW_BODY_INDENT << "\t_compilerIndexForeach_stack = new _COMPILER_INDEX_FOREACH_STACK(_compilerIndexForeach_" << sIndex << ", _compilerMapForeach_" << sIndex << ", _compilerIndexForeach_stack);";
                CW_BODY_ENDL;
                CW_BODY_INDENT << "\t_compilerMapForeach_" << sIndex << " = _compilerForeachNext.getArray();";
                CW_BODY_ENDL;
                CW_BODY_INDENT << "\t_compilerIndexForeach_" << sIndex << " = _compilerMapForeach_" << sIndex << "->begin();";
                CW_BODY_ENDL;
    ...
    if (_iCascading != NO_CASCADING) {
        if (_iCascading == CASCADING_LAST) {
            CW_BODY_INDENT << "CppParsingTree_var _compilerForeachNext(CppParsingTree_var(*_compilerIndexForeach_" << sIndex << ").getNode(\"" << _sCascadedName << "\"));";
            CW_BODY_ENDL;
            CW_BODY_INDENT << "if (!_compilerForeachNext.isNull() && (_compilerForeachNext.getArray() != NULL)) {";
            CW_BODY_ENDL;
        ...
        CW_BODY_INDENT << "}";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "if ((_compilerMapForeach_" << sIndex << " == NULL) || (_compilerIndexForeach_" << sIndex << " == _compilerMapForeach_" << sIndex << "->end())) {";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\tif (_compilerIndexForeach_stack == NULL) _compilerForeach_realBreak = true;";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\telse {";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\t\t_compilerForeach_realBreak = false;";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\t\t_compilerIndexForeach_" << sIndex << " = _compilerIndexForeach_stack->i_;";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\t\t_compilerMapForeach_" << sIndex << " = _compilerIndexForeach_stack->pList_;";
        CW_BODY_ENDL;
        CW_BODY_INDENT << "\t\t_COMPILER_INDEX_FOREACH_STACK* pTemp = _compilerIndexForeach_stack;";
        CW_BODY_ENDL;
