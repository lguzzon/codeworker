...
DtaScriptFactory::SCRIPT_TYPE DtaScript::getType() const
{
    return DtaScriptFactory::COMMON_SCRIPT;
}
bool DtaScript::isAParseScript() const
{
    return false;
}
bool DtaScript::isAGenerateScript() const
{
    return false;
}
...
void DtaScript::parseFile(const char* tcScriptFileName, GrfBlock* pIncluded, bool bDebugMode, const std::string& sQuantifyFile)
{
    ...
    if (ScpStream::existVirtualFile(tcScriptFileName)) {
        ...
        if (!ScpStream::loadVirtualFile(tcScriptFileName, sContent)) {
            throw UtlException(std::string("unable to load the virtual script file \"") + tcScriptFileName + "\"");
        }
...
void DtaScript::handleNotAWordCommand(ScpStream& script, GrfBlock& block)
{
    ...
    if (iChar == (int) '{') {
        ...
        if (!script.readIdentifier(sDirective)) {
            throw UtlException(script, "preprocessor directive expected after '#' symbol");
        }
    ...
    if (sDirective == "#include") {
        ...
        if (sPredicat == "existFunction") {
            ...
            if (!script.readIdentifier(sFunction)) {
                throw UtlException(script, "syntax error: identifier expected (function name)");
            }
        ...
        if (!script.isEqualToIdentifier("end")) {
            throw UtlException(script, "#end directive expected, instead of #" + sDirective);
        }
        ...
        if (!script.readIdentifier(sPackageName)) {
            throw UtlException(script, "syntax error: module name expected as a constant string");
        }
        ...
        if (!script.readInt(iLine)) {
            throw UtlException(script, "syntax error: line number expected as a constant integer");
        }
        ...
        if (iChar == (int) '/') {
            ...
            if (iChar != (int) '/') {
                throw UtlException(script, "syntax error: end of line or line comment expected");
            }
        ...
        if (!script.readString(sReferenceFile)) {
            throw UtlException(script, "syntax error on '#attach': reference script file expected");
        }
        ...
        if (!script.readIdentifier(sKey)) {
            throw UtlException(script, "syntax error on '#attach': identifier expected");
        }
        ...
        if (script.isEqualTo(',')) {
            ...
            if (!script.readInt(iMagicNumber)) {
                throw UtlException(script, "syntax error on '#attach': magic number expected after the comma");
            }
        ...
        if (getFilenamePtr() == NULL) {
            throw UtlException(script, "a '#reference' area must be put into a script file; this isn't the case here");
        }
        ...
        if (!script.readIdentifier(sKey)) {
            throw UtlException(script, "syntax error on '#reference': identifier expected");
        }
...
void DtaScript::handleUnknownCommand(const std::string& sCommand, ScpStream& script, GrfBlock& block)
{
    ...
    if (iChar == (int) ':') {
        if (!script.isEqualTo(':')) {
            ...
            if (pPackage == NULL) {
                throw UtlException(script, "unknown module '" + sCommand + "'; type '#use' to load the dynamic library");
            }

            script.skipEmpty();

            if (!script.readIdentifier(sPackageMember)) {
                throw UtlException(script, "function or variable name expected after '" + sCommand + "::'");
            }
    ...
    if (iChar == (int) '(') {
        if (pPackage != NULL) {
            ...
            if (pFunction == NULL) {
                throw UtlException(script, "the module '" + sCommand + "' doesn't export the function '" + sCommand + "::" + sPackageMember + "'");
            }
        ...
        if (pPackage != NULL) {
            if (pPackage->getVariable(sPackageMember) == NULL) {
                throw UtlException(script, "the module '" + sCommand + "' doesn't export the variable '" + sCommand + "::" + sPackageMember + "'");
            }
        ...
        if (pMethodFunction != NULL) {
            ...
            if (!script.isEqualTo(';')) {
                throw UtlException(script, "';' expected to close an instruction (function call)");
            }
            ...
            if (!script.isEqualTo('=')) {
                throw UtlException(script, "syntax error: assignment expected ('+=' or '=' operator)");
            }
            ...
            if (!script.isEqualTo(';')) {
                throw UtlException(script, "';' expected to close an assignment instruction");
            }
        }
        ...
        if (script.isEqualTo('>')) {
            ...
            if (!script.isEqualTo('(')) {
                throw UtlException(script, "'(' expected after instantiating a template procedure");
            }
            ...
            if (!script.isEqualTo(';')) {
                throw UtlException(script, "';' expected after calling a function or procedure");
            }
        } else {
            script.setInputLocation(iLocation);
            throw UtlException(script, "unknown command or procedure or function \"" + sCommand + "\"");
...
bool DtaScript::betweenCommands(ScpStream& /*script*/, GrfBlock& /*block*/)
{
    return false;
}
...
std::map<std::string, int>& DtaScript::getParsingFunctionRegister()
{
    ...
    if (_register.empty()) {
        ...
        if (_register.find("appendFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('appendFile') already exists");
        }

        _register["appendFile"] = 179126;

        if (_register.find("autoexpand") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('autoexpand') already exists");
        }

        _register["autoexpand"] = 2911241;

        if (_register.find("clearVariable") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('clearVariable') already exists");
        }

        _register["clearVariable"] = 7413225;

        if (_register.find("compileToCpp") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('compileToCpp') already exists");
        }

        _register["compileToCpp"] = 18027701;

        if (_register.find("copyFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('copyFile') already exists");
        }

        _register["copyFile"] = 2106257;

        if (_register.find("copyGenerableFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('copyGenerableFile') already exists");
        }

        _register["copyGenerableFile"] = 9921918;

        if (_register.find("copySmartDirectory") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('copySmartDirectory') already exists");
        }

        _register["copySmartDirectory"] = 18645881;

        if (_register.find("cutString") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('cutString') already exists");
        }

        _register["cutString"] = 5833203;

        if (_register.find("environTable") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('environTable') already exists");
        }

        _register["environTable"] = 19353497;

        if (_register.find("error") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('error') already exists");
        }

        _register["error"] = 16784904;

        if (_register.find("executeString") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('executeString') already exists");
        }

        _register["executeString"] = 10410726;

        if (_register.find("expand") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('expand') already exists");
        }

        _register["expand"] = 5800090;

        if (_register.find("extendExecutedScript") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('extendExecutedScript') already exists");
        }

        _register["extendExecutedScript"] = 11255540;

        if (_register.find("generate") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('generate') already exists");
        }

        _register["generate"] = 3100533;

        if (_register.find("generateString") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('generateString') already exists");
        }

        _register["generateString"] = 17410534;

        if (_register.find("insertElementAt") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('insertElementAt') already exists");
        }

        _register["insertElementAt"] = 13901206;

        if (_register.find("invertArray") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('invertArray') already exists");
        }

        _register["invertArray"] = 7040035;

        if (_register.find("listAllGeneratedFiles") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('listAllGeneratedFiles') already exists");
        }

        _register["listAllGeneratedFiles"] = 3779115;

        if (_register.find("loadProject") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('loadProject') already exists");
        }

        _register["loadProject"] = 15590483;

        if (_register.find("openLogFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('openLogFile') already exists");
        }

        _register["openLogFile"] = 4625718;

        if (_register.find("parseAsBNF") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('parseAsBNF') already exists");
        }

        _register["parseAsBNF"] = 8623893;

        if (_register.find("parseStringAsBNF") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('parseStringAsBNF') already exists");
        }

        _register["parseStringAsBNF"] = 12713924;

        if (_register.find("parseFree") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('parseFree') already exists");
        }

        _register["parseFree"] = 3007935;

        if (_register.find("produceHTML") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('produceHTML') already exists");
        }

        _register["produceHTML"] = 2453643;

        if (_register.find("putEnv") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('putEnv') already exists");
        }

        _register["putEnv"] = 18042462;

        if (_register.find("randomSeed") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('randomSeed') already exists");
        }

        _register["randomSeed"] = 15941076;

        if (_register.find("removeAllElements") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeAllElements') already exists");
        }

        _register["removeAllElements"] = 15019924;

        if (_register.find("removeElement") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeElement') already exists");
        }

        _register["removeElement"] = 15071448;

        if (_register.find("removeFirstElement") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeFirstElement') already exists");
        }

        _register["removeFirstElement"] = 19507408;

        if (_register.find("removeLastElement") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeLastElement') already exists");
        }

        _register["removeLastElement"] = 18768482;

        if (_register.find("removeRecursive") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeRecursive') already exists");
        }

        _register["removeRecursive"] = 5023246;

        if (_register.find("removeVariable") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('removeVariable') already exists");
        }

        _register["removeVariable"] = 7468320;

        if (_register.find("saveBinaryToFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('saveBinaryToFile') already exists");
        }

        _register["saveBinaryToFile"] = 3078933;

        if (_register.find("saveProject") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('saveProject') already exists");
        }

        _register["saveProject"] = 18607068;

        if (_register.find("saveProjectTypes") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('saveProjectTypes') already exists");
        }

        _register["saveProjectTypes"] = 730621;

        if (_register.find("saveToFile") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('saveToFile') already exists");
        }

        _register["saveToFile"] = 14995060;

        if (_register.find("setCommentBegin") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setCommentBegin') already exists");
        }

        _register["setCommentBegin"] = 14972940;

        if (_register.find("setCommentEnd") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setCommentEnd') already exists");
        }

        _register["setCommentEnd"] = 9300734;

        if (_register.find("setGenerationHeader") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setGenerationHeader') already exists");
        }

        _register["setGenerationHeader"] = 3237703;

        if (_register.find("setIncludePath") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setIncludePath') already exists");
        }

        _register["setIncludePath"] = 5288747;

        if (_register.find("setNow") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setNow') already exists");
        }

        _register["setNow"] = 9162260;

        if (_register.find("setProperty") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setProperty') already exists");
        }

        _register["setProperty"] = 10303831;

        if (_register.find("setTextMode") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setTextMode') already exists");
        }

        _register["setTextMode"] = 3850482;

        if (_register.find("setVersion") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setVersion') already exists");
        }

        _register["setVersion"] = 12168214;

        if (_register.find("setWriteMode") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setWriteMode') already exists");
        }

        _register["setWriteMode"] = 5067744;

        if (_register.find("setWorkingPath") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setWorkingPath') already exists");
        }

        _register["setWorkingPath"] = 16222516;

        if (_register.find("sleep") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('sleep') already exists");
        }

        _register["sleep"] = 9522647;

        if (_register.find("slideNodeContent") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('slideNodeContent') already exists");
        }

        _register["slideNodeContent"] = 13707430;

        if (_register.find("sortArray") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('sortArray') already exists");
        }

        _register["sortArray"] = 9410907;

        if (_register.find("traceEngine") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('traceEngine') already exists");
        }

        _register["traceEngine"] = 13561575;

        if (_register.find("traceLine") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('traceLine') already exists");
        }

        _register["traceLine"] = 16138969;

        if (_register.find("traceObject") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('traceObject') already exists");
        }

        _register["traceObject"] = 8856036;

        if (_register.find("traceStack") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('traceStack') already exists");
        }

        _register["traceStack"] = 7087939;

        if (_register.find("traceText") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('traceText') already exists");
        }

        _register["traceText"] = 16373778;

        if (_register.find("translate") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('translate') already exists");
        }

        _register["translate"] = 4934734;

        if (_register.find("attachInputToSocket") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('attachInputToSocket') already exists");
        }

        _register["attachInputToSocket"] = 4116339;

        if (_register.find("detachInputFromSocket") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('detachInputFromSocket') already exists");
        }

        _register["detachInputFromSocket"] = 16057876;

        if (_register.find("goBack") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('goBack') already exists");
        }

        _register["goBack"] = 13375983;

        if (_register.find("setInputLocation") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setInputLocation') already exists");
        }

        _register["setInputLocation"] = 17901405;

        if (_register.find("allFloatingLocations") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('allFloatingLocations') already exists");
        }

        _register["allFloatingLocations"] = 16157783;

        if (_register.find("attachOutputToSocket") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('attachOutputToSocket') already exists");
        }

        _register["attachOutputToSocket"] = 2539668;

        if (_register.find("detachOutputFromSocket") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('detachOutputFromSocket') already exists");
        }

        _register["detachOutputFromSocket"] = 10159185;

        if (_register.find("incrementIndentLevel") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('incrementIndentLevel') already exists");
        }

        _register["incrementIndentLevel"] = 9232969;

        if (_register.find("insertText") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('insertText') already exists");
        }

        _register["insertText"] = 2938502;

        if (_register.find("insertTextOnce") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('insertTextOnce') already exists");
        }

        _register["insertTextOnce"] = 10767911;

        if (_register.find("insertTextToFloatingLocation") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('insertTextToFloatingLocation') already exists");
        }

        _register["insertTextToFloatingLocation"] = 2074012;

        if (_register.find("insertTextOnceToFloatingLocation") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('insertTextOnceToFloatingLocation') already exists");
        }

        _register["insertTextOnceToFloatingLocation"] = 19491581;

        if (_register.find("overwritePortion") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('overwritePortion') already exists");
        }

        _register["overwritePortion"] = 4193756;

        if (_register.find("populateProtectedArea") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('populateProtectedArea') already exists");
        }

        _register["populateProtectedArea"] = 19875729;

        if (_register.find("resizeOutputStream") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('resizeOutputStream') already exists");
        }

        _register["resizeOutputStream"] = 3517365;

        if (_register.find("setFloatingLocation") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setFloatingLocation') already exists");
        }

        _register["setFloatingLocation"] = 11884637;

        if (_register.find("setOutputLocation") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setOutputLocation') already exists");
        }

        _register["setOutputLocation"] = 8004824;

        if (_register.find("setProtectedArea") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('setProtectedArea') already exists");
        }

        _register["setProtectedArea"] = 14826777;

        if (_register.find("writeBytes") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('writeBytes') already exists");
        }

        _register["writeBytes"] = 6512844;

        if (_register.find("writeText") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('writeText') already exists");
        }

        _register["writeText"] = 16211116;

        if (_register.find("writeTextOnce") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('writeTextOnce') already exists");
        }

        _register["writeTextOnce"] = 8521805;

        if (_register.find("closeSocket") != _register.end()) {
            throw UtlException("DtaScript::getParsingFunctionRegister('closeSocket') already exists");
        }
...
void DtaScript::parseForeachListDeclaration(GrfBlock& block, ScpStream& script, GrfForeach* pForeach)
{
    ...
    for (;;) {
        if (script.isEqualToIdentifier("sorted")) {
            ...
            if (!pForeach->getSorted()) {
                throw UtlException(script, "'sorted' keywork expected before 'by_value'");
            }
...
void DtaScript::parseDo(GrfBlock& block, ScpStream& script)
{
    ...
    if (!script.readIdentifier(sWord)) {
        throw UtlException(script, "syntax error, 'while' expected after a 'do' statement");
    }
    ...
    while (!script.isEqualTo('}')) {
        ...
        if ((!script.readIdentifier(sWord)) || ((sWord != "case") && (sWord != "start") && (sWord != "default"))) {
            if (bDefault) {
                ...
                throw UtlException(script, "syntax error, 'case', 'start' or 'default' expected");
            }
        }
        ...
        if (sWord == "default") {
            ...
            if (!script.readStringOrCharLiteral(sConstant)) {
                throw UtlException(script, "syntax error, constant string expected after a '" + sWord + "'");
            }

            if (sWord == "case") {
                if (pSwitch->equalString(sConstant.c_str())) {
                    throw UtlException(script, "syntax error, constant string \"" + sConstant + "\" already registered as a 'case'");
                }

                pSwitch->insertEqualString(sConstant.c_str());
            } else {
                if (pSwitch->startString(sConstant.c_str())) {
                    throw UtlException(script, "syntax error, constant string \"" + sConstant + "\" already registered as a 'start'");
                }
        ...
        if (!script.isEqualTo(':')) {
            throw UtlException(script, "syntax error, ':' expected after a '" + sWord + "'");
        }
...
void DtaScript::parseFinally(GrfBlock& block, ScpStream& script)
{
    if (_sCurrentFunctionBody.empty()) {
        throw UtlException(script, "'finally' must be called inside a function's body!");
    }
...
{
    if (_sCurrentFunctionBody.empty()) {
        throw UtlException(script, "'return' must be called inside a function's body!");
    }
...
void DtaScript::parseDeclare(GrfBlock& block, ScpStream& script)
{
    ...
    if (sWord == "function") {
        ...
        throw UtlException(script, std::string("unknown type of declaration: \"") + sWord + "\"");
    }
...
void DtaScript::parseExternal(GrfBlock& block, ScpStream& script)
{
    ...
    if (sWord == "function") {
        ...
        if (pFunction->isBodyDefined()) {
            throw UtlException(script, "syntax error, function \"" + pFunction->getName() + "\" is expected external, but it has already a body");
        }
...
void DtaScript::parseStringAsStandardInput(GrfBlock& block, ScpStream& script)
{
    ...
    if (requiresParsingInformation()) {
        pStringAsStandardInputBlock->setParsingInformation(getFilenamePtr(), script);
    }
...
{
    if (!isAParseScript()) {
        throw UtlException(script, "statement modifier 'parsed_file' is available on parsing scripts only");
    }
...
{
    if (!isAParseScript()) {
        throw UtlException(script, "statement modifier 'parsed_string' is available on parsing scripts only");
    }
...
EXPRESSION_TYPE DtaScript::parseVariableType(GrfBlock& block, ScpStream& script)
{
    ...
    if (sType == "value") {
        ...
        throw UtlException(script, std::string("Unknown parameter type \"") + sType + "\"");
    }
...
EXPRESSION_TYPE DtaScript::parseClassType(GrfBlock& block, ScpStream& script, EXPRESSION_TYPE exprType)
{
    ...
    if (script.isEqualTo('<')) {
        if (exprType == VALUE_EXPRTYPE) {
            throw UtlException(script, "only a node or a reference parameter can point to a class");
        }
        ...
        if (hasTargetLanguage()) {
            ...
            if (pNode == NULL) {
                throw UtlException(script, "instruction type 'target_type' is expected to have the attribute 'target_type'");
            }
...
GrfFunction* DtaScript::parseFunction(GrfBlock& block, ScpStream& script, bool bPrototypeOnly)
{
    ...
    if (script.isEqualTo('<')) {
        ...
        if (!script.readString(sTemplate)) {
            if (script.readIdentifier(sTemplate)) {
                ...
                if (script.readDouble(dValue)) {
                    ...
                    throw UtlException(script, "syntax error, template function \"" + sFunctionName + "\" expects a constant string or an identifier for instantiating");
                }
            }
        }
        ...
        if (!script.isEqualTo('>')) {
            throw UtlException(script, "syntax error, template function \"" + sFunctionName + "\" expects '>' after constant string \"" + sTemplate + "\"");
        }
    }
    ...
    if (bAlreadyExist && (!bPrototypeOnly) && pFunction->isBodyDefined()) {
        throw UtlException(script, "syntax error, function \"" + sFunctionName + "\" has already been defined");
    }
        ...
        do {
            ...
            if (script.isEqualTo('=')) {
                ...
                if ((exprType & 0xFF) == VALUE_EXPRTYPE) {
                    ...
                    if (!script.readIdentifier(sVarName) || ((sVarName != "this") && (sVarName != "project") && (sVarName != "null") && (DtaProject::getInstance().getGlobalVariableType(sVarName) == UNKNOWN_EXPRTYPE))) {
                        throw UtlException(script, "global variable name or 'null' expected");
                    }
            ...
            if (bAlreadyExist) {
                ...
                if (!pFunction->addParameterAndType(sWord.c_str(), exprType, pDefaultValue)) {
                    throw UtlException(script, "syntax error, two parameters with the same name");
                }
    ...
    if (!bPrototypeOnly) {
        ...
        if (pFunction->isExternal()) {
            throw UtlException(script, "syntax error, function \"" + sFunctionName + "\" is external, so it shouldn't implement a body here");
        }
        ...
        if (bGenericKey && script.isEqualTo("{{")) {
            ...
            if (!script.isEqualTo('}')) {
                throw UtlException(script, "syntax error, '}}' expected to close the pattern-template function");
            }
...
DtaClass* DtaScript::parseClass(GrfBlock& block, ScpStream& script)
{
    ...
    if (pClass != NULL) {
        if (!pClass->isForwardDeclaration()) {
            throw UtlException(script, "the class '" + sClassName + "' cannot be declared twice");
        }
    } else {
    ...
    if (script.isEqualTo(':')) {
        ...
        if (!script.readIdentifier(sExtendedClass)) {
            throw UtlException(script, "syntax error, extended class expected after ':'");
        }

        DtaClass* pExtendedClass = block.getClass(sExtendedClass);

        if (pExtendedClass == NULL) {
            throw UtlException(script, "the extended class '" + sExtendedClass + "' doesn't exist");
        }
    ...
    if (script.isEqualTo(';')) {
        ...
        if (pClass->getExtendedClass() != NULL) {
            throw UtlException(script, "a forward declaration of a class doesn't admit the specification of the extended class");
        }
    } else {
        // a body
        if (!script.isEqualTo('{')) {
        ...
        while (!script.isEqualTo('}')) {
            ...
            if (pClass->getAttributeType(sAttribute) != UNKNOWN_EXPRTYPE) {
                throw UtlException(script, "the attribute '" + sClassName + "::" + sAttribute + "' is declared twice");
            }
...
ExprScriptExpression* DtaScript::parseLiteralUnsignedExpression(GrfBlock& block, ScpStream& script)
{
    ...
    if (script.readPythonString(sWord)) {
        ...
        if (!script.isEqualTo('$')) {
            throw UtlException(script, "syntax error, '$' expected to close a numerical expression");
        }
    } else if (script.isEqualTo('#')) {
        pExpr = parsePreprocessorExpression(block, script);
    } else if (script.isEqualTo('~')) {
        if (!_bNumericalExpression) {
            throw UtlException(script, "syntax error, bitwise not is an arithmetic operator to put between '$'");
        }
        ...
        if (script.readCharLiteral(iChar)) {
            ...
            if (bWithoutBrackets || script.isEqualTo('(')) {
                ...
                if (pPackage == NULL) {
                    throw UtlException(script, "unknown module '" + sWord + "'; type '#use' to load the dynamic library");
                }
                ...
                if (!script.readIdentifier(sFunction)) {
                    throw UtlException(script, "function or variable name expected after '" + sWord + "::'");
                }
                ...
                if (script.isEqualTo('(')) {
                    ...
                    if (pFunction == NULL) {
                        throw UtlException(script, "the module '" + sWord + "' doesn't export the function '" + sWord + "::" + sFunction + "'");
                    }
                    ...
                    if (pPackage->getVariable(sFunction) == NULL) {
                        throw UtlException(script, "the module '" + sWord + "' doesn't export the variable '" + sWord + "::" + sFunction + "'");
                    }
...
ExprScriptExpression* DtaScript::parseComparisonExpression(GrfBlock& block, ScpStream& script)
{
    ...
    do {
        ...
        if (pResultExpr == NULL) {
            ...
            pResultExpr = new ExprScriptNumericComparison(pResultExpr, pExpr, cComparison);
        } else {
...
ExprScriptVariable* DtaScript::parseReferenceExpression(GrfBlock& /*block*/, ScpStream& script)
{
    ...
    if (script.isEqualTo('.') || script.isEqualTo('#') || script.isEqualTo('[') || (script.isEqualTo('$') && script.peekChar() == (int) '[')) {
        throw UtlException(script, "syntax error: reference name expected (not a variable expression)");
    }
...
ExprScriptVariable* DtaScript::parseVariableExpression(GrfBlock& block, ScpStream& script, ExprScriptVariable* pParentExpr, ExprScriptExpression*& pMethodExpr, GrfCommand*& pMethodProc)
{
        ...
        if (bDirective) {
            ...
            if (script.isEqualTo("::")) {
                ...
                if (!script.readIdentifier(sVariable)) {
                    throw UtlException(script, "variable name expected after '" + sWord + "::'");
                }
    ...
    do {
        ...
        if (script.isEqualTo('[')) {
            ...
            if (!script.readIdentifier(sWord) || ((sWord != "front") && (sWord != "back") && (sWord != "parent") && (sWord != "root"))) {
                throw UtlException(script, "'[' or 'front' or 'back' expected after '#' to precise an array index");
            }
        ...
        if (bNext) {
            ...
            if (script.isEqualTo('<')) {
                ...
                if (script.isEqualTo('>')) {
                    ...
                    if (!script.isEqualTo('(')) {
                        throw UtlException(script, "'(' expected after instantiating a template method");
                    }
                } else {
    ...
    if (!sIdentifier.empty()) {
        ...
        while (script.isEqualTo('[')) {
            ...
            if (script.isEqualTo(']')) {
                ...
                if (!script.isEqualTo(']')) {
                    throw UtlException(script, "']' expected to close an array in a motif");
                }
            }
...
ExprScriptFunction* DtaScript::parseFunctionExpression(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller, ExprScriptFunction* pFunction, ExprScriptExpression* pTemplate, bool bBracket)
{
                ...
                do {
                    try {
                        ...
                        case SCRIPTFILE_EXPRTYPE:
                            pFunction->addParameter(parseScriptFileExpression(block, script, DtaScriptFactory::COMMON_SCRIPT));
                            break;

                        case SCRIPTFILE_PATTERN_EXPRTYPE:
                            pFunction->addParameter(parseScriptFileExpression(block, script, DtaScriptFactory::PATTERN_SCRIPT));
                            break;

                        case SCRIPTFILE_FREE_EXPRTYPE:
                            pFunction->addParameter(parseScriptFileExpression(block, script, DtaScriptFactory::FREE_SCRIPT));
                            break;

                        case SCRIPTFILE_BNF_EXPRTYPE:
                            pFunction->addParameter(parseScriptFileExpression(block, script, DtaScriptFactory::BNF_SCRIPT));
                            break;

                        case SCRIPTFILE_TRANSLATE_EXPRTYPE:
                            pFunction->addParameter(parseScriptFileExpression(block, script, DtaScriptFactory::TRANSLATE_SCRIPT));
                            break;
                        }
    ...
    if (iIndex < pFunction->getMinArity()) {
        ...
        if (pFunction != NULL) {
            sMessageFormat += " for call to function '" + std::string(pFunction->getName()) + "'";
        }
...
void DtaScript::checkIfAllowedFunction(ScpStream& script, ExprScriptFunction& theFunction)
{
    if (theFunction.isAGenerateFunction() && !isAGenerateScript()) {
        throw UtlException(script, "function '" + std::string(theFunction.getName()) + "' reserved for template-based scripts only");
    }

    if (theFunction.isAParseFunction() && !isAParseScript()) {
        throw UtlException(script, "function '" + std::string(theFunction.getName()) + "' reserved for parse scripts only");
    }
}
...
void DtaScript::parseReadonlyHook(GrfBlock& block, ScpStream& script)
{
    if (DtaProject::getInstance().getReadonlyHook() != NULL) {
        throw UtlException(script, "function 'readonlyHook(<filename>)' has already been defined and can't be implemented twice");
    }
...
void DtaScript::parseWritefileHook(GrfBlock& block, ScpStream& script)
{
    if (DtaProject::getInstance().getWritefileHook() != NULL) {
        throw UtlException(script, "hook 'writefileHook(<filename>, <position>, <creation>)' has already been defined and can't be implemented twice");
    }
    ...
    if (!script.readIdentifier(sIdentifier)) {
        throw UtlException(script, "syntax error: argument '<filename>' expected for hook 'writefileHook(<filename>, <position>, <creation>)'");
    }
    ...
    if (!script.readIdentifier(sIdentifier)) {
        throw UtlException(script, "syntax error: argument '<position>' expected for hook 'writefileHook(<filename>, <position>, <creation>)'");
    }
    ...
    if (!script.readIdentifier(sIdentifier)) {
        throw UtlException(script, "syntax error: argument '<creation>' expected for hook 'writefileHook(<filename>, <position>, <creation>)'");
    }
...
void DtaScript::parseExpand(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pExpand->setPatternFileName(parseScriptFileExpression(block, script, DtaScriptFactory::PATTERN_SCRIPT));
    } else {
...
void DtaScript::parseGenerate(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pGenerate->setPatternFileName(parseScriptFileExpression(block, script, DtaScriptFactory::PATTERN_SCRIPT));
    } else {
...
void DtaScript::parseGenerateString(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pGenerateString->setPatternFileName(parseScriptFileExpression(block, script, DtaScriptFactory::PATTERN_SCRIPT));
    } else {
...
void DtaScript::parseParseAsBNF(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pParseAsBNF->setBNFFileName(parseScriptFileExpression(block, script, DtaScriptFactory::BNF_SCRIPT));
    } else {
...
void DtaScript::parseParseStringAsBNF(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pParseStringAsBNF->setBNFFileName(parseScriptFileExpression(block, script, DtaScriptFactory::BNF_SCRIPT));
    } else {
...
void DtaScript::parseParseFree(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pParseFree->setDesignFileName(parseScriptFileExpression(block, script, DtaScriptFactory::FREE_SCRIPT));
    } else {
...
void DtaScript::parseTranslate(GrfBlock& block, ScpStream& script, ExprScriptVariable* pMethodCaller)
{
    ...
    if (pMethodCaller == NULL) {
        pTranslate->setPatternFileName(parseScriptFileExpression(block, script, DtaScriptFactory::TRANSLATE_SCRIPT));
    } else {
...
void DtaScript::parseLocalVariable(GrfBlock& block, ScpStream& script)
{
    ...
    do {
        ...
        if (!script.readIdentifier(sVariable)) {
            throw UtlException(script, "syntax error: name of the local variable expected");
        }

        if ((sVariable == "project") || (sVariable == "null") || (sVariable == "this")) {
            throw UtlException(script, "'" + sVariable + "' is a reserved variable, and cannot be redeclared");
        }
...
void DtaScript::parseGlobalVariable(GrfBlock& block, ScpStream& script)
{
    ...
    do {
        ...
        if (!script.readIdentifier(sVariable)) {
            throw UtlException(script, "syntax error: name of the global variable expected");
        }
...
void DtaScript::parseSetAll(GrfBlock& block, ScpStream& script)
{
    ...
    if (!script.isEqualTo('=')) {
        throw UtlException(script, "syntax error: assignation expected ('=' operator)");
    }
...
void DtaScript::parseSyntax(GrfBlock& block, ScpStream& script)
{
    ...
    if (!sParsingMode.empty()) {
        ...
        if (cursor != _mapOfSyntaxModes.end()) {
            ...
            if (sParsingScript.empty()) {
                throw UtlException("on directive '#syntax': no BNF script file attached to the parsing mode '" + sParsingMode + "'");
            }
...
void DtaScript::compileCpp(CppCompilerEnvironment& theCompilerEnvironment, const std::string& sScriptFilename) const
{
    if (theCompilerEnvironment.pushFilename(sScriptFilename)) {
        try {
            ...
            if (hasTargetLanguage()) {
                CW_BODY_STREAM << "// The two following preprocessor definitions delimit the " << getTargetLanguage() << " code";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "// you have written in the original CodeWorker script. They do nothing but";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "// to inform you about the position of this piece of code in the script:";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "//     - lang: target language (certainly " << getTargetLanguage() << " here),";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "//     - file: CodeWorker script file the piece of code is coming from,";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "//     - line: line number of the piece of code in the script,";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "#define BEGIN_TARGET_LANGUAGE_CODE(lang, file, line)";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "#define END_TARGET_LANGUAGE_CODE(lang)";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "// The next preprocessor definitions indicate a syntactic entity,";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "// defined in the target language:";
                CW_BODY_ENDL;
                CW_BODY_STREAM << "#define TARGET_LANGUAGE_TYPE_SPECIFIER(type_specifier) type_specifier";
                CW_BODY_ENDL;
            ...
            CW_BODY_STREAM << "#include \"" << theCompilerEnvironment.getRadical() << ".h\"";
            CW_BODY_ENDL;
            CW_BODY_ENDL;
            ...
            CW_BODY_STREAM << "Execute" << sIdentifier << "& Execute" << sIdentifier << "::instance() {";
            CW_BODY_ENDL;
            ...
            if (theCompilerEnvironment.getIncludeParentScript().empty()) {
                ...
                if (DtaProject::getInstance().getReadonlyHook() != NULL) {
                    CW_BODY_STREAM << "\t\tCGRuntime::registerReadonlyHook(" << DtaProject::getInstance().getReadonlyHook()->getFunctionName() << ");";
                    CW_BODY_ENDL;
                }

                if (DtaProject::getInstance().getWritefileHook() != NULL) {
                    CW_BODY_STREAM << "\t\tCGRuntime::registerWritefileHook(" << DtaProject::getInstance().getWritefileHook()->getFunctionName() << ");";
                    CW_BODY_ENDL;
                }
                ...
                for (i = listOfModules.begin(); i != listOfModules.end(); i++) {
                    CW_BODY_STREAM << "\t\tCGRuntime::registerScript(\"" << CppCompilerEnvironment::getRadical(*i) << "\", &Execute" << convertFilenameAsIdentifier(CppCompilerEnvironment::getRadical(*i)) << "::instance());";
                    CW_BODY_ENDL;
                }

                {
                    ...
                    for (std::map<std::string, DynPackage*>::const_iterator i = allPackages.begin(); i != allPackages.end(); ++i) {
                        CW_BODY_STREAM << "\t\t" << i->first << " initializeDynamicPackage" << i->first << ";";
                        CW_BODY_ENDL;
                    }
                }
                ...
                for (i = listOfModules.begin(); i != listOfModules.end(); i++) {
                    ...
                    theCompilerEnvironment.getMainHeader().endl();
                    theCompilerEnvironment.getMainHeader() << "\t\tstatic Execute" << sIdentifier << "& instance();";
                    theCompilerEnvironment.getMainHeader().endl();
...
int DtaScript::computeReferenceMagicNumber(ScpStream& script, const char* tcFilename, const std::string& sKey)
{
    ...
    for (;;) {
        ...
        if (iChar == -1) {
            throw UtlException(script, "syntax error on '#reference' area: '#end " + sKey + "' expected before end of file");
        }
...
int DtaScript::computeReferenceMagicNumber(ScpStream& theStream, const std::string& sKey)
{
    ...
    for (;;) {
        ...
        if (iChar == -1) {
            throw UtlException(theStream, "syntax error on '#reference' area: '#end " + sKey + "' expected before end of file");
        }
        ...
        if ((iChar == '#') && (iOldChar == '\n')) {
            ...
            if (!theStream.readIdentifier(sDirective)) {
                throw UtlException(theStream, "preprocessor directive expected after '#' symbol");
            }
            ...
            if (sDirective == "reference") {
                ...
                if (!theStream.readIdentifier(sEmbeddedKey)) {
                    throw UtlException(theStream, "syntax error on '#reference': identifier expected");
                }
