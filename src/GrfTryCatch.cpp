...
void
GrfTryCatch::applyRecursively(APPLY_ON_COMMAND_FUNCTION apply)
{
  GrfCommand::applyRecursively(apply);
  if (_pTryBlock != NULL)
    _pTryBlock->applyRecursively(apply);
  if (_pCatchBlock != NULL)
    _pCatchBlock->applyRecursively(apply);
}

SEQUENCE_INTERRUPTION_LIST
GrfTryCatch::executeInternal(DtaScriptVariable& visibility)
{
  ...
  try {
    ...
    std::string sError =
      exception.getMessage() + CGRuntime::endl() + exception.getTraceStack();
...
SEQUENCE_INTERRUPTION_LIST
GrfTryCatch::handleCatch(DtaScriptVariable& visibility,
                         const std::string& sError)
{
  DtaScriptVariable stackCatch(&visibility, "##stack## catch");
  DtaScriptVariable* pVariable =
    stackCatch.getNonRecursiveVariable(stackCatch, *_pVariable);
  pVariable->setValue(sError.c_str());
  return _pCatchBlock->execute(stackCatch);
}

void
GrfTryCatch::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  CW_BODY_INDENT << "try ";
  ...
  CW_BODY_INDENT << "CppParsingTree_value " << _pVariable->getName()
                 << "(_compilerException_" << _pVariable->getName()
                 << ".getMessage());";
  CW_BODY_ENDL;
