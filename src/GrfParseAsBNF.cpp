...
void
GrfParseAsBNF::setBNFFileName(ExprScriptScriptFile* pBNFFileName)
{
...
GrfParseAsBNF::executeInternal(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pClass = visibility.getExistingVariable(*_pClass);
  if (pClass == NULL)
    throw UtlException(
      "runtime error: variable '" + _pClass->toString() +
      "' doesn't exist while calling procedure 'parseAsBNF()'");
  ...
  if (_pBNFFileName != NULL) {
    ...
    if ((_pCachedScript == NULL) || (_sCachedBNFFile != sBNFFileName) ||
        ScpStream::existVirtualFile(sBNFFileName)) {
      if (_pCachedScript != NULL)
        delete _pCachedScript;
      _pCachedScript = new DtaBNFScript(getParent());
      _sCachedBNFFile = sBNFFileName;
      _pCachedScript->parseFile(sBNFFileName.c_str(),
                                _sCurrentDirectoryAtCompileTime);
    }
  }

  SEQUENCE_INTERRUPTION_LIST result =
    _pCachedScript->generate(sOutputFile.c_str(), *pClass);
  switch (result) {
...
void
GrfParseAsBNF::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ExprScriptConstant* pConstantFileName =
    dynamic_cast<ExprScriptConstant*>(_pBNFFileName);
  ...
  CW_BODY_INDENT << "CGRuntime::parseAsBNF(&Execute" << sRadical
                 << "::instance(), ";
