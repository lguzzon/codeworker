...
GrfGeneratedString::executeInternal(DtaScriptVariable& visibility)
{
  ...
  std::string sResult =
    generatedString.closeGenerate(true, NULL, pOldOutputStream);
  if (pVariable != NULL)
    pVariable->setValue(sResult.c_str());
  return result;
}

void
GrfGeneratedString::compileCpp(
