...
GrfSetAssignment::executeInternal(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pVariable = visibility.getExistingVariable(*_pVariable);
  if (pVariable == NULL) {
    std::string sErrorMessage =
      "variable '" + _pVariable->toString() +
      "' should exist either into the tree (see 'insert') or as a local "
      "variable (see 'local') before assigning a value through 'set'";
