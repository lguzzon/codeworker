...
bool BNFMultiplicity::isABNFCommand() const
{
    return true;
}
...
void BNFMultiplicity::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    if (!_boundaries.bConstantBoundaries) {
        throw UtlException("BNFMultiplicity::compileCpp(): variable boundaries not handled yet!");
    }
    ...
    if (i != getCommands().end()) {
        ...
        if (_boundaries.choice.constant._iEnd != 1) {
            ...
            if (_boundaries.choice.constant._iBegin > 1) {
                CW_BODY_STREAM << "if (_compilerClause_iteration_" << iCursor << " >= " << _boundaries.choice.constant._iBegin << ") ";
            }
            ...
            if (_boundaries.choice.constant._iEnd > 0) {
                CW_BODY_STREAM << " && (_compilerClause_iteration_" << iCursor << " < " << _boundaries.choice.constant._iEnd << ")";
            }
        ...
        if (_pVariableToAssign != NULL) {
            ...
            CW_BODY_INDENT << "std::string sText = CGRuntime::getLastReadChars(CGRuntime::getInputLocation() - " << tcNotEmptyLocation << ");";
            CW_BODY_ENDL;
