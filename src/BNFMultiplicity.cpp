...
void
BNFMultiplicityBoundaries::setMultiplicity(int iBegin, int iEnd)
{
...
void
BNFMultiplicityBoundaries::setMultiplicity(ExprScriptExpression* pBegin,
                                           ExprScriptExpression* pEnd)
{
...
void
BNFMultiplicityBoundaries::computeBoundaries(DtaScriptVariable& visibility,
                                             int& iBegin,
                                             int& iEnd) const
{
  if (bConstantBoundaries) {
    ...
    if (choice.variable._pEnd == NULL)
      ...
      iEnd = choice.variable._pEnd->getIntValue(visibility);
  }
}

std::string
BNFMultiplicityBoundaries::toString() const
{
  std::string sString;
  if (bConstantBoundaries) {
    if (choice.constant._iBegin == 0) {
      if (choice.constant._iEnd < 0)
        sString = "*";
      ...
      if (choice.constant._iBegin == choice.constant._iEnd) {
        ...
        char tcNumber[32];
        sprintf(
          tcNumber, "%d..%d", choice.constant._iBegin, choice.constant._iEnd);
        sString = tcNumber;
      }
    }
  } else {
...
BNFMultiplicity::BNFMultiplicity(DtaBNFScript* pBNFScript,
                                 GrfBlock* pParent,
                                 bool bContinue)
...
void
BNFMultiplicity::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFMultiplicity(*this, env);
}

bool
BNFMultiplicity::isABNFCommand() const
{
  return true;
}

void
BNFMultiplicity::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                     bool bConcat,
                                     BNFClause& theClause)
{
...
BNFMultiplicity::execute(DtaScriptVariable& visibility)
{
  ...
  if (result == NO_INTERRUPTION) {
    ...
    for (i = iEnd - iBegin; i != 0; i--) {
      ...
      /*				if (iLocation == iNewLocation) {
                                              ...
                                              if (iInfiniteLoopCounter > 10)
         throw UtlException("infinite loop expected: the parse cursor hasn't
         move after repeating a sequence 10 times");
                                      }
      */
  ...
  if (result == NO_INTERRUPTION) {
    if (!_listOfConstants.empty()) {
      bool bMatch = false;
      std::string sTextToMatch = CGRuntime::getLastReadChars(
...
void
BNFMultiplicity::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  if (!_boundaries.bConstantBoundaries)
    throw UtlException(
      "BNFMultiplicity::compileCpp(): variable boundaries not handled yet!");
  ...
  if (i != getCommands().end()) {
    ...
    CW_BODY_INDENT << "int _compilerClauseCursor_" << iCursor
    ...
    if (!_listOfConstants.empty() || (_pVariableToAssign != NULL)) {
      ...
      CW_BODY_ENDL;
      sprintf(
        tcNotEmptyLocation, "_compilerClauseNotEmptyLocation_%d", iCursor);
      CW_BODY_INDENT << "int " << tcNotEmptyLocation
                     << " = CGRuntime::getInputLocation();";
      CW_BODY_ENDL;
    }
    if (_boundaries.choice.constant._iEnd != 1) {
      if ((_boundaries.choice.constant._iBegin > 0) ||
          (_boundaries.choice.constant._iEnd > 0)) {
        CW_BODY_INDENT << "int _compilerClause_iteration_" << iCursor
                       << " = 0;";
        CW_BODY_ENDL;
      }
      CW_BODY_INDENT << "do {";
    ...
    if (_boundaries.choice.constant._iEnd != 1) {
      ...
      CW_BODY_ENDL;
      CW_BODY_INDENT
        << "	CGRuntime::setInputLocation(_compilerClauseCursor_" << iCursor
        << ");";
      CW_BODY_ENDL;
      if (_boundaries.choice.constant._iBegin == 0) {
      ...
      CW_BODY_INDENT << "	";
      if (_boundaries.choice.constant._iBegin > 1)
        CW_BODY_STREAM << "if (_compilerClause_iteration_" << iCursor
                       << " >= " << _boundaries.choice.constant._iBegin << ") ";
      CW_BODY_STREAM << "_compilerClauseCursor_" << iCursor
                     << " = CGRuntime::getInputLocation();";
      CW_BODY_ENDL;
      CW_BODY_INDENT << "}";
      CW_BODY_ENDL;
      ...
      if (_boundaries.choice.constant._iEnd > 0)
        CW_BODY_STREAM << " && (_compilerClause_iteration_" << iCursor << " < "
                       << _boundaries.choice.constant._iEnd << ")";
      CW_BODY_STREAM << ");";
      CW_BODY_ENDL;
      if (_boundaries.choice.constant._iBegin > 0) {
        CW_BODY_INDENT
          << "if (!_compilerClauseSuccess && (_compilerClause_iteration_"
          << iCursor << " >= " << _boundaries.choice.constant._iBegin
          << ")) _compilerClauseSuccess = true;";
        CW_BODY_ENDL;
      }
      ...
      CW_BODY_INDENT << "if (!_compilerClauseSuccess) "
                        "CGRuntime::setInputLocation(_compilerClauseCursor_"
                     << iCursor << ");";
      CW_BODY_ENDL;
    }
    if (_bContinue) {
    ...
    if (theCompilerEnvironment.popLastRepeatCursor()) {
      CW_BODY_INDENT << "_compilerRepeatLabel" << iCursor << ":";
      CW_BODY_ENDL;
    }
    if (_pVariableToAssign != NULL) {
      if (!_bContinue) {
...
std::string
BNFMultiplicity::toString() const
{
  std::string sString;
  if (_bContinue)
    sString = "#continue ";
