...
bool BNFPushItem::isABNFCommand() const
{
    return true;
}
...
SEQUENCE_INTERRUPTION_LIST BNFPushItem::execute(DtaScriptVariable& visibility)
{
    ...
    if (bCreation) {
        ...
        if (pVariable->isLocal()) {
            CGRuntime::throwBNFExecutionError("declare the local variable '" + _pVariable->toString() + "' before calling '#pushItem'");
        }
    }
...
void BNFPushItem::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (i != getCommands().end()) {
        ...
        _pVariable->compileCpp(theCompilerEnvironment);
        CW_BODY_STREAM << ".isLocal()) CGRuntime::throwBNFExecutionError(\"declare the local variable '" << _pVariable->toString() << "' before calling '#pushItem'\");";
        CW_BODY_ENDL;
