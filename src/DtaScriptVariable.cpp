...
public:
  inline DtaNodeListeners(DtaScriptVariable* pNode)
    : _pNode(pNode)
  {}
  ~DtaNodeListeners()
  {
    for (std::list<DtaNodeListener*>::iterator i = _listeners.begin();
         i != _listeners.end();
         ++i) {
      (*i)->onDelete();
      delete *i;
...
void
DtaScriptVariableList::pushBack(DtaScriptVariable* pVariable)
{
...
{
  for (std::list<DtaScriptVariable*>::iterator i = items.begin();
       i != items.end();
       i++) {
    delete (*i);
  }
...
DtaScriptVariable::NodeArrayType::NodeArrayType(DtaScriptVariable* pElt)
  : cache(NULL)
{
  ...
  if (_pReferencedByVariables != NULL) {
    while (!_pReferencedByVariables->empty()) {
      ...
      /*
                              ...
                              sErrorMessage += "' that is being removed from the
         stack" + CGRuntime::endl(); sErrorMessage += UtlTrace::getTraceStack();
                              if
         (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
...
void
DtaScriptVariable::addListener(DtaNodeListener* pListener)
{
  if (_pListeners == NULL)
    _pListeners = new DtaNodeListeners(this);
  _pListeners->addListener(pListener);
}

bool
DtaScriptVariable::removeListener(DtaNodeListener* pListener)
{
  if (_pListeners == NULL)
    return false;
...
DtaScriptVariable::getRoot() const
{
  ...
  while ((pParent != NULL) &&
         (strncmp(pParent->_tcName, "##stack##", 9) != 0)) {
    pVariable = pParent;
...
void
DtaScriptVariable::changeName(const char* tcName)
{
...
bool
DtaScriptVariable::isLocal() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
  if (pVariable->_pParent == NULL || pVariable->_pParent->getName() == NULL)
    return false;
...
DtaScriptVariable::detachAttribute(const char* tcName)
{
  ...
  while (pAttributes != NULL) {
    if (strcmp(pAttributes->getNode()->_tcName, tcName) == 0) {
      ...
      if (pLastAttribute == NULL)
        _pAttributes = pAttributes->getNext();
      else
        pLastAttribute->setNext(pAttributes->getNext());
...
const std::list<DtaScriptVariable*>*
DtaScriptVariable::getArray() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
...
int
DtaScriptVariable::getArraySize() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
  if (pVariable->_pArray == NULL)
    return 0;
  return pVariable->_pArray->items.size();
}

const std::map<std::string, DtaScriptVariable*>*
DtaScriptVariable::getSortedArray() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
  ...
  if (pANT->cache == NULL) {
    ...
    for (std::list<DtaScriptVariable*>::const_iterator i = pANT->items.begin();
         i != pANT->items.end();
         i++) {
...
std::auto_ptr<std::vector<DtaScriptVariable*>>
DtaScriptVariable::getSortedNoCaseArray() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
  ...
  if (pANT != NULL) {
    ...
    for (std::map<std::string,
                  std::map<std::string, DtaScriptVariable*>>::const_iterator j =
           mapOfNodes.begin();
         j != mapOfNodes.end();
         ++j) {
      for (std::map<std::string, DtaScriptVariable*>::const_iterator k =
             j->second.begin();
           k != j->second.end();
           ++k) {
...
std::auto_ptr<std::vector<DtaScriptVariable*>>
DtaScriptVariable::getSortedArrayOnValue() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
...
std::auto_ptr<std::vector<DtaScriptVariable*>>
DtaScriptVariable::getSortedNoCaseArrayOnValue() const
{
  const DtaScriptVariable* pVariable = this;
  while (pVariable->getReferencedVariable() != NULL)
    pVariable = pVariable->getReferencedVariable();
...
DtaScriptVariable::getArrayElement(const std::string& sPosition) const
{
  if (getReferencedVariable() != NULL)
    return getReferencedVariable()->getArrayElement(sPosition);
  if (_pArray == NULL)
    return NULL;
  if (_pArray->cache != NULL) {
    std::map<std::string, DtaScriptVariable*>::const_iterator i =
      _pArray->cache->find(sPosition);
    if (i != _pArray->cache->end())
      return i->second;
    ...
    for (std::list<DtaScriptVariable*>::const_iterator i =
         ...
         i != _pArray->items.end();
         i++) {
      if (strcmp((*i)->_tcName, sPosition.c_str()) == 0)
        pVariable = (*i);
...
{
  if (getReferencedVariable() != NULL)
    return getReferencedVariable()->getArrayElement(iPosition);
  if (_pArray == NULL)
    return NULL;
  if (iPosition >= 0) {
    for (std::list<DtaScriptVariable*>::const_iterator i =
...
void
DtaScriptVariable::invertArray()
{
  if (getReferencedVariable() != NULL) {
    getReferencedVariable()->invertArray();
  } else if (_pArray != NULL) {
    std::list<DtaScriptVariable*> array;
    for (std::list<DtaScriptVariable*>::iterator i = _pArray->items.begin();
         i != _pArray->items.end();
         i++) {
      array.push_front(*i);
    }
    _pArray->items = array;
    if (_pListeners != NULL)
      _pListeners->onArrayHasChanged();
  }
}

DtaScriptVariable*
DtaScriptVariable::insertElementAt(const std::string& sProposedKey,
                                   int iPosition)
{
  if (getReferencedVariable() != NULL) {
...
void
DtaScriptVariable::removeArrayElement()
{
  if (getReferencedVariable() != NULL) {
...
void
DtaScriptVariable::removeArrayElement(const char* sPosition)
{
  if (getReferencedVariable() != NULL)
    getReferencedVariable()->removeArrayElement(sPosition);
  else if (_pArray != NULL) {
    for (std::list<DtaScriptVariable*>::iterator i = _pArray->items.begin();
         i != _pArray->items.end();
         i++) {
...
void
DtaScriptVariable::removeArrayElement(int iPosition)
{
  if (getReferencedVariable() != NULL)
    getReferencedVariable()->removeArrayElement(iPosition);
  else if (_pArray != NULL) {
    if (iPosition >= 0) {
      for (std::list<DtaScriptVariable*>::iterator i = _pArray->items.begin();
           i != _pArray->items.end();
...
void
DtaScriptVariable::removeFirstElement()
{
  if (getReferencedVariable() != NULL)
    getReferencedVariable()->removeFirstElement();
  else if (_pArray != NULL) {
    std::list<DtaScriptVariable*>::iterator cursor = _pArray->items.begin();
    if (cursor != _pArray->items.end()) {
      if (_pArray->cache != NULL)
        _pArray->cache->erase((*cursor)->_tcName);
      delete *cursor;
...
void
DtaScriptVariable::removeLastElement()
{
  if (getReferencedVariable() != NULL)
    getReferencedVariable()->removeLastElement();
  else if ((_pArray != NULL) && !_pArray->items.empty()) {
    DtaScriptVariable* pElt = _pArray->items.back();
    if (_pArray->cache != NULL)
      _pArray->cache->erase(pElt->_tcName);
...
std::string
DtaScriptVariable::getCompleteName() const
{
  std::string sName;
  if (_pParent != NULL) {
...
DtaScriptVariable::sortArray()
{
  ...
  for (std::map<std::string, DtaScriptVariable*>::const_iterator j =
         mapOfElts->begin();
       j != mapOfElts->end();
       ++j) {
...
void
DtaScriptVariable::removeReference()
{
  _value.var->_pReferencedByVariables->erase(this);
  _eValueNode = NO_VALUE_NODE;
  if (_pListeners != NULL)
    _pListeners->onSetValue(NULL);
}

void
DtaScriptVariable::addReferencedBy(DtaScriptVariable* pVariable)
{
  if (pVariable != NULL) {
    if (_pReferencedByVariables == NULL)
      _pReferencedByVariables = new std::set<DtaScriptVariable*>();
    _pReferencedByVariables->insert(pVariable);
  }
}

DtaScriptVariable*
DtaScriptVariable::getInternalVariableExtraction(
  DtaScriptVariable& visibility,
  const ExprScriptVariable& exprVariable,
  bool bRecursiveSearch,
  bool bCreateIfUnknown,
  bool bReferencedVariable)
{
  DtaScriptVariable* pVariable = NULL;
  if (exprVariable.isEvaluation()) {
    ...
    pVariable = getInternalVariableExtraction(visibility,
                                              *pExprVariable,
                                              bRecursiveSearch,
                                              bCreateIfUnknown,
                                              bReferencedVariable);
  ...
  if (exprVariable.getPackage() != NULL) {
    ...
    if (pVariable == NULL) {
      bool bExtractVariableWithReference =
        bReferencedVariable || !exprVariable.isAttributeOnly();
      std::string sName = exprVariable.getName();
      pVariable = getVariable(sName.c_str(),
                              bRecursiveSearch,
                              bCreateIfUnknown,
                              bExtractVariableWithReference);
      if (pVariable == NULL)
        return NULL;
    }
  }
  if (exprVariable.isArrayKey()) {
    ...
    if (sPosition == "parent") {
      ...
      if ((pParent != NULL) && (strncmp(pParent->_tcName, "##stack##", 9) != 0))
        pVariable = pParent;
      ...
      int iPosition;
      if (sPosition == "back")
        iPosition = pVariable->getArraySize() - 1;
      else
        iPosition = atoi(sPosition.c_str());
      DtaScriptVariable* pArrayElement = pVariable->getArrayElement(iPosition);
      if (pArrayElement == NULL)
        throw UtlException(
          std::string("Array index is out of bounds on variable '" +
                      exprVariable.toString() + "': \"") +
          sPosition + "\"");
      pVariable = pArrayElement;
      if (bReferencedVariable || exprVariable.isNext()) {
        pVariable = pVariable->getReferencedVariable();
...
DtaScriptVariable*
DtaScriptVariable::getNonRecursiveVariable(
  DtaScriptVariable& visibility,
  const ExprScriptVariable& exprVariable)
{
  return getInternalVariableExtraction(
    visibility, exprVariable, false, true, true);
}

DtaScriptVariable*
DtaScriptVariable::getNonRecursiveNonReferencedVariable(
  DtaScriptVariable& visibility,
  const ExprScriptVariable& exprVariable)
{
  return getInternalVariableExtraction(
    visibility, exprVariable, false, true, false);
}

DtaScriptVariable*
DtaScriptVariable::getVariableForReferenceAssignment(
  const ExprScriptVariable& exprVariable)
{
...
DtaScriptVariable::getExistingVariable(const ExprScriptVariable& exprVariable)
{
  DtaScriptVariable* pVariable =
    getInternalVariableExtraction(*this, exprVariable, true, false, true);
  if ((pVariable == NULL) && Workspace::warningOnUnknownVariables()) {
    std::string sErrorMessage = "warning: variable '" +
                                exprVariable.toString() +
                                "' doesn't exist yet" + CGRuntime::endl();
...
DtaScriptVariable*
DtaScriptVariable::getNoWarningExistingVariable(
  const ExprScriptVariable& exprVariable)
{
...
DtaScriptVariable*
DtaScriptVariable::getVariable(const char* sName,
                               bool bRecursiveSearch,
                               bool bCreateIfUnknown,
                               bool bReferencedVariable)
{
  ...
  while (pAttributes != NULL) {
    ...
    if (pAttributes->getNext() == NULL)
      pLastAttribute = pAttributes;
    pAttributes = pAttributes->getNext();
  }
  if (pAttributes == NULL) {
    if (bRecursiveSearch) {
      //			pVariable = _pParent->getVariable(sName, true,
      //false, bReferencedVariable);
      // we are looking for the variable into local variables, above the
      // function call
      DtaScriptVariable* pLocalScope = NULL;
      bool bClause = false;
      if ((strncmp(_tcName, "##stack## ", 10) == 0) &&
          (strncmp(_tcName + 10, "function", 8) != 0) &&
          (strncmp(_tcName + 10, "clause", 6) != 0)) {
        pLocalScope = getParent();
        while ((pVariable == NULL) && (pLocalScope != NULL) &&
               (pLocalScope->_tcName != NULL) &&
               (strncmp(pLocalScope->_tcName, "##stack##", 9) == 0)) {
          pVariable =
            pLocalScope->getVariable(sName, false, false, bReferencedVariable);
          const char* tcFunction = pLocalScope->_tcName + 10;
          if (strncmp(tcFunction, "function", 8) == 0)
            break;
      ...
      if ((pVariable == NULL) && !bClause) {
        // the variable doesn't belong to the stack, perhaps it is a node of the
        // current subtree
        DtaScriptVariable* pThis = &CGRuntime::getThisInternalNode();
        if (pThis == 0)
          throw UtlException("INTERNAL ERROR: NULL 'this'!");
        pVariable =
          pThis->getVariable(sName, false, false, bReferencedVariable);
        int tiVersion[] = { 1, 13, -1 };
        if ((pVariable == NULL) && (pLocalScope != NULL) &&
            DtaProject::getInstance().isScriptVersionOlderThan(tiVersion)) {
          // this is an old version of scripts, so variable may be stored
          // into the stack, above the function call
          while ((pVariable == NULL) && (pLocalScope->_tcName != NULL) &&
                 (strncmp(pLocalScope->_tcName, "##stack##", 9) == 0)) {
            pVariable = pLocalScope->getVariable(
              sName, false, false, bReferencedVariable);
            pLocalScope = pLocalScope->getParent();
          }
          if (pVariable != NULL) {
            std::string sErrorMessage =
              "since version 1.13, the scope of a local variable is restricted "
              "to the function it is declared into, so a change of behaviour "
              "may occur when looking at local variable '";
            sErrorMessage += sName;
            sErrorMessage += "'" + UtlTrace::getTraceStack();
            if (DtaProject::getInstance().addWarning(sErrorMessage) == 1)
              CGRuntime::traceLine(sErrorMessage);
    ...
    if (pVariable == NULL) {
      if (bRecursiveSearch)
        pVariable = DtaProject::getInstance().getGlobalVariable(sName);
      if ((pVariable == NULL) && bCreateIfUnknown) {
        pVariable = new DtaScriptVariable(this, sName);
        if (_pAttributes == NULL)
          _pAttributes = new DtaScriptVariableList(pVariable);
        else
          pLastAttribute->pushBack(pVariable);
        if (_pListeners != NULL)
          _pListeners->onAddAttribute(pVariable);
      }
    }
  }
  return pVariable;
...
DtaScriptVariable::getEvaluatedNode(const std::string& sDynamicVariable)
{
  ...
  std::auto_ptr<ExprScriptVariable> pExprVariable(
    script.parseVariableExpression(script.getBlock(), theCommand));
  DtaScriptVariable* pVariable =
    getInternalVariableExtraction(*this,
                                  *pExprVariable,
                                  true /*bRecursiveSearch*/,
                                  false /*bCreateIfUnknown*/,
                                  true /*bReferencedVariable*/);
  return pVariable;
}

DtaScriptVariable*
DtaScriptVariable::getOrCreateLocalEvaluatedNode(
...
DtaScriptVariable*
DtaScriptVariable::insertClassicalEvaluatedNode(
  const std::string& sDynamicVariable)
{
  ScpStream theCommand(sDynamicVariable);
  DtaScript script;
...
DtaScriptVariable*
DtaScriptVariable::createNodeArgument(const char* sName,
                                      DtaScriptVariable* pParameter)
{
  ...
  if (pAttributes == NULL) {
    ...
    if (_pAttributes == NULL)
      _pAttributes = new DtaScriptVariableList(pIndex);
    else
      pLastAttribute->pushBack(pIndex);
    if (_pListeners != NULL)
      _pListeners->onAddAttribute(pIndex);
  }
...
DtaScriptVariable::getNodeArgument(const char* sName) const
{
  ...
  if (pAttributes == NULL) {
    if (_pParent != NULL)
      pIndex = _pParent->getNodeArgument(sName);
    if (pIndex == NULL) {
      throw UtlException(std::string("unable to find argument called '") +
                         sName + "'");
    }
  }
  return pIndex;
...
DtaScriptVariable*
DtaScriptVariable::createIterator(const char* sName,
                                  DtaArrayIterator** pIteratorData)
{
  DtaScriptVariable* pIndex = NULL;
  DtaScriptVariableList* pAttributes = _pAttributes;
  DtaScriptVariableList* pLastAttribute = _pAttributes;
...
DtaScriptVariable::getIterator(const char* sName) const
{
  ...
  if (pAttributes == NULL) {
    if (_pParent != NULL)
      pIndex = _pParent->getIterator(sName);
  }
  if (pIndex != NULL) {
    while (pIndex->_eValueNode == REFERENCE_VALUE_NODE)
      pIndex = pIndex->_value.var;
    if (pIndex->_eValueNode != INDEX_VALUE_NODE)
      return NULL;
  }
  return pIndex;
}
...
DtaArrayIterator*
DtaScriptVariable::getIteratorData() const
{
  if (_eValueNode != INDEX_VALUE_NODE)
    return NULL;
...
DtaScriptVariable::addElement(const std::string& sName)
{
  ...
  if (pVariable == NULL)
    pVariable = addInternalElement(sName);
  return pVariable;
}
...
DtaScriptVariable::addInternalElement(const std::string& sName)
{
  ...
  if (_pArray == NULL)
    _pArray = new NodeArrayType(pVariable);
  else {
    _pArray->items.push_back(pVariable);
    if (_pArray->cache != NULL)
      (*_pArray->cache)[sName] = pVariable;
  }
  if (_pListeners != NULL)
    _pListeners->onArrayHasChanged();
  return pVariable;
...
void
DtaScriptVariable::setValueAtVariable(const char* sVariable,
                                      const char* sValue,
                                      bool bRecursiveSearch,
                                      bool bCreateIfUnknown)
{
...
const char*
DtaScriptVariable::getValue() const
{
  if (_eValueNode == STRING_VALUE_NODE)
    return _value.str.text;
  if (_eValueNode == REFERENCE_VALUE_NODE)
    return _value.var->getValue();
  if (_eValueNode == EXTERNAL_VALUE_NODE)
    return _value.ext->getValue();
  if (_eValueNode == INDEX_VALUE_NODE)
    return (*_value.ind.var)->item()->getValue();
  return NULL;
}

size_t
DtaScriptVariable::getValueLength() const
{
  if (_eValueNode == STRING_VALUE_NODE)
    return _value.str.length;
  if (_eValueNode == REFERENCE_VALUE_NODE)
    return _value.var->getValueLength();
  if (_eValueNode == EXTERNAL_VALUE_NODE)
    return _value.ext->getValueLength();
  if (_eValueNode == INDEX_VALUE_NODE)
    return (*_value.ind.var)->item()->getValueLength();
  return 0;
}

double
DtaScriptVariable::getDoubleValue() const
{
  if (_eValueNode == STRING_VALUE_NODE)
    return atof(_value.str.text);
  if (_eValueNode == REFERENCE_VALUE_NODE)
    return _value.var->getDoubleValue();
  if (_eValueNode == EXTERNAL_VALUE_NODE)
    return _value.ext->getDoubleValue();
  if (_eValueNode == INDEX_VALUE_NODE)
    return (*_value.ind.var)->item()->getDoubleValue();
  return 0.0;
}

bool
DtaScriptVariable::getBooleanValue() const
{
  if (_eValueNode == STRING_VALUE_NODE)
    return (_value.str.length != 0);
  if (_eValueNode == REFERENCE_VALUE_NODE)
    return _value.var->getBooleanValue();
  if (_eValueNode == EXTERNAL_VALUE_NODE)
    return _value.ext->getBooleanValue();
  if (_eValueNode == INDEX_VALUE_NODE)
    return (*_value.ind.var)->item()->getBooleanValue();
  return false;
}

int
DtaScriptVariable::getIntValue() const
{
  if (_eValueNode == STRING_VALUE_NODE)
    return atoi(_value.str.text);
  if (_eValueNode == REFERENCE_VALUE_NODE)
    return _value.var->getIntValue();
  if (_eValueNode == EXTERNAL_VALUE_NODE)
    return _value.ext->getIntValue();
  if (_eValueNode == INDEX_VALUE_NODE)
    return (*_value.ind.var)->item()->getIntValue();
  return 0;
}

void
DtaScriptVariable::clearValue()
{
  if (_eValueNode == REFERENCE_VALUE_NODE) {
    removeReference();
...
void
DtaScriptVariable::setValue(const char* tcValue)
{
  if (_pListeners != NULL)
    _pListeners->onSetValue(tcValue);
...
void
DtaScriptVariable::setValue(bool bValue)
{
  setValue(((bValue) ? "true" : NULL));
}

void
DtaScriptVariable::setValue(double dValue)
{
...
void
DtaScriptVariable::setValue(int iValue)
{
  char tcNumber[16];
...
void
DtaScriptVariable::setValue(const std::list<std::string>& listOfValues)
{
  int iIndex = 0;
  char sNumber[16];
  clearArray();
  for (std::list<std::string>::const_iterator i = listOfValues.begin();
       i != listOfValues.end();
       i++) {
...
void
DtaScriptVariable::setValue(DtaScriptVariable* pVariable)
{
  if (pVariable != NULL) {
    if (pVariable == this)
      throw UtlException("DtaScriptVariable::setValue() cyclic reference");
  ...
  if (pVariable != NULL) {
    ...
    if (_pListeners != NULL)
      _pListeners->onSetReference(pVariable);
  }
}

void
DtaScriptVariable::setValue(ExternalValueNode* pExternalValue)
{
  if ((_eValueNode == EXTERNAL_VALUE_NODE) && (pExternalValue == _value.ext))
    return;
  clearValue();
  _eValueNode = EXTERNAL_VALUE_NODE;
  _value.ext = pExternalValue;
  if (_pListeners != NULL)
    _pListeners->onSetExternal(pExternalValue);
}

void
DtaScriptVariable::setValue(DtaArrayIterator* pIteratorData)
{
...
void
DtaScriptVariable::setIteratorData(DtaArrayIterator** pIteratorData)
{
  clearValue();
  ...
  if (_pListeners != NULL)
    _pListeners->onSetIterator(pIteratorData);
}

void
DtaScriptVariable::concatValue(const char* tcValue)
{
  if (_pListeners != NULL)
    _pListeners->onConcatValue(tcValue);
  if ((tcValue != NULL) && (*tcValue != '\0')) {
    size_t iLength = strlen(tcValue);
    if (_eValueNode == STRING_VALUE_NODE) {
...
void
DtaScriptVariable::remove(DtaScriptVariable* pVariable)
{
  if (getReferencedVariable() != NULL)
    getReferencedVariable()->remove(pVariable);
  else {
    if (_pAttributes != NULL) {
      if (_pAttributes->getNode() == pVariable) {
        DtaScriptVariableList* pNext = _pAttributes->getNext();
        if (_pListeners != NULL)
          _pListeners->onRemoveAttribute(pVariable);
        delete pVariable;
...
void
DtaScriptVariable::remove(const char* sAttribute)
{
  if (_pAttributes != NULL) {
    if (strcmp(_pAttributes->getNode()->_tcName, sAttribute) == 0) {
      if (_pListeners != NULL)
        _pListeners->onRemoveAttribute(_pAttributes->getNode());
      DtaScriptVariableList* pNext = _pAttributes->getNext();
      ...
      while (pNext != NULL) {
        if (strcmp(pNext->getNode()->_tcName, sAttribute) == 0) {
          if (_pListeners != NULL)
            _pListeners->onRemoveAttribute(pNext->getNode());
          pAttributes->setNext(pNext->getNext());
...
void
DtaScriptVariable::removeRecursive(const char* sAttribute)
{
  remove(sAttribute);
  if (_pAttributes != NULL) {
    DtaScriptVariableList* pAttributes = _pAttributes;
  ...
  if (_pArray != NULL) {
    for (std::list<DtaScriptVariable*>::iterator j = _pArray->items.begin();
         j != _pArray->items.end();
         j++) {
      if ((*j)->getParent() == this)
        (*j)->removeRecursive(sAttribute);
    }
  }
}

void
DtaScriptVariable::clearContent()
{
  clearValue();
  ...
  clearArray();
  if (_pListeners != NULL)
    _pListeners->onClearContent();
}

void
DtaScriptVariable::addArrayElements(const DtaScriptVariable& variable)
{
  NodeArrayType* pANT = variable._pArray;
  if ((pANT != NULL) && !pANT->items.empty()) {
    if (_pArray == NULL)
      _pArray = new NodeArrayType;
    for (std::list<DtaScriptVariable*>::const_iterator j = pANT->items.begin();
         j != pANT->items.end();
         j++) {
      ...
      _pArray->items.push_back(pVariable);
      if (_pArray->cache != NULL)
        (*_pArray->cache)[pVariable->_tcName] = pVariable;
    }
    if (_pListeners != NULL)
      _pListeners->onArrayHasChanged();
  }
...
int
DtaScriptVariable::getNumberOfAttributes() const
{
  int iSize = 0;
...
bool
DtaScriptVariable::equal(const DtaScriptVariable& variable)
{
  if (_pArray == NULL) {
    if (variable._pArray != NULL) {
      if (!variable._pArray->items.empty())
        return false;
    }
  } else if (variable._pArray == NULL) {
    if (!_pArray->items.empty())
      return false;
    delete _pArray;
    _pArray = NULL;
  } else if (_pArray->items.size() != variable._pArray->items.size())
    return false;
  if (_eValueNode != variable._eValueNode)
    return false;
  if (_eValueNode == REFERENCE_VALUE_NODE) {
    if (_value.var != variable._value.var)
      return false;
  } else if (_eValueNode == INDEX_VALUE_NODE) {
    if ((*_value.ind.var)->item() != (*variable._value.ind.var)->item())
      return false;
  } else if (_eValueNode == STRING_VALUE_NODE) {
    if (strcmp(_value.str.text, variable._value.str.text) != 0)
      return false;
  } else if (_eValueNode == EXTERNAL_VALUE_NODE) {
    if (_value.ext != variable._value.ext)
      return false;
  }
  DtaScriptVariableList* pAttributes = _pAttributes;
  if (getNumberOfAttributes() != variable.getNumberOfAttributes())
    return false;
  while (pAttributes != NULL) {
    DtaScriptVariableList* pVariableAttributes = variable._pAttributes;
    DtaScriptVariable* pVariable = pAttributes->getNode();
    while (pVariableAttributes != NULL) {
      if (strcmp(pVariable->_tcName, pVariableAttributes->getNode()->_tcName) ==
          0) {
        if (!pVariable->equal(*(pVariableAttributes->getNode())))
          return false;
        break;
      }
      pVariableAttributes = pVariableAttributes->getNext();
    }
    if (pVariableAttributes == NULL)
      return false;
    pAttributes = pAttributes->getNext();
  }
  ...
  if (_pArray != NULL) {
    if (getArraySize() != variable.getArraySize())
      return false;
    for (i = _pArray->items.begin(); i != _pArray->items.end(); i++) {
      for (j = variable._pArray->items.begin();
           j != variable._pArray->items.end();
           ++j) {
      ...
      if (j == variable._pArray->items.end())
        return false;
    }
  }
  return true;
...
void
DtaScriptVariable::copyAll(const DtaScriptVariable& variable, bool bMerge)
{
  if (!bMerge)
    clearContent();
  ...
  while (pVariableAttributes != NULL) {
    ...
    } else if (pVariableAttributes->getNode()->_eValueNode ==
               INDEX_VALUE_NODE) {
      pVariable->_eValueNode = REFERENCE_VALUE_NODE;
      pVariable->_value.var =
        (*pVariableAttributes->getNode()->_value.ind.var)->item();
      pVariable->_value.var->addReferencedBy(pVariable);
    } else {
  ...
  if ((pANT != NULL) && !pANT->items.empty()) {
    ...
    for (std::list<DtaScriptVariable*>::const_iterator i = pANT->items.begin();
      ...
      std::map<std::string, DtaScriptVariable*>::const_iterator cursor =
        _pArray->cache->find(tcName);
      DtaScriptVariable* pVariable = NULL;
...
void
DtaScriptVariable::slideNodeContent(DtaScriptVariable& visibility,
                                    const ExprScriptVariable& varExpr)
{
  DtaScriptVariable* pTemporary = new DtaScriptVariable;
  pTemporary->copyAll(*this);
  clearContent();
  DtaScriptVariable* pDestVariable =
    getNonRecursiveVariable(visibility, varExpr);
  ...
  if (pParentAttributes == NULL) {
    ...
    if (pANT != NULL) {
      ...
      if (i == pANT->items.end()) {
        delete pTemporary;
        throw UtlException(
          "internal error: unable to slide a node content because destination "
          "node isn't attached to its parent properly");
      }
    } else {
      delete pTemporary;
      throw UtlException(
...
bool
DtaScriptVariable::traceValue(int iDepth,
                              const std::string& sIndent,
                              const std::string& sPrefix)
{
  if (_eValueNode == STRING_VALUE_NODE) {
    if (_value.str.length != 0)
      CGRuntime::traceLine(sPrefix + "\"" + std::string(_value.str.text) +
                           "\"");
    else
      return false;
...
void
DtaScriptVariable::traceObject(int iDepth, const std::string& sIndent)
{
  if (iDepth < 0)
    return;
  ...
  if (sIndent.empty()) {
    char tcMessage[64];
    if (iDepth > 0)
      sprintf(tcMessage, "' to depth %d:", iDepth);
    else
      strcpy(tcMessage, "':");
    CGRuntime::traceLine("Tracing variable '" + getCompleteName() + tcMessage);
    if (_eValueNode == INDEX_VALUE_NODE)
      traceValue(iDepth, sNextIndent, "\t");
    else
      traceValue(iDepth - 1, sNextIndent, "\t");
    ...
    if (iDepth > 0) {
      ...
      if ((pANT != NULL) && !pANT->items.empty()) {
        CGRuntime::traceText(sCurrentIndent + std::string(pNode->_tcName) +
                             "[");
        for (std::list<DtaScriptVariable*>::iterator j = pANT->items.begin();
             j != pANT->items.end();
             j++) {
          if (j != pANT->items.begin())
            CGRuntime::traceText(", ");
  ...
  if ((_pArray != NULL) && !_pArray->items.empty()) {
    if (iDepth <= 0) {
      ...
      for (std::list<DtaScriptVariable*>::iterator k = _pArray->items.begin();
           k != _pArray->items.end();
           k++) {
        if (k != _pArray->items.begin())
          CGRuntime::traceText(", ");
        CGRuntime::traceText("\"" + std::string((*k)->_tcName) + "\"");
        const char* tcValue = (*k)->getValue();
        if ((tcValue != NULL) && (*tcValue != '\0'))
          CGRuntime::traceText(" -> \"" + std::string(tcValue) + "\"");
      }
      CGRuntime::traceLine("]");
    } else {
      for (std::list<DtaScriptVariable*>::iterator k = _pArray->items.begin();
           ...
           k++) {
        CGRuntime::traceText(sCurrentIndent + "[\"" +
                             std::string((*k)->_tcName) + "\"]");
        if (!(*k)->traceValue(iDepth - 1, sCurrentIndent, " = "))
          CGRuntime::traceLine("");
...
void
DtaScriptVariable::traceStack()
{
  if (strcmp(_tcName, "project") == 0)
    return;
  if (_pParent != 0)
    _pParent->traceStack();
  ...
  while (pAttributes != NULL) {
    ...
    if (pNode->getReferencedVariable() != NULL) {
      std::string sReference =
        pNode->getReferencedVariable()->getCompleteName();
      CGRuntime::traceLine(std::string(pNode->_tcName) + " = " + sReference);
    } else {
      const char* tcValue = pNode->getValue();
      if (tcValue != NULL) {
        CGRuntime::traceLine(std::string(pNode->_tcName) + " = \"" +
                             std::string(tcValue) + "\"");
      } else {
