...
void
DtaBreakpoint::setCondition(const std::string& sCondition)
{
...
void
DtaBreakpoint::setAction(const std::string& sAction)
{
  _sAction = sAction;
  delete _pAction;
  _pAction = NULL;
...
bool
DtaBreakpoint::executeCondition(GrfCommand* pCommand,
                                DtaScriptVariable& visibility)
{
  if (_sCondition.empty())
    return true;
  if (_pCondition == NULL) {
    ...
    try {
      _pCondition =
        pCommand->getParent()->getScript()->parseExpression(*pBlock, stream);
    } catch (UtlException& exception) {
      CGRuntime::printDebugLine("in 'when' clause of a breakpoint: " +
                                _sCondition);
    ...
    if (stream.skipEmpty()) {
      if (stream.peekChar() != '{') {
        ...
        CGRuntime::printDebugLine(
          "syntax error at the end of the 'when' clause of a breakpoint: " +
          _sCondition);
        return true;
      }
...
void
DtaBreakpoint::executeAction(GrfCommand* pCommand,
                             DtaScriptVariable& visibility)
{
  if (_sAction.empty())
    return;
...
public:
  inline DtaWatchNodeListener(const std::string& sKey,
                              GrfDebugExecution* pDebugContext)
    : _sKey(sKey)
  ...
  {
    return (_pBP == NULL) ? "" : _pBP->getCondition();
  }

  void setCondition(const std::string& sCondition)
  {
    if (_pBP == NULL)
      _pBP = new DtaBreakpoint;
  ...
  virtual void onSetExternal(ExternalValueNode* pExternalValue)
  {
    onHasChanged();
  }
  virtual void onSetIterator(DtaArrayIterator** pIteratorData)
  {
    onHasChanged();
  }
  virtual void onAddAttribute(DtaScriptVariable* pAttribute) { onHasChanged(); }
  virtual void onRemoveAttribute(DtaScriptVariable* pAttribute)
  {
    onHasChanged();
  }
...
private:
  ...
  {
    if (!_bDeleted) {
      CGRuntime::printDebugLine("debug: the variable '" +
                                getNode()->getCompleteName() + "' has changed");
      if (_pBP == NULL)
        _pDebugContext->_bStop = true;
      else
        _pDebugContext->_pConditionalStop = _pBP;
    }
  }
};
...
public:
  ...
  ~GrfDebugActivation() { _pActiveDebugger = _pPreviousActiveDebugger; }

  static void unactivateCurrentDebugger() { _pActiveDebugger = NULL; }
  static bool isActiveDebugger(GrfDebugExecution* pDebugger)
  {
    return (_pActiveDebugger == pDebugger);
  }
};
...
void
GrfDebugExecution::clearBreakpoints()
{
  for (std::map<std::string, std::map<int, DtaBreakpoint*>>::iterator i =
         _listOfBreakpoints.begin();
       i != _listOfBreakpoints.end();
       ++i) {
    for (std::map<int, DtaBreakpoint*>::iterator j = i->second.begin();
         j != i->second.end();
         ++j) {
      delete j->second;
    }
...
void
GrfDebugExecution::clearWatchpoints()
{
...
void
GrfDebugExecution::clearWatchpoint(const std::string& sKey)
{
  for (std::list<DtaWatchNodeListener*>::iterator i = _watchListeners.begin();
       i != _watchListeners.end();
       ++i) {
    if ((*i)->getKey() == sKey) {
...
bool
GrfDebugExecution::executeCommand(GrfCommand* pCommand,
                                  DtaScriptVariable& visibility,
                                  ScpStream& stream)
{
  if (!stream.skipEmpty())
  ...
  if (sIdentifier.empty()) {
    if (stream.isEqualTo('?')) {
      ...
      try {
        ...
        DtaPatternScript* pPatternScript =
          dynamic_cast<DtaPatternScript*>(pScript);
    ...
    if (stream.readInt(_iIterations)) {
      if (_iIterations <= 0) {
        CGRuntime::printDebugLine(
          "debug: 'step' cannot require a negative number of iterations");
        return false;
      }
    } else
      _iIterations = 1;
    ...
    if (stream.readInt(_iIterations)) {
      if (_iIterations <= 0) {
        CGRuntime::printDebugLine(
          "debug: 'next' cannot require a negative number of iterations");
        return false;
      }
    } else
      _iIterations = 1;
    ...
    if (!stream.readString(sFilename)) {
      if (stream.isEqualToIdentifier("list") || !stream.skipEmpty()) {
        ...
        for (std::map<std::string,
                      std::map<int, DtaBreakpoint*>>::const_iterator i =
               _listOfBreakpoints.begin();
             i != _listOfBreakpoints.end();
             ++i) {
          for (std::map<int, DtaBreakpoint*>::const_iterator j =
                 i->second.begin();
               j != i->second.end();
               ++j) {
            ...
            if (sFilename.size() > sCurrentDirectory.size()) {
              ...
              if (stricmp(sPath.c_str(), sCurrentDirectory.c_str()) == 0) {
                if ((sFilename[iSize] == '/') || (sFilename[iSize] == '\\'))
                  ++iSize;
            ...
            if (!j->second->getCondition().empty()) {
              CGRuntime::printDebugLine("\t\twhen " +
                                        j->second->getCondition());
            }
          }
        }
    ...
    if ((!stream.readInt(iLineNumber)) || (iLineNumber <= 0)) {
      CGRuntime::printDebugLine(
        "debug: positive line number expected after 'at'");
      return false;
    }
    stream.skipEmpty();
    DtaBreakpoint* pBP =
      registerBreakpoint(visibility, sFilename.c_str(), iLineNumber);
    ...
    if (stream.readString(sFilename)) {
      ...
      for (std::map<std::string, std::map<int, DtaBreakpoint*>>::const_iterator
             i = _listOfBreakpoints.begin();
           i != _listOfBreakpoints.end();
           ++i) {
        for (std::map<int, DtaBreakpoint*>::const_iterator j =
               i->second.begin();
      ...
      CGRuntime::printDebugLine("debug: bad breakpoint rank; please check its "
                                "value by typing 'b list'");
      return false;
    } else {
      try {
        GrfBlock* pBlock = pCommand->getParent();
        std::auto_ptr<ExprScriptVariable> pVarExpr(
          pCommand->getParent()->getScript()->parseVariableExpression(*pBlock,
                                                                      stream));
    ...
    stream.skipEmpty();
    int iDisplaySize;
    if (stream.readInt(iDisplaySize) && (iDisplaySize >= 0))
      _iDisplaySize = iDisplaySize;
    display(pCommand, visibility, _iDisplaySize);
  } else if ((sIdentifier == "fl") || (sIdentifier == "fs") ||
             (sIdentifier == "file")) {
    stream.skipEmpty();
    if (sIdentifier == "file") {
    ...
    if (!stream.readString(sFile)) {
      CGRuntime::printDebugLine(
        "debug: file name between double quotes expected");
      return false;
    }
    if (!_bReadingHistory) {
      if (sIdentifier == "fl") {
    ...
    DtaScript* pScript = pCommand->getParent()->getScript();
    ExprScriptVariable* pVariableExpr =
      pScript->parseVariableExpression(*pCommand->getParent(), stream);
    stream.skipEmpty();
    int iDepth;
    if (!stream.readInt(iDepth))
      iDepth = 0;
    if (pVariableExpr == NULL)
      CGRuntime::printDebugLine("debug: syntax error on variable");
    else {
      DtaScriptVariable* pVariable =
        visibility.getExistingVariable(*pVariableExpr);
      if (pVariable == NULL) {
    ...
    std::auto_ptr<ExprScriptExpression> pExpr(
      pScript->parseExpression(*pCommand->getParent(), stream));
    if (pExpr.get() == NULL)
      CGRuntime::printDebugLine("debug: syntax error on expression");
    else {
    ...
    if (stream.skipEmpty()) {
      for (std::list<DtaWatchNodeListener*>::const_iterator i =
             _watchListeners.begin();
           i != _watchListeners.end();
           ++i) {
        DtaWatchNodeListener* pListener = *i;
      ...
      try {
        ...
        std::auto_ptr<ExprScriptVariable> pVarExpr(
          pCommand->getParent()->getScript()->parseVariableExpression(*pBlock,
                                                                      stream));
        DtaScriptVariable* pVariable =
          visibility.getExistingVariable(*pVarExpr);
        if (pVariable == NULL) {
        ...
        if (stream.isEqualToIdentifier("when")) {
          std::string sCondition =
            stream.readBuffer() + stream.getInputLocation();
          pListener->setCondition(sCondition);
        } else if (stream.peekChar() == '{') {
          std::string sAction = stream.readBuffer() + stream.getInputLocation();
          if (!sAction.empty())
            pListener->setAction(sAction);
        }
      } catch (UtlException& exception) {
        CGRuntime::printDebugLine(exception.getMessage());
        CGRuntime::printDebugText(exception.getTraceStack());
...
finallyExecuteCommand:
  if (!_bReadingHistory)
    _history << stream.readBuffer() << '\n';
  return bContinue;
}

void
GrfDebugExecution::help()
{
  CGRuntime::printDebugLine("Debugger");
  CGRuntime::printDebugLine("  Commands available:");
  CGRuntime::printDebugLine("    - 'b' [<filename>] 'at' <line> [<when>] "
                            "[<action>] (or 'breakpoint'):");
  CGRuntime::printDebugLine(
    "        sets a breakpoint into file <filename> at line <line>");
  CGRuntime::printDebugLine("        - <when> ::= 'when' <boolean_expression>");
  CGRuntime::printDebugLine(
    "            conditional breakpoint; the expression is in CW script");
  CGRuntime::printDebugLine("        - <action> ::= '{' <instructions> '}'");
  CGRuntime::printDebugLine(
    "            a piece of CW script to execute when activating the bp");
  CGRuntime::printDebugLine("    - 'b' ['list'] (or 'breakpoint'):");
  CGRuntime::printDebugLine(
    "        displays all breakpoints and their condition, if any");
  CGRuntime::printDebugLine(
    "    - 'c' [<filename> ['at' <line>]] (or 'clear'):");
  CGRuntime::printDebugLine(
    "        1. clears a breakpoint into file <filename> at line <line>");
  CGRuntime::printDebugLine(
    "        2. or clears all breakpoints into file <filename>");
  CGRuntime::printDebugLine("    - 'c' <rank> (or 'clear'):");
  CGRuntime::printDebugLine(
    "        clears the breakpoint designated by its line number in the list");
  CGRuntime::printDebugLine("    - 'c' <variable> (or 'clear'):");
  CGRuntime::printDebugLine(
    "        clears the watchpoint on <variable> if any");
  CGRuntime::printDebugLine("    - 'c' 'all' (or 'clear all'):");
  CGRuntime::printDebugLine(
    "        clears all breakpoints and all watchpoints");
  CGRuntime::printDebugLine("    - 'd' [<size>] or 'display' [<size>]:");
  CGRuntime::printDebugLine(
    "        current line is displayed with <size> lines above and below");
  CGRuntime::printDebugLine(
    "        If size isn't specified, the last known is taken");
  CGRuntime::printDebugLine("    - 'fl' \"<filename>\" (or 'file load'):");
  CGRuntime::printDebugLine(
    "        loads a file containing debugger commands to execute");
  CGRuntime::printDebugLine("    - 'fs' \"<filename>\" (or 'file save'):");
  CGRuntime::printDebugLine(
    "        saves the debugger commands executed up to now to a file");
  CGRuntime::printDebugLine("    - 'h' or 'help' or '?': help");
  CGRuntime::printDebugLine(
    "    - 'l' or 'local': displays local variables on the stack");
  CGRuntime::printDebugLine("    - 'n' or 'next': next");
  CGRuntime::printDebugLine("    - 'o' or 'object' <variable> [<depth>]:");
  CGRuntime::printDebugLine("        displays the content of a variable, "
                            "eventually up to a given depth");
  ...
  CGRuntime::printDebugLine(
    "    - 't' or 'trace' <expression>: displays result of an expression");
  CGRuntime::printDebugLine(
    "    - 'w' <variable> [<when>] [<action>] (or 'watch'):");
  CGRuntime::printDebugLine(
    "        the controlling sequence stops when <variable> changes");
  CGRuntime::printDebugLine("        - <when> ::= 'when' <boolean_expression>");
  CGRuntime::printDebugLine(
    "            conditional watchpoint; the expression is in CW script");
  CGRuntime::printDebugLine("        - <action> ::= '{' <instructions> '}'");
  CGRuntime::printDebugLine(
    "            a piece of CW script to execute at the watchpoint");
  CGRuntime::printDebugLine("    - 'w' or 'watch':");
  CGRuntime::printDebugLine(
    "        displays all watchpoints and their condition, if any");
  CGRuntime::printDebugLine(
    "    - '{' <instructions> '}': a piece of CW script to execute immediatly");
  CGRuntime::printDebugLine("    - '//' or '/*'...'*/': comment");
}

std::string
GrfDebugExecution::display(GrfCommand* pCommand,
                           DtaScriptVariable& visibility,
                           int iSize,
                           bool bEchoOn)
{
  ...
  if (CGRuntime::getInputStream() != NULL) {
    sDisplayedText =
      "parsed file is \"" + CGRuntime::getInputStream()->getFilename() + "\":";
    char sLocation[80];
    sprintf(sLocation,
            "%d,%d\n",
  ...
  if (tcParsingFile == NULL) {
    sDisplayedText +=
      "no debug information on the current command for displaying";
  } else {
    std::ifstream* pFile =
      openInputFileFromIncludePath(tcParsingFile, sCompleteFileName);
    if (pFile == NULL) {
  ...
  if (bEchoOn && !sDisplayedText.empty())
    CGRuntime::printDebugText(sDisplayedText);
  return sDisplayedText;
}

void
GrfDebugExecution::traceStack(GrfCommand* pCommand,
                              DtaScriptVariable& visibility)
{
  display(pCommand, visibility, 0);
  std::string sPreviousText;
  for (std::list<GrfCommand*>::iterator i = _stack.begin(); i != _stack.end();
       i++) {
    if (*i != NULL) {
      std::string sText = display(*i, visibility, 0, false);
      if (sPreviousText != sText)
        CGRuntime::printDebugText(sText);
      sPreviousText = sText;
    }
  }
}
...
DtaBreakpoint*
GrfDebugExecution::registerBreakpoint(DtaScriptVariable& visibility,
                                      ...
                                      int iLineNumber)
{
  ...
  if (_listOfBreakpoints.find(sFilename) == _listOfBreakpoints.end()) {
    std::ifstream* pFile =
      openInputFileFromIncludePath(sFilename, sCompleteFileName);
    if (pFile == NULL) {
      CGRuntime::printDebugLine("debug: unable to open file \"" +
                                std::string(sFilename) + "\" for breakpoint");
      return NULL;
    }
    pFile->close();
    delete pFile;
  ...
  if (bpList.find(iLineNumber) == bpList.end()) {
    ...
    if (iBeginning >= iEnd) {
      ...
      CGRuntime::printDebugLine("debug: unable to set a breakpoint at line " +
                                std::string(tcNumber));
      return NULL;
    }
...
void
GrfDebugExecution::clearBreakpoint(DtaScriptVariable& /*visibility*/,
                                   const char* sFilename,
                                   int iLineNumber)
{
  if ((sFilename == NULL) || (*sFilename == '\0')) {
    ...
    if (_listOfBreakpoints.find(sFilename) == _listOfBreakpoints.end()) {
      CGRuntime::printDebugLine("debug: no breakpoint on file \"" +
                                std::string(sFilename) + "\"");
      ...
      if (bpList.find(iLineNumber) == bpList.end()) {
        char tcNumber[32];
        sprintf(tcNumber, "%d", iLineNumber);
        CGRuntime::printDebugLine("debug: no breakpoint at line " +
                                  std::string(tcNumber) + " into file \"" +
                                  std::string(sFilename) + "\"");
      } else {
...
DtaBreakpoint*
GrfDebugExecution::stopOnBreakpoint(GrfCommand* pCommand,
                                    DtaScriptVariable& visibility)
{
  const char* sFilename = getParsingFilePtr(pCommand);
  int iCharLocation = getFileLocation(pCommand);
  std::map<std::string, std::map<int, DtaBreakpoint*>>::const_iterator cursor =
    _listOfBreakpoints.find(sFilename);
  if (cursor == _listOfBreakpoints.end())
    return NULL;
  const std::map<int, DtaBreakpoint*>& bpList = cursor->second;
  for (std::map<int, DtaBreakpoint*>::const_iterator i = bpList.begin();
       i != bpList.end();
       i++) {
    if (i->second->_iEnd > iCharLocation) {
      if (iCharLocation < i->second->_iBeginning)
        return NULL;
      if (i->second->executeCondition(pCommand, visibility))
        return i->second;
      break;
    }
  }
...
void
GrfDebugExecution::handleBeforeExecutionCBK(GrfCommand* pCommand,
                                            DtaScriptVariable& visibility)
{
  if (!GrfDebugActivation::isActiveDebugger(this))
    return;
  if (getParsingFilePtr(pCommand) != NULL) {
    ...
    if (_pConditionalStop != NULL) {
      if (_pConditionalStop->executeCondition(pCommand, visibility))
        pBP = _pConditionalStop;
      else
        pBP = stopOnBreakpoint(pCommand, visibility);
...
void
GrfDebugExecution::handleAfterExecutionCBK(GrfCommand* pCommand,
                                           DtaScriptVariable& /*visibility*/)
{
  if (_pNext == pCommand) {
    _bStop = true;
...
void
GrfDebugExecution::handleAfterExceptionCBK(GrfCommand* pCommand,
                                           DtaScriptVariable& visibility,
...
GrfDebugExecution::openSession(DtaScriptVariable& visibility)
{
  ...
  CGRuntime::printDebugLine("-- end of debug session 'D" +
                            std::string(tcDepth) + "' --");
  return result;
}

void
GrfDebugExecution::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  CW_BODY_INDENT << "// DEBUG execution required into the source code "
                    "generation script: not translated";
  CW_BODY_ENDL;
}
}
