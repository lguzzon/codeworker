...
public:
  BNFTransformationModeScope(DtaBNFScript* pBNFScript,
                             bool bNewMode,
...
BNFTransformationMode::BNFTransformationMode(DtaBNFScript* pBNFScript,
                                             GrfBlock* pParent,
                                             bool bImplicitCopy,
                                             ExprScriptFunction* pFunction)
  : _pBNFScript(pBNFScript)
...
void
BNFTransformationMode::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFTransformationMode(*this, env);
}

bool
BNFTransformationMode::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFTransformationMode::executeInternal(DtaScriptVariable& visibility)
{
...
std::string
BNFTransformationMode::toString() const
{
  std::string sText;
  if (_bImplicitCopy)
    sText = "#implicitCopy";
...
void
BNFTransformationMode::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (i != getCommands().end()) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "CGBNFRuntimeTransformationMode "
                      "_compilerClause_transformationMode(theEnvironment, "
                   << ((_bImplicitCopy) ? "true" : "false") << ");";
    CW_BODY_ENDL;
    ...
    CW_BODY_INDENT << "\tCGRuntime::setInputLocation(_compilerClauseCursor_"
                   << iCursor << ");";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tCGRuntime::resizeOutputStream(" << tcImplicitCopy
                   << ");";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "}";
    CW_BODY_ENDL;
