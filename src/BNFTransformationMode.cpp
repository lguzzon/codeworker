...
bool BNFTransformationMode::isABNFCommand() const
{
    return true;
}
...
void BNFTransformationMode::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (i != getCommands().end()) {
        ...
        CW_BODY_ENDL;
        CW_BODY_INDENT << "CGBNFRuntimeTransformationMode _compilerClause_transformationMode(theEnvironment, " << ((_bImplicitCopy) ? "true" : "false") << ");";
        CW_BODY_ENDL;
        ...
        CW_BODY_INDENT << "int " << tcImplicitCopy << " = CGRuntime::getOutputLocation();";
        CW_BODY_ENDL;
