...
void GrfFunction::setPatternTemplateBody(DtaScriptFactory::SCRIPT_TYPE eParentScriptType, DtaPatternScript* pScript)
{
    if (_pTemplateFunction != NULL) {
        ...
        _pPatternTemplateBody = new PatternTemplateStruct(eParentScriptType, pScript);
    }
}
...
void GrfFunction::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (isATemplateInstantiation()) {
        sFunctionName = "_compilerTemplateFunction_" + sFunctionName + "_compilerInstantiation_" + theCompilerEnvironment.convertTemplateKey(_sTemplateInstantiation);
    } else {
    ...
    if (!_mapOfInstantiatedFunctions.empty()) {
        theCompilerEnvironment.getHeader() << "const std::string& _compilerTemplateFunction_dispatching";
    }
    ...
    if (!_mapOfInstantiatedFunctions.empty()) {
        CW_BODY_STREAM << "const std::string& _compilerTemplateFunction_dispatching";
    }
    ...
    if (_bIsBodyDefined) {
        ...
        if (_sTemplateInstantiation.empty()) {
            CW_BODY_INDENT << "\tthrow UtlException(\"function '" << _sName << "' hasn't been implemented\");";
        } else {
            CW_BODY_INDENT << "\tthrow UtlException(\"function '" << _sName << "<\\\"" << _sTemplateInstantiation << "\\\">' hasn't been implemented\");";
        }
