...
BNFReadText::BNFReadText(DtaBNFScript* pBNFScript,
                         GrfBlock* pParent,
                         bool bContinue,
                         bool bNoCase)
...
void
BNFReadText::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadText(*this, env);
}

bool
BNFReadText::isABNFCommand() const
{
  return true;
}

void
BNFReadText::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                 bool bConcat,
                                 BNFClause& theClause)
{
...
BNFReadText::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
  ...
  if (_bNoCase)
    bSuccess = CGRuntime::readIfEqualToIgnoreCase(sValue);
  else
    bSuccess = CGRuntime::readIfEqualTo(sValue);
  if (!bSuccess) {
...
void
BNFReadText::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT << "} else if (" << tcImplicitCopy
