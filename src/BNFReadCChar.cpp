...
BNFReadCChar::BNFReadCChar(DtaBNFScript* pBNFScript,
                           GrfBlock* pParent,
                           bool bContinue,
                           bool bNoCase)
...
void
BNFReadCChar::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadCChar(*this, env);
}

bool
BNFReadCChar::isABNFCommand() const
{
  return true;
}

void
BNFReadCChar::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                  bool bConcat,
                                  BNFClause& theClause)
{
...
BNFReadCChar::executeInternal(DtaScriptVariable& visibility)
{
...
void
BNFReadCChar::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "std::string _compilerClauseText_" << iCursor
                 << " = CGRuntime::readCChar();";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "_compilerClauseSuccess = (_compilerClauseNotEmptyLocation_"
                 << iCursor << " != _compilerClauseLocation_" << iCursor
                 << ");";
  CW_BODY_ENDL;
  if (!_listOfConstants.empty()) {
...
std::string
BNFReadCChar::toString() const
{
  std::string sText =
    "#readCChar" + DtaBNFScript::constantsToString(_listOfConstants) +
