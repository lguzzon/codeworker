...
GrfSwitch::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if (equalString(sExpression.c_str()))
    result = GrfBlock::executeAt(stackSwitch, _directionEqualList[sExpression]);
  else {
    bool bFound = false;
    int iRank;
    for (std::map<std::string, int>::const_iterator i =
           _directionStartList.begin();
         i != _directionStartList.end();
         i++) {
      if (strncmp(i->first.c_str(), sExpression.c_str(), i->first.size()) ==
          0) {
        bFound = true;
        iRank = i->second;
      } else if (bFound) {
    ...
    if (bFound) {
      ...
      if (_iDefault >= 0) {
        ...
        throw UtlException("switch internal error: unknown '" + sExpression +
                           "' case encountered, it lacks either a 'case' or a "
                           "'default' statement");
      }
    }
  }
...
void
GrfSwitch::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (!_directionEqualList.empty()) {
    CW_BODY_INDENT << "static std::map<std::string, int> _compilerSwitch_table"
                   << iSwitchNumber << ";";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "if (_compilerSwitch_table" << iSwitchNumber
                   << ".empty()) {";
    CW_BODY_ENDL;
    theCompilerEnvironment.incrementIndentation();
    for (std::map<std::string, int>::const_iterator i =
           _directionEqualList.begin();
         i != _directionEqualList.end();
         i++) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT
      << "std::map<std::string, int>::const_iterator _compilerSwitch_cursor"
      << iSwitchNumber << " = _compilerSwitch_table" << iSwitchNumber
      << ".find(_compilerSwitch_key" << iSwitchNumber << ");";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "if (_compilerSwitch_cursor" << iSwitchNumber
                   << " != _compilerSwitch_table" << iSwitchNumber
                   << ".end()) {";
    CW_BODY_ENDL;
    theCompilerEnvironment.incrementIndentation();
    CW_BODY_INDENT << "_compilerSwitch_statement" << iSwitchNumber
                   << " = _compilerSwitch_cursor" << iSwitchNumber
                   << "->second;";
    CW_BODY_ENDL;
    theCompilerEnvironment.decrementIndentation();
    CW_BODY_INDENT << "}";
  ...
  if (!_directionStartList.empty()) {
    ...
    for (std::map<std::string, int>::const_iterator i =
      ...
      mapOfStatements[i->second] = i->first;
      CW_BODY_STREAM << "if (strncmp(_compilerSwitch_key" << iSwitchNumber
                     << ".c_str(), ";
      CW_BODY_STREAM.writeString(i->first);
  ...
  if (_directionStartList.empty() && _directionEqualList.empty()) {
    throw UtlException("empty 'switch' statement encountered into file \"" +
                       std::string(_sParsingFilePtr) + "\"");
  }
  CW_BODY_STREAM << " else {";
  CW_BODY_ENDL;
  theCompilerEnvironment.incrementIndentation();
  ...
  CW_BODY_ENDL;
  if (_iDefault >= 0)
    mapOfStatements[_iDefault] = "default";
  theCompilerEnvironment.decrementIndentation();
  CW_BODY_INDENT << "}";
  CW_BODY_ENDL;
  ...
  theCompilerEnvironment.incrementIndentation();
  int iLastPosition = -1;
  for (std::map<int, std::string>::const_iterator i = mapOfStatements.begin();
       i != mapOfStatements.end();
       i++) {
    if (iLastPosition >= 0) {
      theCompilerEnvironment.incrementIndentation();
      CW_BODY_INDENT << "{";
  ...
  if (_iDefault < 0) {
    ...
    CW_BODY_ENDL;
    theCompilerEnvironment.incrementIndentation();
    CW_BODY_INDENT << "throw UtlException(\"switch internal error: unknown '\" "
                      "+ _compilerSwitch_key"
                   << iSwitchNumber << " + \"' case encountered into script \\";
    CW_BODY_STREAM.writeString(_sParsingFilePtr);
    CW_BODY_STREAM.setOutputLocation(CW_BODY_STREAM.getOutputLocation() - 1);
    CW_BODY_STREAM
      << "\\\", it lacks either a 'case' or a 'default' statement\");";
    CW_BODY_ENDL;
    theCompilerEnvironment.decrementIndentation();
  }
  theCompilerEnvironment.decrementIndentation();
