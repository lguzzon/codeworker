...
BNFReadChar::BNFReadChar(DtaBNFScript* pBNFScript,
                         GrfBlock* pParent,
                         bool bContinue)
...
void
BNFReadChar::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadChar(*this, env);
}

bool
BNFReadChar::isABNFCommand() const
{
  return true;
}

void
BNFReadChar::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                 bool bConcat,
                                 BNFClause& theClause)
{
...
void
BNFReadChar::setConstantsToMatch(
...
BNFReadChar::executeInternal(DtaScriptVariable& visibility)
{
...
void
BNFReadChar::postExecutionAssignment(DtaScriptVariable& variable,
                                     unsigned char cChar)
{
  char tcText[] = { '\0', '\0' };
  tcText[0] = cChar;
...
void
BNFReadChar::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT << "int _compilerClauseChar_" << iCursor
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT
    << "\tunsigned char cChar = (unsigned char) _compilerClauseChar_" << iCursor
    << ";";
  CW_BODY_ENDL;
...
void
BNFReadChar::compileCppAssignment(
...
std::string
BNFReadChar::toString() const
{
  std::string sText =
    "#readChar" + DtaBNFScript::constantsToString(_listOfConstants) +
