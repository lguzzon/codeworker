...
GrfSaveProject::executeInternal(DtaScriptVariable& visibility)
{
...
void
GrfSaveProject::populateDefaultParameters()
{
  if (_pNodeToSave == NULL)
    _pNodeToSave = new ExprScriptVariable("project");
}

//##protect##"implementation"
void
GrfSaveProject::generateXMLFile(UtlXMLStream& myXMLFile,
                                DtaScriptVariable& myNode,
                                const std::string& sObjectName)
{
  std::string sTagName = sObjectName;
  if (sTagName.empty())
    sTagName = myNode.getName();
  ...
  if (pReference != NULL) {
    ...
    if (bEmpty) {
      ...
      if (iIndex == std::string::npos)
        myXMLFile.writeEndOfObject(sObjectName);
      else
        myXMLFile.writeEndOfObject(sObjectName.substr(0, iIndex));
    }
  }
}

void
GrfSaveProject::generateTextFile(std::ostream& theStream,
                                 DtaScriptVariable& myNode,
                                 const std::string& sIndent)
{
  ...
  if (!bEmptyAttributes || !bEmptyArray) {
    ...
    while (pAttributes != NULL) {
      theStream << std::endl
                << sNewIndent << "." << pAttributes->getNode()->getName()
                << " = ";
...
void
GrfSaveProject::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
