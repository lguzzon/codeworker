...
void
GrfGenerateString::setPatternFileName(ExprScriptScriptFile* pPatternFileName)
{
...
GrfGenerateString::executeInternal(DtaScriptVariable& visibility)
{
  ...
  DtaScriptVariable* pOutputString =
    visibility.getExistingVariable(*_pOutputString);
  if (pOutputString == NULL) {
    throw UtlException("runtime error: variable " + _pOutputString->toString() +
                       " doesn't exist while calling procedure '" +
                       std::string(getFunctionName()) + "()'");
  }
...
SEQUENCE_INTERRUPTION_LIST
GrfGenerateString::executeScript(DtaScriptVariable* pOutputString,
                                 DtaScriptVariable* pThisContext,
                                 EXECUTE_FUNCTION* executeFunction)
{
  if (executeFunction != NULL) {
    CGRuntime::generateString(executeFunction,
                              CppParsingTree_var(pThisContext),
                              CppParsingTree_var(pOutputString));
    return NO_INTERRUPTION;
  }
  std::string sOutputString;
  SEQUENCE_INTERRUPTION_LIST result =
    _pCachedScript->generateString(sOutputString, *pThisContext);
