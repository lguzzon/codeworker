...
public:
    ...
    virtual UtlInternalDate* copyInstance() const
    {
        return new UtlInternalDate(_iDay, _iMonth, _iYear, _iHour, _iMin, _iSec, _iMillis, _sComment);
    }
    ...
    virtual bool isAFixedDate() const
    {
        return true;
    }
    virtual bool isAVariableDate() const
    {
        return false;
    }
    ...
    virtual void setDay(int iDay)
    {
        _iDay = iDay;
    }
    virtual void setMonth(int iMonth)
    {
        _iMonth = iMonth;
    }
    virtual void setYear(int iYear)
    {
        _iYear = iYear;
    }
    ...
    inline void setSec(int iSec)
    {
        _iSec = iSec;
    }
    inline int getMillis() const
    {
        return _iMillis;
    }
    inline void setMillis(int iMillis)
    {
        _iMillis = iMillis;
    }
    virtual bool instanceOf(const char *pName)
    {
        if (!strcmp(pName, "UtlInternalDate")) {
            return true ;
        } else {
            return false;
        }
    ...
    std::string getFormattedDate(const std::string& sFormat) const
    {
        ...
        while (i < sFormat.size()) {
            if (sFormat[i] == '%') {
                ...
                switch (sFormat[i]) {
                case 'Y':
                    sDate += formatInteger(_iYear, 4);
                    break;

                case 'y':
                    sDate += formatInteger(_iYear % 100, 2);
                    break;

                case 'b':
                    sDate += std::string(_tsMonths[_iMonth - 1]).substr(0, 3);
                    break;

                case 'B':
                    sDate += _tsMonths[_iMonth - 1];
                    break;

                case 'm':
                    sDate += formatInteger(_iMonth, 2);
                    break;

                case 'd':
                    sDate += formatInteger(_iDay, 2);
                    break;

                case 'e':
                    sDate += formatInteger(_iDay, 1);
                    break;
                ...
                case 't': {
                        ...
                        if (dWingz == floor(dWingz)) {
                            sprintf(sNumber, "%d", (int) dWingz);
                        } else {
                            sprintf(sNumber, "%f", dWingz);
                        }
                ...
                case 'w':
                    sDate += formatInteger(getWeekDay(), 1);
                    break;

                case 'W':
                    sDate += std::string(_tsWeekDays[getWeekDay()]);
                    break;

                case 'H':
                    sDate += formatInteger(_iHour, 2);
                    break;

                case 'I':
                    sDate += formatInteger(((_iHour > 12) ? _iHour - 12 : _iHour), 2);
                    break;

                case 'p':
                    sDate += ((_iHour > 12) ? "PM" : "AM");
                    break;
                ...
                case 'Q':
                    ...
                    if (_iMillis != 0) {
                        sDate += "." + formatInteger(_iMillis,  3);
                    }

                    break;

                case 'M':
                    sDate += formatInteger(_iMin,  2);
                    break;

                case 'S':
                    sDate += formatInteger(_iSec,  2);
                    break;
                ...
                case 'j': {
                        ...
                        while (iMonth < _iMonth) {
                            iDays += getNbDaysInMonth(iMonth++, _iYear);
                        }
                ...
                case 'D':
                    sDate += formatInteger(_iMonth, 2) + "/" + formatInteger(_iDay, 2) + "/" + formatInteger(_iYear % 100, 2);
                    break;

                case 'r':
                    sDate += formatInteger(((_iHour > 12) ? _iHour - 12 : _iHour), 2) + ":" + formatInteger(_iMin, 2) + ":" + formatInteger(_iSec, 2) + ((_iHour > 12) ? " PM" : " AM");
                    break;

                case 'T':
                    sDate += formatInteger(_iHour, 2) + ":" + formatInteger(_iMin, 2) + ":" + formatInteger(_iSec, 2);
                    break;
    ...
    {
        return (int) getWINGZDate();
    }

    double getWINGZDate() const
    {
        if (isInfinite()) {
            return WINGZ_INFINITY;
        }
...
public:
    ...
    virtual UtlInternalDate* copyInstance() const
    {
        return new UtlVariableDate(_iFrequency, _cUnit);
    }
    ...
    virtual int getWeekDay() const
    {
        ...
        switch (_cUnit) {
        case 'D':
            date.addDay(_iFrequency);
            break;

        case 'W':
            date.addDay(7 * _iFrequency);
            break;

        case 'M':
            date.addMonth(_iFrequency);
            break;

        case 'Y':
            date.addYear(_iFrequency);
            break;
        }
    ...
    virtual bool instanceOf(const char *pName)
    {
        if (!strcmp(pName, "UtlVariableDate")) {
            return true ;
        } else {
            return false;
        }
...
{
    if (date._pInternalDate == NULL) {
        _pInternalDate = NULL;
    } else {
        _pInternalDate = date._pInternalDate->copyInstance();
    }
}
...
UtlDate::UtlDate(double dWINGZDate)
{
    if (dWINGZDate <= 0.0) {
        ...
        _pInternalDate = new UtlInternalDate(dWINGZDate);
    }
}
...
{
    if (&date == this) {
        throw UtlException("a date can't assign itself");
    }
...
UtlDate UtlDate::getDateFromFormat(const std::string& sDate, const std::string& sFormat)
{
    ...
    while ((i < sFormat.size()) && (j < sDate.size())) {
        if (sFormat[i] == '%') {
            ...
            case 'Y':
                if (j + 4 > sDate.size()) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Y'");
                }
            ...
            case 'y': {
                    ...
                    if (iTodayShortYear > iYear) {
                        if (iTodayShortYear - iYear > 50) {
                            iYear += 100;
                        }
                    } else {
                        if (iYear - iTodayShortYear > 50) {
                            iYear -= 100;
                        }
                    }
            ...
            case 'm':
                ...
                if ((today.getMonth() < 1) || (today.getMonth() > 12)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%m'");
                }
            ...
            case 'd':
                ...
                if ((today.getDay() < 1) || (today.getDay() > 31)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%d'");
                }
            ...
            case 'H':
                ...
                if ((today.getHour() < 0) || (today.getHour() > 23)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%H'");
                }
            ...
            case 'I':
                if (j + 2 > sDate.size()) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%I'");
                }
            ...
            case 'M':
                ...
                if ((today.getMin() < 0) || (today.getMin() > 59)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%M'");
                }
            ...
            case 'S':
                ...
                if ((today.getSec() < 0) || (today.getSec() > 59)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%S'");
                }
            ...
            case 'D':
                ...
                if ((today.getMonth() < 1) || (today.getMonth() > 12)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%D -> %m'");
                }

                if (sDate[j++] != '/') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%D -> '/' expected after %m'");
                }

                today.setDay(getInteger(sDate, 2, j));

                if ((today.getDay() < 1) || (today.getDay() > 31)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%D -> %d'");
                }

                if (sDate[j++] != '/') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%D -> '/' expected after %d'");
                }
            ...
            case 'Q':
                ...
                if (sDate[j++] != '-') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> '-' expected after %Y'");
                }

                today.setMonth(getInteger(sDate, 2, j));

                if ((today.getMonth() < 1) || (today.getMonth() > 12)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> %m'");
                }

                if (sDate[j++] != '-') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> '-' expected after %m'");
                }

                today.setDay(getInteger(sDate, 2, j));

                if ((today.getDay() < 1) || (today.getDay() > 31)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> %d'");
                }

                if (sDate[j++] != ' ') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> ' ' expected after %d'");
                }

                today.setHour(getInteger(sDate, 2, j));

                if ((today.getHour() < 0) || (today.getHour() > 23)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> %H'");
                }

                if (sDate[j++] != ':') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> ':' expected after %H'");
                }
                ...
                if ((today.getSec() < 0) || (today.getSec() > 59)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%Q -> %S'");
                }
            ...
            case 'L':
                ...
                {
                    if (j >= sDate.size()) {
                        throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%L'");
                    }
            ...
            case 'b':
                if (j + 3 > sDate.size()) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%b'");
                }
                ...
                sNumber = sDate.substr(j, 3);
                {
                    ...
                    for (k = 0; k < 12; k++) if (strnicmp(UtlInternalDate::_tsMonths[k], sNumber, 3) == 0) {
                            break;
                        }

                    if (k >= 12) {
                        throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%b' -> unknow month = '" + sNumber + "'");
                    }
            ...
            case 'B': {
                    ...
                    for (k = 0; k < 12; k++) {
                        ...
                        if (j + iLength <= sDate.size()) {
                            if (strnicmp(UtlInternalDate::_tsMonths[k], sDate.c_str() + j, iLength) == 0) {
                                break;
                            }
                        }
                    }
            ...
            case 'T':
                ...
                if ((today.getHour() < 0) || (today.getHour() > 23)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%T -> %H'");
                }

                if (sDate[j++] != ':') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%T -> ':' expected after %H'");
                }

                today.setMin(getInteger(sDate, 2, j));

                if ((today.getMin() < 0) || (today.getMin() > 59)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%T -> %M'");
                }

                if (sDate[j++] != ':') {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%T -> ':' expected after %M'");
                }

                today.setSec(getInteger(sDate, 2, j));

                if ((today.getSec() < 0) || (today.getSec() > 59)) {
                    throw UtlException("syntax error on date format: format \"" + sFormat + "\" doesn't match with date '" + sDate + "' for '%T -> %S'");
                }
