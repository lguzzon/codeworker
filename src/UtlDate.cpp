...
public:
  ...
  UtlInternalDate(double dWINGZDate) { setWINGZDate(dWINGZDate); }
  UtlInternalDate(int iWinfrontDate) { setWinfrontDate(iWinfrontDate); }
  UtlInternalDate(int iDay,
                  int iMonth,
                  int iYear,
                  int iHour,
                  int iMin,
  ...
  {
    return new UtlInternalDate(
      _iDay, _iMonth, _iYear, _iHour, _iMin, _iSec, _iMillis, _sComment);
  }

  bool isInfinite() const { return (getYear() >= INFINITE_YEAR); }
    ...
    while (i < sFormat.size()) {
      if (sFormat[i] == '%') {
        ...
        switch (sFormat[i]) {
          ...
          case 'Q':
            sDate += formatInteger(_iYear, 4) + "-" +
                     formatInteger(_iMonth, 2) + "-" + formatInteger(_iDay, 2) +
                     " " + formatInteger(_iHour, 2) + ":" +
                     formatInteger(_iMin, 2) + ":" + formatInteger(_iSec, 2);
            if (_iMillis != 0)
          ...
          case 'D':
            sDate += formatInteger(_iMonth, 2) + "/" + formatInteger(_iDay, 2) +
                     "/" + formatInteger(_iYear % 100, 2);
            break;
          case 'r':
            sDate += formatInteger(((_iHour > 12) ? _iHour - 12 : _iHour), 2) +
                     ":" + formatInteger(_iMin, 2) + ":" +
                     formatInteger(_iSec, 2) + ((_iHour > 12) ? " PM" : " AM");
            break;
          case 'T':
            sDate += formatInteger(_iHour, 2) + ":" + formatInteger(_iMin, 2) +
                     ":" + formatInteger(_iSec, 2);
            break;
          default:
...
const char* UtlInternalDate::_tsWeekDays[7] = {
  "sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"
};
const char* UtlInternalDate::_tsMonths[12] = {
  "january", "february", "march",     "april",   "may",      "june",
  "july",    "august",   "september", "october", "november", "december"
};
int UtlInternalDate::_tNbDays[12] = { 31, 28, 31, 30, 31, 30,
                                      31, 31, 30, 31, 30, 31 };
...
public:
  UtlVariableDate(int iFrequency, char cUnit)
    ...
    switch (_cUnit) {
      ...
      default:
        sprintf(sErrorMessage,
                "Undefined unit of variable date: '%c' for \"%d%c\"",
                cUnit,
                iFrequency,
                cUnit);
  ...
  {
    UtlInternalDate date(UtlInternalDate::getDay(),
                         UtlInternalDate::getMonth(),
                         UtlInternalDate::getYear(),
                         0,
                         0,
                         0,
                         0,
  ...
  virtual int getFrequency() const { return _iFrequency; }

  virtual char getUnit() const { return _cUnit; }
...
{
  _pInternalDate =
    new UtlInternalDate(iDay, iMonth, iYear, iHour, iMin, iSec, iMillis, "");
}
...
bool
UtlDate::operator<(const UtlDate& date) const
{
...
UtlDate::operator==(const UtlDate& date) const
{
  ...
  return (getDay() == date.getDay()) && (getMonth() == date.getMonth()) &&
         (getYear() == date.getYear()) && (getHour() == date.getHour()) &&
         (getMin() == date.getMin()) && (getSec() == date.getSec()) &&
...
void
UtlDate::setTime(int iHour, int iMin, int iSec, int iMillis)
{
  _pInternalDate->setTime(iHour, iMin, iSec, iMillis);
}

int
UtlDate::compareTime(const UtlDate& date) const
{
  int iHour1 = getHour();
  int iHour2 = date.getHour();
  if (iHour1 != iHour2)
...
std::string
UtlDate::getFormattedDate(const std::string& sFormat) const
{
  return _pInternalDate->getFormattedDate(sFormat);
}

UtlDate
UtlDate::getDateFromFormat(const std::string& sDate, const std::string& sFormat)
{
  UtlDate today;
  today.ignoreHours();
  std::string::size_type i = 0;
  ...
  while ((i < sFormat.size()) && (j < sDate.size())) {
    if (sFormat[i] == '%') {
      ...
      switch (sFormat[i]) {
        case 'Y':
          if (j + 4 > sDate.size())
            throw UtlException("syntax error on date format: format \"" +
                               sFormat + "\" doesn't match with date '" +
                               sDate + "' for '%Y'");
        ...
        case 'L':
          // Be careful!
          // Don't move this case label and don't insert anything before case
          // '%Q'
          {
            if (j >= sDate.size())
              throw UtlException("syntax error on date format: format \"" +
        ...
        case 'b':
          ...
          sNumber = sDate.substr(j, 3);
          {
            int k;
            for (k = 0; k < 12; k++)
              if (strnicmp(UtlInternalDate::_tsMonths[k], sNumber, 3) == 0)
                break;
            if (k >= 12)
              throw UtlException("syntax error on date format: format \"" +
                                 sFormat + "\" doesn't match with date '" +
                                 sDate + "' for '%b' -> unknow month = '" +
                                 sNumber + "'");
            today.setMonth(k + 1);
          }
          j += 3;
        ...
        case 'B': {
          ...
          for (k = 0; k < 12; k++) {
            ...
            if (j + iLength <= sDate.size()) {
              if (strnicmp(UtlInternalDate::_tsMonths[k],
                           sDate.c_str() + j,
                           iLength) == 0)
                break;
            }
        ...
        case 'U':
        case 'w':
          throw UtlException("date format \"... " +
                             std::string(sFormat.c_str() + (i - 1)) +
                             "' not implemented yet");
        default:
          throw UtlException("syntax error on date format \"" + sFormat +
                             "\" for '%" + sFormat[i] + "'");
  ...
  if (i < sFormat.size()) {
    if ((sFormat[i] != '%') || (sFormat[i + 1] != '|')) {
      throw UtlException("date format \"" + sFormat + "\" not applied on \"" +
                         sDate + "\" completely");
    }
  }
  return today;
...
void
UtlDate::addDateFromFormat(const std::string& sFormat, const std::string& sDate)
{
  std::string::size_type i = 0;
  std::string::size_type j = 0;
  std::string sNumber;
...
int
UtlDate::getDayWINGZDate() const
{
  return _pInternalDate->getDayWINGZDate();
}

double
UtlDate::getWINGZDate() const
{
  if (_pInternalDate == NULL)
    return 0.0;
...
void
UtlDate::setWINGZDate(double dWINGZDate)
{
  delete _pInternalDate;
  if (dWINGZDate <= 0.0)
    _pInternalDate = NULL;
...
int
UtlDate::getInteger(const std::string& sValue,
                    int iNbMaxOfDigits,
                    std::string::size_type& iCursor)
{
  bool bSigned;
...
double
UtlDate::getDouble(const std::string& sValue,
                   int iNbMaxOfDigits,
                   std::string::size_type& iCursor)
{
  bool bSigned;
...
void
UtlDate::setComment(const std::string& sComment)
{
  if (_pInternalDate != NULL) {
