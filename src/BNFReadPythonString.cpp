...
BNFReadPythonString::BNFReadPythonString(DtaBNFScript* pBNFScript,
                                         GrfBlock* pParent,
                                         bool bContinue,
                                         bool bNoCase)
...
BNFReadPythonString::~BNFReadPythonString()
{
  delete _pVariableToAssign;
}

void
BNFReadPythonString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadPythonString(*this, env);
}

bool
BNFReadPythonString::isABNFCommand() const
{
  return true;
}

void
BNFReadPythonString::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                         bool bConcat,
                                         BNFClause& theClause)
{
...
BNFReadPythonString::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
  ...
  std::string sValue;
  bSuccess = CGRuntime::readPythonString(sValue);
  if (!bSuccess) {
...
void
BNFReadPythonString::compileCpp(
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "_compilerClauseSuccess = "
                    "CGRuntime::readPythonString(_compilerClauseText_"
                 << iCursor << ");";
  CW_BODY_ENDL;
  if (!_listOfConstants.empty()) {
...
std::string
BNFReadPythonString::toString() const
{
  std::string sText =
    "#readPythonString" + DtaBNFScript::constantsToString(_listOfConstants) +
