...
{
BNFReadPythonString::BNFReadPythonString(DtaBNFScript* pBNFScript, GrfBlock* pParent, bool bContinue, bool bNoCase) : _pBNFScript(pBNFScript), GrfCommand(pParent), _pVariableToAssign(NULL), _bContinue(bContinue), _iClauseReturnType(BNFClause::NO_RETURN_TYPE), _bNoCase(bNoCase) {}
...
void BNFReadPythonString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
    visitor.visitBNFReadPythonString(*this, env);
}

bool BNFReadPythonString::isABNFCommand() const
{
    return true;
}

void BNFReadPythonString::setVariableToAssign(ExprScriptVariable* pVariableToAssign, bool bConcat, BNFClause& theClause)
{
...
SEQUENCE_INTERRUPTION_LIST BNFReadPythonString::executeInternal(DtaScriptVariable& visibility)
{
    bool bSuccess;
...
void BNFReadPythonString::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "_compilerClauseSuccess = CGRuntime::readPythonString(_compilerClauseText_" << iCursor << ");";
    CW_BODY_ENDL;
...
std::string BNFReadPythonString::toString() const
{
    std::string sText = "#readPythonString" + DtaBNFScript::constantsToString(_listOfConstants) + DtaBNFScript::assignmentToString(_pVariableToAssign, _bConcatVariable);
