...
SEQUENCE_INTERRUPTION_LIST GrfQuantifyExecution::openSession(DtaScriptVariable& visibility)
{
    ...
    try {
        ...
        if (_pFilename != NULL) {
            ...
            if ((stricmp(sExtension.c_str(), ".html") != 0) && (stricmp(sExtension.c_str() + 1, ".htm") != 0)) {
                throw UtlException("HTML file must have extension '.htm' or '.html'");
            }
        }
        ...
        myTimer.stop();
        char tcMessage[80];
        sprintf(tcMessage, "quantify execution time = %ldms", myTimer.getTimeInMillis());
    ...
    if ((sFilename != NULL) && (sFilename[0] != '\0')) {
        ...
        for (std::map<std::string, std::map<int, int> >::const_iterator i = _coveredLines.begin(); i != _coveredLines.end(); i++) {
            ...
            if (pFile != NULL) {
                ...
                for (j = i->second.begin(); j != i->second.end(); j++) if (j->second > iMaxCounter) {
