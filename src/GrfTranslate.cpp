...
GrfTranslate::GrfTranslate()
  ...
  , _pInputFileName(NULL)
  , _pOutputFileName(NULL)
{
...
void
GrfTranslate::setPatternFileName(ExprScriptScriptFile* pPatternFileName)
{
  if (pPatternFileName->isFileName())
    _pPatternFileName = pPatternFileName->getFileName();
  _pCachedScript =
    dynamic_cast<DtaTranslateScript*>(pPatternFileName->getBody());
  pPatternFileName->release();
}

SEQUENCE_INTERRUPTION_LIST
GrfTranslate::executeInternal(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pClass = visibility.getExistingVariable(*_pClass);
  if (pClass == NULL) {
    throw UtlException("runtime error: variable '" + _pClass->toString() +
                       "' doesn't exist while calling procedure '" +
  ...
  if (_pPatternFileName != NULL) {
    ...
    if ((_pCachedScript == NULL) || (_sCachedPatternFile != sPatternFileName) ||
        ScpStream::existVirtualFile(sPatternFileName)) {
      executeFunction =
        DtaScript::getRegisteredScript(sPatternFileName.c_str());
      if (executeFunction == NULL) {
        ...
        _pCachedScript->parseFile(sPatternFileName.c_str(),
                                  _sCurrentDirectoryAtCompileTime);
      }
    }
  }

  SEQUENCE_INTERRUPTION_LIST result =
    executeScript(sInputFile, sOutputFile, pClass, executeFunction);
  switch (result) {
...
SEQUENCE_INTERRUPTION_LIST
GrfTranslate::executeScript(const std::string& sInputFile,
                            ...
                            EXECUTE_FUNCTION* executeFunction)
{
  if (executeFunction != NULL) {
    CGRuntime::translate(executeFunction,
                         CppParsingTree_var(pThisContext),
                         sInputFile,
                         sOutputFile);
...
std::string
GrfTranslate::translateString(DtaScriptVariable& visibility)
{
  DtaScriptVariable* pClass = visibility.getExistingVariable(*_pClass);
  if (pClass == NULL) {
    throw UtlException("runtime error: variable '" + _pClass->toString() +
...
void
GrfTranslate::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ExprScriptConstant* pConstantFileName =
    dynamic_cast<ExprScriptConstant*>(_pPatternFileName);
  ...
  std::string sRadical =
    DtaScript::convertFilenameAsIdentifier(CppCompilerEnvironment::getRadical(
      CppCompilerEnvironment::filename2Module(sScriptFilename)));
  CW_BODY_INDENT << "CGRuntime::" << getFunctionName() << "(&Execute"
                 << sRadical << "::instance(), ";
