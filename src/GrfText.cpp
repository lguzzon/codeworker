...
GrfText::executeInternal(DtaScriptVariable&)
{
  if (*_pOutputCoverage != NULL) {
    ...
    pCoverage->insertNode("output")->setValue(
      (*_pOutput)->getOutputLocation() + (*_pOutputCoverage)->getIntValue());
  }
...
void
GrfText::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
