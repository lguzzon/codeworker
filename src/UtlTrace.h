...
#define UTLTRACE_METHOD(iLevel, sClass, sMethod)                               \
  UtlTrace traceInstance(iLevel, sClass, sMethod)
#define UTLTRACE_TEXT(iLevel, sText)                                           \
  if (UtlTrace::isValidLevel(iLevel))                                          \
  UtlTrace::traceText(__FILE__, __LINE__, sText)
#define UTLTRACE_FORMAT UtlTrace::traceFormat
#define UTLTRACE_STACK_INSTRUCTION(sFileName, iLine)                           \
  UtlTrace::traceStackInstruction(sFileName, iLine)
#define UTLTRACE_STACK_FUNCTION(sFileName, sFunctionName, iLine)               \
  UtlTraceStackFunction myTraceStackFunction(sFileName, sFunctionName, iLine)

#ifdef THROW_UTLEXCEPTION
#undef THROW_UTLEXCEPTION
#endif
#define THROW_UTLEXCEPTION(sText)                                              \
  {                                                                            \
    std::string sExceptionTrace = std::string("EXCEPTION: \"") + sText + "\""; \
    UtlTrace::writeText(sExceptionTrace.c_str());                              \
    throw UtlException(sText);                                                 \
  }

#ifdef THROW_UTLEXCEPTION2
#undef THROW_UTLEXCEPTION2
#endif
#define THROW_UTLEXCEPTION2(myStream, sText)                                   \
  {                                                                            \
    std::string sExceptionTrace = std::string("EXCEPTION: \"") + sText + "\""; \
    UtlTrace::writeText(sExceptionTrace.c_str());                              \
    throw UtlException(myStream, sText);                                       \
  }

#ifdef THROW_UTLEXCEPTION_STATIC
#undef THROW_UTLEXCEPTION_STATIC
#endif
#define THROW_UTLEXCEPTION_STATIC(sText)                                       \
  {                                                                            \
    std::string sExceptionTrace = std::string("EXCEPTION: \"") + sText + "\""; \
    UtlTrace::writeText(sExceptionTrace.c_str());                              \
    throw UtlException(sText);                                                 \
...
public:
  ...
  static void traceText(const char* sSourceFile,
                        int iLineCode,
                        const char* sText);
  ...
  static void traceStackFunction(const char* sFileName,
  ...
  static void initialize(const char* sTraceFile,
                         bool bPersistent,
                         int iLowerLevel,
                         int iUpperLevel);
  static void setConfiguration(const char* sTraceFile,
                               bool bPersistent,
                               int iLowerLevel,
                               int iUpperLevel);
...
{
public:
  UtlTraceStackFunction(const char* sFileName,
                        const char* sFunctionName,
                        int iLine);
