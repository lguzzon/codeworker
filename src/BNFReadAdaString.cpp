...
BNFReadAdaString::BNFReadAdaString(DtaBNFScript* pBNFScript,
                                   GrfBlock* pParent,
                                   bool bContinue,
                                   bool bNoCase)
...
BNFReadAdaString::~BNFReadAdaString()
{
  delete _pVariableToAssign;
}

void
BNFReadAdaString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFReadAdaString(*this, env);
}

bool
BNFReadAdaString::isABNFCommand() const
{
  return true;
}

void
BNFReadAdaString::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                      bool bConcat,
                                      BNFClause& theClause)
{
...
BNFReadAdaString::executeInternal(DtaScriptVariable& visibility)
{
  bool bSuccess;
  ...
  std::string sValue;
  bSuccess = CGRuntime::readAdaString(sValue);
  if (!bSuccess) {
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT
    << "_compilerClauseSuccess = CGRuntime::readAdaString(_compilerClauseText_"
    << iCursor << ");";
  CW_BODY_ENDL;
  if (!_listOfConstants.empty()) {
...
std::string
BNFReadAdaString::toString() const
{
  std::string sText =
    "#readAdaString" + DtaBNFScript::constantsToString(_listOfConstants) +
