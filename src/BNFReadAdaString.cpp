...
{
BNFReadAdaString::BNFReadAdaString(DtaBNFScript* pBNFScript, GrfBlock* pParent, bool bContinue, bool bNoCase) : _pBNFScript(pBNFScript), GrfCommand(pParent), _pVariableToAssign(NULL), _bContinue(bContinue), _iClauseReturnType(BNFClause::NO_RETURN_TYPE), _bNoCase(bNoCase) {}
...
void BNFReadAdaString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
    visitor.visitBNFReadAdaString(*this, env);
}

bool BNFReadAdaString::isABNFCommand() const
{
    return true;
}

void BNFReadAdaString::setVariableToAssign(ExprScriptVariable* pVariableToAssign, bool bConcat, BNFClause& theClause)
{
...
SEQUENCE_INTERRUPTION_LIST BNFReadAdaString::executeInternal(DtaScriptVariable& visibility)
{
    bool bSuccess;
...
void BNFReadAdaString::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "_compilerClauseSuccess = CGRuntime::readAdaString(_compilerClauseText_" << iCursor << ");";
    CW_BODY_ENDL;
...
std::string BNFReadAdaString::toString() const
{
    std::string sText = "#readAdaString" + DtaBNFScript::constantsToString(_listOfConstants) + DtaBNFScript::assignmentToString(_pVariableToAssign, _bConcatVariable);
