...
{
BNFReadCString::BNFReadCString(DtaBNFScript* pBNFScript, GrfBlock* pParent, bool bContinue, bool bNoCase) : _pBNFScript(pBNFScript), GrfCommand(pParent), _pVariableToAssign(NULL), _bContinue(bContinue), _iClauseReturnType(BNFClause::NO_RETURN_TYPE), _bNoCase(bNoCase) {}
...
void BNFReadCString::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
    visitor.visitBNFReadCString(*this, env);
}

bool BNFReadCString::isABNFCommand() const
{
    return true;
}

void BNFReadCString::setVariableToAssign(ExprScriptVariable* pVariableToAssign, bool bConcat, BNFClause& theClause)
{
...
SEQUENCE_INTERRUPTION_LIST BNFReadCString::executeInternal(DtaScriptVariable& visibility)
{
    bool bSuccess;
...
void BNFReadCString::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "_compilerClauseSuccess = CGRuntime::readString(_compilerClauseText_" << iCursor << ");";
    CW_BODY_ENDL;
...
std::string BNFReadCString::toString() const
{
    std::string sText = "#readCString" + DtaBNFScript::constantsToString(_listOfConstants) + DtaBNFScript::assignmentToString(_pVariableToAssign, _bConcatVariable);
