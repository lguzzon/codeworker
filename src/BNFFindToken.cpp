...
BNFFindToken::BNFFindToken(DtaBNFScript* pBNFScript,
                           GrfBlock* pParent,
                           bool bContinue)
  : BNFStepper(pBNFScript, pParent, bContinue)
  , _pIntermediateSequence(NULL)
  , _pIntermediateVariableToAssign(NULL)
...
void
BNFFindToken::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFFindToken(*this, env);
}

GrfBlock&
BNFFindToken::createIntermediateSequence()
{
...
void
BNFFindToken::setIntermediateVariableToAssign(
...
void
BNFFindToken::setFinalVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                       bool bConcat,
                                       BNFClause& theClause)
{
  if (pVariableToAssign != NULL) {
...
BNFFindToken::executeInternal(DtaScriptVariable& visibility)
{
  ...
  for (iLoop = 0; (iLoop <= iBegin) || (iEnd < 0) || (iLoop <= iEnd); iLoop++) {
    ...
    if (_iStepLocation > iStartToken) {
      ...
      if (iImplicitCopyPosition >= 0) {
        std::string sText =
          CGRuntime::getLastReadChars(_iStepLocation - iStartToken);
  ...
  if (result == NO_INTERRUPTION) {
    if (iLoop >= iBegin) {
      ...
      result = resolveAssignment(visibility,
                                 _listOfConstants,
                                 _pVariableToAssign,
                                 _bConcatVariable,
                                 iImplicitCopyPosition,
                                 iLocation,
                                 iNotEmptyLocation,
                                 iCurrentLocation);
      if (result == NO_INTERRUPTION) {
        result = resolveAssignment(visibility,
                                   _listOfFinalConstants,
                                   _pFinalVariableToAssign,
                                   _bConcatFinalVariable,
                                   iImplicitCopyPosition,
                                   iLocation,
                                   iIntermediateLocation,
                                   iCurrentLocation);
        if (result == NO_INTERRUPTION) {
          CGRuntime::setInputLocation(iIntermediateLocation);
          result = resolveAssignment(visibility,
                                     _listOfIntermediateConstants,
                                     _pIntermediateVariableToAssign,
                                     _bConcatIntermediateVariable,
                                     iImplicitCopyPosition,
                                     iLocation,
                                     iNotEmptyLocation,
                                     iIntermediateLocation);
          ...
          if (result == NO_INTERRUPTION)
            CGRuntime::setInputLocation(iCurrentLocation);
        }
      }
    } else {
...
SEQUENCE_INTERRUPTION_LIST
BNFFindToken::resolveAssignment(DtaScriptVariable& visibility,
                                ...
                                int iEndLocation)
{
  ...
  if (pVariableToAssign != NULL) {
    ...
    if (_iClauseReturnType == BNFClause::LIST_RETURN_TYPE) {
      ...
      if (bConcatVariable)
        pVariable->concatValue(sTextToMatch.c_str());
      else
        pVariable->setValue(sTextToMatch.c_str());
    }
  }
...
void
BNFFindToken::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (i != getCommands().end()) {
    ...
    if (!_listOfConstants.empty() || (_pVariableToAssign != 0) ||
        ...
        (_pIntermediateVariableToAssign != 0) ||
        !_listOfFinalConstants.empty() || (_pFinalVariableToAssign != 0)) {
      ...
      if (!_listOfIntermediateConstants.empty() ||
          (_pIntermediateVariableToAssign != 0) ||
          !_listOfFinalConstants.empty() || (_pFinalVariableToAssign != 0)) {
        CW_BODY_INDENT << "int _compilerClauseIntermediateLocation_" << iCursor
                       << " = _compilerClauseNotEmptyLocation_" << iCursor
                       << ";";
        CW_BODY_ENDL;
      }
    }
    ...
    CW_BODY_INDENT << "int _compilerClauseNextLocation_" << iCursor << " = 0;";
    CW_BODY_ENDL;
    if (_pBoundaries != 0) {
      if (!_pBoundaries->bConstantBoundaries)
        throw UtlException(
          "BNFFindToken::compileCpp(): variable boundaries not handled yet!");
      CW_BODY_INDENT << "int _compilerClauseLoop_" << iCursor << ";";
      CW_BODY_ENDL;
      CW_BODY_INDENT << "int _compilerClauseBegin_" << iCursor << " = "
                     << _pBoundaries->choice.constant._iBegin << ";";
      CW_BODY_ENDL;
      CW_BODY_INDENT << "int _compilerClauseEnd_" << iCursor << " = "
                     << _pBoundaries->choice.constant._iEnd << ";";
      CW_BODY_ENDL;
      CW_BODY_INDENT << "for (_compilerClauseLoop_" << iCursor
                     << " = 0; (_compilerClauseLoop_" << iCursor
                     << " <= _compilerClauseBegin_" << iCursor
                     << ") || (_compilerClauseEnd_" << iCursor
                     << " < 0) || (_compilerClauseLoop_" << iCursor
                     << " <= _compilerClauseEnd_" << iCursor
                     << "); ++_compilerClauseLoop_" << iCursor << ") {";
      CW_BODY_ENDL;
    } else {
      CW_BODY_INDENT << "for (;;) {";
      CW_BODY_ENDL;
    ...
    CW_BODY_INDENT << "if (_compilerClauseSuccess) break;";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "int _compilerClauseStartToken_" << iCursor
                   << " = CGRuntime::getInputLocation();";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "if (_compilerClauseNextLocation_" << iCursor
                   << " > _compilerClauseStartToken_" << iCursor << ") {";
    CW_BODY_ENDL;
    CW_BODY_INDENT
      << "\tCGRuntime::setInputLocation(_compilerClauseNextLocation_" << iCursor
      << ");";
    CW_BODY_ENDL;
    if (!_listOfIntermediateConstants.empty() ||
        (_pIntermediateVariableToAssign != 0) ||
        !_listOfFinalConstants.empty() || (_pFinalVariableToAssign != 0)) {
      CW_BODY_INDENT << "\t_compilerClauseIntermediateLocation_" << iCursor
                     << " = _compilerClauseNextLocation_" << iCursor << ";";
      CW_BODY_ENDL;
    }
    CW_BODY_INDENT << "\tif (" << tcImplicitCopy << " >= 0) {";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t\tstd::string sText = "
                      "CGRuntime::getLastReadChars(_compilerClauseNextLocation_"
                   << iCursor << " - _compilerClauseStartToken_" << iCursor
                   << ");";
    CW_BODY_ENDL;
    CW_BODY_INDENT
      << "\t\ttheEnvironment.writeBinaryData(sText.c_str(), sText.size());";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t}";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t_compilerClauseNextLocation_" << iCursor << " = 0;";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "} else {";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tint iChar = CGRuntime::readCharAsInt();";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\tif (iChar < 0) break;";
    CW_BODY_ENDL;
    if (!_listOfIntermediateConstants.empty() ||
        (_pIntermediateVariableToAssign != 0) ||
        !_listOfFinalConstants.empty() || (_pFinalVariableToAssign != 0)) {
      CW_BODY_INDENT << "\t_compilerClauseIntermediateLocation_" << iCursor
                     << " = _compilerClauseStartToken_" << iCursor << " + 1;";
      CW_BODY_ENDL;
    }
    CW_BODY_INDENT << "\tif (" << tcImplicitCopy << " >= 0) {";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t\tunsigned char cChar = (unsigned char) iChar;";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t\ttheEnvironment.writeBinaryData(cChar);";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\t}";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "}";
    ...
    if (_pBoundaries != 0) {
      CW_BODY_INDENT << "_compilerClauseSuccess = _compilerClauseSuccess && "
                        "(_compilerClauseLoop_"
                     << iCursor << " >= _compilerClauseBegin_" << iCursor
                     << ");";
      CW_BODY_ENDL;
    }
    if (!_listOfConstants.empty() || (_pVariableToAssign != 0) ||
      ...
      CW_BODY_ENDL;
      theCompilerEnvironment.incrementIndentation();
      CW_BODY_INDENT << "int _compilerClauseCurrentLocation_" << iCursor
                     << " = CGRuntime::getInputLocation();";
      CW_BODY_ENDL;
      if (!_listOfConstants.empty() || (_pVariableToAssign != 0)) {
        compileCppResolveAssignment(theCompilerEnvironment,
                                    iCursor,
                                    _listOfConstants,
                                    _pVariableToAssign,
                                    _bConcatVariable,
      ...
      if (!_listOfIntermediateConstants.empty() ||
        ...
        CW_BODY_INDENT
          << "CGRuntime::setInputLocation(_compilerClauseIntermediateLocation_"
          << iCursor << ");";
        CW_BODY_ENDL;
        compileCppResolveAssignment(theCompilerEnvironment,
                                    iCursor,
        ...
        CW_BODY_STREAM
          << "CGRuntime::setInputLocation(_compilerClauseCurrentLocation_"
          << iCursor << ");";
        CW_BODY_ENDL;
        if (!_listOfConstants.empty() || !_listOfFinalConstants.empty()) {
          theCompilerEnvironment.decrementIndentation();
...
void
BNFFindToken::compileCppResolveAssignment(
  ...
  const std::string& sEndLocation) const
{
  if (!listOfConstants.empty()) {
    CW_BODY_INDENT << "std::string _compilerClauseText_" << iCursor
                   << " = CGRuntime::getLastReadChars(_compilerClause"
                   << sEndLocation << "_" << iCursor << " - _compilerClause"
                   << sBeginLocation << "_" << iCursor << ");";
    CW_BODY_ENDL;
    CW_BODY_INDENT << "_compilerClauseSuccess = ";
    for (std::vector<std::string>::const_iterator i = _listOfConstants.begin();
      ...
      //				if (_bNoCase) {
      //					CW_BODY_STREAM <<
      //"stricmp(_compilerClauseText_" << iCursor << ".c_str(), ";
...
BNFFindToken::toString() const
{
  ...
  if (hasIntermediateCode()) {
    std::string sConstVar =
      DtaBNFScript::constantsToString(_listOfIntermediateConstants) +
      DtaBNFScript::assignmentToString(_pIntermediateVariableToAssign,
                                       _bConcatVariable);
    if (!_listOfFinalConstants.empty() || (_pFinalVariableToAssign != NULL)) {
      sConstVar += "-" +
                   DtaBNFScript::constantsToString(_listOfFinalConstants) +
                   DtaBNFScript::assignmentToString(_pFinalVariableToAssign,
                                                    _bConcatVariable);
    }
    sText += "(" + sConstVar;
    if (_pIntermediateSequence != NULL) {
  ...
  sText +=
    getCommands()[0]->toString() +
    DtaBNFScript::constantsToString(_listOfConstants) +
