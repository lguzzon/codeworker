...
void
GrfLoadProject::populateDefaultParameters()
{
  if (_pNodeToLoad == NULL)
    _pNodeToLoad = new ExprScriptVariable("project");
}

//##protect##"implementation"
void
GrfLoadProject::parseTextFile(std::istream& theStream, DtaScriptVariable* pNode)
{
  std::string sNodeName;
  if (!CodeWorker::readString(theStream, sNodeName)) {
    throw UtlException(theStream,
                       "complete name of the parse tree to load is expected "
...
bool
GrfLoadProject::populateConstantTree(std::istream& theStream,
                                     DtaScriptVariable& theNode)
{
  std::string sValue;
  CodeWorker::skipEmpty(theStream);
  if (CodeWorker::isEqualTo(theStream, '&')) {
    ...
    if (!CodeWorker::isEqualToIdentifier(theStream, "ref")) {
      throw UtlException(theStream,
                         "reference node should be announced by '&ref'");
    }
    CodeWorker::skipEmpty(theStream);
    std::string sReferenceNode;
    if (!CodeWorker::readString(theStream, sReferenceNode)) {
      throw UtlException(theStream,
                         "reference node expected between double quotes");
    }
    CodeWorker::skipEmpty(theStream);
    if (CodeWorker::isEqualTo(theStream, '=')) {
      CodeWorker::skipEmpty(theStream);
      if (!CodeWorker::readString(theStream, sValue)) {
        throw UtlException(
          theStream, "syntax error: value expected between double quotes");
      }
    }
  } else {
  ...
  if (!CodeWorker::isEqualTo(theStream, '}')) {
    do {
      ...
      if (CodeWorker::isEqualTo(theStream, '.')) {
        ...
        populateConstantTree(theStream,
                             *theNode.insertNode(sAttribute.c_str()));
      } else if (CodeWorker::isEqualTo(theStream, '[')) {
        do {
          CodeWorker::skipEmpty(theStream);
          if (CodeWorker::readString(theStream, sValue) &&
              skipEmpty(theStream) && CodeWorker::isEqualTo(theStream, ':')) {
            // "key" : "value" or "node"
...
void
GrfLoadProject::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
