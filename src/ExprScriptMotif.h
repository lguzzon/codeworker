...
public:
  ...
  virtual DtaScriptVariable* filterNodes(
    DtaScriptVariable& visibility,
    DtaScriptVariable& currentNode,
    std::list<DtaScriptVariable*>& listOfNodes) const = 0;
};
...
public:
  inline ExprScriptMotifPath()
    : _pPrecedentPath(NULL)
    , _pNextPath(NULL)
  {}
  ...
  inline ExprScriptMotifPath* getPrecedentPath() const
  {
    return _pPrecedentPath;
  }
  inline ExprScriptMotifPath* getNextPath() const { return _pNextPath; }
  inline void setNextPath(ExprScriptMotifPath* pNextPath)
  {
...
public:
  inline ExprScriptMotifStep(const std::string& sName)
    : _sName(sName)
  {}
...
{
public:
  inline ExprScriptMotifEllipsis(ExprScriptMotifPath* pLeft,
                                 ExprScriptMotifPath* pRight)
    : ExprScriptMotifPath(pLeft, pRight)
  {}
...
public:
  inline ExprScriptMotifArray(ExprScriptMotifPath* pPrec,
                              ExprScriptExpression* pPosition = NULL)
    : ExprScriptMotifPath(pPrec)
...
public:
  inline ExprScriptMotifBoolean(char cOperator, ExprScriptMotif* pMember)
  ...
  inline void addMember(ExprScriptMotif* pMember)
  {
    _members.push_back(pMember);
  }
