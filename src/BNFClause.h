...
struct BNFClauseMatchingArea
{
  ...
  BNFClauseMatchingArea(BNFClause* p, int i)
    : clause(p)
    , beginPosition(i)
    , endPosition(-1)
  {}
  ...
  inline void pushChild(BNFClauseMatchingArea* pChild)
  {
...
public:
  BNFClause(DtaBNFScript* pBNFScript,
            GrfBlock* pParent,
  ...
  {
    return (_pTemplateClause != NULL) ||
           !_mapOfTemplateInstantiations.empty() ||
           (_pGenericTemplateClause != NULL);
  }
  inline BNFClause* getGenericTemplateClause() const
  {
  ...
  inline const std::vector<std::string>& getParameters() const
  {
    return _parameters;
  }
  ...
  inline const EXPRESSION_TYPE getParameterType(int i) const
  {
  ...
  inline int /*IGNORE_MODE*/ getPreprocessingIgnoreMode() const
  {
    return _iPreprocessingIgnoreMode;
  }
  void setPreprocessingIgnoreMode(int /*IGNORE_MODE*/ iPreprocessingIgnoreMode,
  ...
  virtual SEQUENCE_INTERRUPTION_LIST executeClause(
    DtaScriptVariable& visibility,
    int iSuperCallDepth);

  virtual std::string toString() const;
  virtual void compileCppFunction(
    CppCompilerEnvironment& theCompilerEnvironment) const;

private:
  SEQUENCE_INTERRUPTION_LIST executeInternalSuperClause(
    DtaScriptVariable& visibility,
    int& iSuperCallDepth);
};
}
