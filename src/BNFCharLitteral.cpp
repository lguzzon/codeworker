...
BNFCharLitteral::BNFCharLitteral(DtaBNFScript* pBNFScript,
                                 GrfBlock* pParent,
                                 int iChar,
                                 bool bContinue,
                                 bool bNoCase)
...
{
  if (_bNoCase) {
    if ((_iChar >= (int)'A') && (_iChar <= (int)'Z'))
      _iChar += 32;
    else if ((iChar < (int)'a') || (iChar > (int)'z'))
      _bNoCase = false;
  }
}
...
void
BNFCharLitteral::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFCharLitteral(*this, env);
}

bool
BNFCharLitteral::isABNFCommand() const
{
  return true;
}

void
BNFCharLitteral::setVariableToAssign(ExprScriptVariable* pVariableToAssign,
                                     bool bConcat,
                                     BNFClause& theClause)
{
...
BNFCharLitteral::executeInternal(DtaScriptVariable& visibility)
{
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "int " << tcChar << " = CGRuntime::readCharAsInt();";
  CW_BODY_ENDL;
  if (_bNoCase) {
  ...
  if (_pVariableToAssign != NULL) {
    ...
    CW_BODY_ENDL;
    CW_BODY_INDENT << "\ttcText[0] = (char) " << tcChar << ";";
    CW_BODY_ENDL;
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\t\tunsigned char cChar = (unsigned char) " << tcChar
                 << ";";
  CW_BODY_ENDL;
...
std::string
BNFCharLitteral::toString() const
{
  std::string sString;
  if (_bContinue)
    sString = "#continue ";
