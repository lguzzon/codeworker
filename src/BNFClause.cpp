...
{
  for (std::list<BNFClauseMatchingArea*>::iterator i = childs.begin();
       i != childs.end();
       ++i) {
    delete *i;
  }
...
void
BNFClauseMatchingArea::purgeChildsAfterPosition(int iPosition)
{
...
public:
  BNFClauseMatchingAreaValidator(BNFClause* pClause, DtaBNFScript* pBNFScript)
  {
    if (!pBNFScript->hasCoverageRecording()) {
      ...
      pBNFScript->setParentClauseMatching(
        new BNFClauseMatchingArea(pClause, CGRuntime::getInputLocation()));
    }
  }
...
BNFClause::BNFClause(DtaBNFScript* pBNFScript,
                     GrfBlock* pParent,
  ...
  , _bGenericKey(bGenericKey)
  , _pTemplateClause(NULL)
  , _pGenericTemplateClause(NULL)
  , _iReturnType(0)
  , _parameters(listOfParameters)
  , _parameterTypes(listOfParameterTypes)
  , _pOverloadClause(NULL)
  , _bPropagatedParameters(false)
  , _iPreprocessingIgnoreMode(0 /*UNDEFINED_IGNORE*/)
  , _pPreprocessingIgnoreClause(NULL)
{}
...
void
BNFClause::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFClause(*this, env);
}

void
BNFClause::setPreprocessingIgnoreMode(
  int /*IGNORE_MODE*/ iPreprocessingIgnoreMode,
...
bool
BNFClause::isABNFCommand() const
{
  return true;
}

void
BNFClause::setTemplateClause(BNFClause* pTemplateClause)
{
  _pTemplateClause = pTemplateClause;
  if (isGenericKey())
    _pTemplateClause->_pGenericTemplateClause = this;
  else
    _pTemplateClause->_mapOfTemplateInstantiations[_sTemplateKey] = this;
}

BNFClause*
BNFClause::getInstantiatedClause(const std::string& sInstantiationKey) const
{
  std::map<std::string, BNFClause*>::const_iterator cursor =
    _mapOfTemplateInstantiations.find(sInstantiationKey);
  if (cursor == _mapOfTemplateInstantiations.end()) {
    if (sInstantiationKey.empty() && (getNbCommands() != 0))
      return const_cast<BNFClause*>(this);
...
void
BNFClause::setParameter(int i,
                        const std::string& sParameter,
                        EXPRESSION_TYPE iType)
{
...
void
BNFClause::setOverloadClause(BNFClause* pOverloadClause)
{
  if (_pOverloadClause != NULL)
    _pOverloadClause->setOverloadClause(pOverloadClause);
  else
    _pOverloadClause = pOverloadClause;
}

bool
BNFClause::propagateParameters(
  ExprScriptExpression& theFilter,
  const std::string& sFunctionQuantity,
  const std::vector<std::string>& listOfParameters,
  const std::vector<EXPRESSION_TYPE>& listOfParameterTypes)
{
  ...
  if (bResult) {
    ...
    if (listOfParameters.size() < getArity())
      throw UtlException("while propagating parameters on non terminals, not "
                         "enough parameters assigned to " +
                         sFunctionQuantity + "='" + sSignature + "'");
    std::string::size_type i;
    for (i = 0; i < getArity(); ++i) {
      if (getParameterType(i) != listOfParameterTypes[i])
        throw UtlException("while propagating parameters on non terminals, "
                           "incompatible parameter type assigned to " +
                           sFunctionQuantity + "='" + sSignature +
  ...
  if ((_pGenericTemplateClause != NULL) &&
      _pGenericTemplateClause->propagateParameters(
        theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes))
    bResult = true;
  for (std::map<std::string, BNFClause*>::iterator i =
         _mapOfTemplateInstantiations.begin();
       i != _mapOfTemplateInstantiations.end();
       ++i) {
    if (i->second->propagateParameters(
          theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes))
      bResult = true;
  }
  if ((_pOverloadClause != NULL) &&
      _pOverloadClause->propagateParameters(
        theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes))
    bResult = true;
  if (bResult && !_bPropagatedParameters) {
    ExprScriptConstant alwaysTrue(true);
    propagateParameters(
      alwaysTrue, sFunctionQuantity, listOfParameters, listOfParameterTypes);
  }
  return bResult;
}

bool
BNFClause::addBNFLocalVariable(const std::string& sVarName,
...
EXPRESSION_TYPE
BNFClause::getLocalVariable(const std::string& sVarName) const
{
  EXPRESSION_TYPE result = GrfBlock::getLocalVariable(sVarName);
  if (result == UNKNOWN_EXPRTYPE) {
    int iIndex = 0;
    for (std::vector<std::string>::const_iterator i = _parameters.begin();
         i != _parameters.end();
         i++) {
      if ((*i) == sVarName) {
...
EXPRESSION_TYPE
BNFClause::getVariable(const std::string& sVarName) const
{
  EXPRESSION_TYPE result = getLocalVariable(sVarName);
  if (result == UNKNOWN_EXPRTYPE) {
    if ((sVarName == "this") || (sVarName == "project") ||
        (sVarName == "null") ||
...
BNFClause::execute(DtaScriptVariable& visibility)
{
  throw UtlException("internal error: call 'BNFClause::executeClause(..., 0)' "
                     "instead of 'BNFClause::execute(...)'");
}

SEQUENCE_INTERRUPTION_LIST
BNFClause::executeClause(DtaScriptVariable& visibility, int iSuperCallDepth)
{
  ...
  if (iSuperCallDepth == 0) {
    ...
    while (pLastOverloadClause->_pOverloadClause != NULL)
      pLastOverloadClause = pLastOverloadClause->_pOverloadClause;
    ...
    if (iSuperCallDepth > 0)
      throw UtlException(
        "'#super' has failed: no overloaded non-terminal found for '" +
        getSignature() + "'");
  }
...
SEQUENCE_INTERRUPTION_LIST
BNFClause::executeInternalSuperClause(DtaScriptVariable& visibility,
                                      int& iSuperCallDepth)
{
  SEQUENCE_INTERRUPTION_LIST result;
  if (_pOverloadClause != NULL) {
    result =
      _pOverloadClause->executeInternalSuperClause(visibility, iSuperCallDepth);
    iSuperCallDepth--;
    if (iSuperCallDepth == 0)
      result = GrfBlock::execute(visibility);
  } else {
    result = NO_INTERRUPTION;
  }
  return result;
...
BNFClause::getSignature() const
{
    ...
    for (std::vector<std::string>::const_iterator i = _parameters.begin();
      ...
      switch (exprType & 0x00FF) {
        ...
        case NODE_EXPRTYPE:
          ...
          break;
        default:
          throw UtlException("internal error in BNFClause::getSignature(): "
  ...
  switch (getReturnType()) {
    case 0 /*NO_RETURN_TYPE*/:
      break;
    case 1 /*LIST_RETURN_TYPE*/:
      sText += " : list";
      break;
    case 2 /*NODE_RETURN_TYPE*/:
      sText += " : node";
      break;
    case 3 /*VALUE_RETURN_TYPE*/:
      sText += " : value";
      break;
    default:
      throw UtlException(
        "internal error in BNFClause::toString(): unhandled enum");
  }
  return sText;
}

std::string
BNFClause::toString() const
{
  ...
  if (getNbCommands() == 1) {
    ...
    for (std::vector<GrfCommand*>::const_iterator i = getCommands().begin();
         i != getCommands().end();
         i++) {
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  int iPointerToDeclarations =
    theCompilerEnvironment.getPointerToDeclarations();
  ...
  if (isATemplateInstantiation()) {
    if (_bGenericKey)
      sClauseName =
        "_compilerTemplateClause_" + sClauseName + "_compilerGeneric";
    else
      sClauseName = "_compilerTemplateClause_" + sClauseName +
                    "_compilerInstantiation_" +
                    theCompilerEnvironment.convertTemplateKey(_sTemplateKey);
  ...
  if (isATemplateInstantiation() && _sTemplateKey.empty()) {
    ...
    theCompilerEnvironment.getHeader()
      << theCompilerEnvironment.getIndentation() << "\t\tstatic bool "
      << sClauseName
      << "(const std::string& _compilerTemplateClause_dispatching, ";
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      switch (getParameterType(iIndex) & 0x00FF) {
        case VALUE_EXPRTYPE:
          theCompilerEnvironment.getHeader()
            << "CodeWorker::CppParsingTree_value ";
          break;
        case ITERATOR_EXPRTYPE:
        case REFERENCE_EXPRTYPE:
          theCompilerEnvironment.getHeader()
            << "const CodeWorker::CppParsingTree_var& ";
          break;
        case NODE_EXPRTYPE:
          theCompilerEnvironment.getHeader()
            << "const CodeWorker::CppParsingTree_var& ";
        ...
        default:
          throw UtlException(
            "internal error in BNFClause::compileCppFunction(): unhandled "
            "parameter type");
      }
      theCompilerEnvironment.getHeader() << (*i) << ", ";
    }
    theCompilerEnvironment.getHeader()
      << "CodeWorker::CGBNFRuntimeEnvironment& theEnvironment);";
    theCompilerEnvironment.getHeader().endl();
    iIndex = 0;
    theCompilerEnvironment.getHeader()
      << theCompilerEnvironment.getIndentation() << "\t\tstatic bool "
      << sClauseName
      << "(const std::string& _compilerTemplateClause_dispatching, const "
         "CodeWorker::CppParsingTree_var& _compilerClause_returnValue, ";
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      EXPRESSION_TYPE exprType = getParameterType(iIndex);
      if ((exprType & ~0x00FF) == 0) {
        ...
        DtaClass* pTypeSpecifier = DtaClass::getClass(exprType);
        theCompilerEnvironment.getHeader()
          << pTypeSpecifier->getCppTypeSpecifier() << " ";
      }
      theCompilerEnvironment.getHeader() << (*i) << ", ";
    }
    theCompilerEnvironment.getHeader()
  ...
  theCompilerEnvironment.getHeader()
    << theCompilerEnvironment.getIndentation() << "\t\tstatic bool "
    << sClauseName
    << "(const CodeWorker::CppParsingTree_var& _compilerClause_returnValue, ";
  for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
    EXPRESSION_TYPE exprType = getParameterType(iIndex);
    if ((exprType & ~0x00FF) == 0) {
      switch (exprType) {
  ...
  if (isATemplateInstantiation() && _sTemplateKey.empty()) {
    ...
    CW_BODY_INDENT
      << "bool "
      << DtaScript::convertFilenameAsIdentifier(
           theCompilerEnvironment.getRadical())
      << "::" << sClauseName
      << "(const std::string& _compilerTemplateClause_dispatching, ";
    iIndex = 0;
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      ...
      if ((exprType & ~0x00FF) == 0) {
        switch (exprType) {
          case VALUE_EXPRTYPE:
            CW_BODY_STREAM << "CppParsingTree_value ";
            break;
          case ITERATOR_EXPRTYPE:
          case REFERENCE_EXPRTYPE:
            CW_BODY_STREAM << "const CppParsingTree_var& ";
            break;
          case NODE_EXPRTYPE:
            CW_BODY_STREAM << "const CppParsingTree_var& ";
            break;
    ...
    CW_BODY_STREAM
      << "	return " << sClauseName
      << "(_compilerTemplateClause_dispatching, _compilerClauseThis, ";
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      CW_BODY_STREAM << (*i) << ", ";
    }
    ...
    CW_BODY_INDENT
      ...
      << "::" << sClauseName
      << "(const std::string& _compilerTemplateClause_dispatching, const "
         "CppParsingTree_var& _compilerClause_returnValue, ";
    iIndex = 0;
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      EXPRESSION_TYPE exprType = getParameterType(iIndex);
      if ((exprType & ~0x00FF) == 0) {
    ...
    if (_mapOfTemplateInstantiations.size() < 8) {
      // just a battery of if/else on the key for determining the correct
      // function call
      for (std::map<std::string, BNFClause*>::const_iterator j =
             _mapOfTemplateInstantiations.begin();
           j != _mapOfTemplateInstantiations.end();
           ++j) {
        CW_BODY_STREAM << "\t";
        if (j != _mapOfTemplateInstantiations.begin())
          CW_BODY_STREAM << "else ";
        CW_BODY_STREAM << "if (_compilerTemplateClause_dispatching == ";
        CW_BODY_STREAM.writeString(j->first);
        CW_BODY_STREAM << ") return " << sClauseName
                       << theCompilerEnvironment.convertTemplateKey(j->first)
                       << "(_compilerClause_returnValue, ";
        for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
          CW_BODY_STREAM << (*i) << ", ";
        }
        CW_BODY_STREAM << "theEnvironment);";
      ...
      for (std::map<std::string, BNFClause*>::const_iterator j =
        ...
        while (*u != '\0')
          iHashCode = iHashCode * 31 + (((long)*u++) % 31);
        keysByCode[iHashCode].push_back(j->first);
      }
      CW_BODY_STREAM << "\tlong _compilerInternal_hashCode = 0;";
      CW_BODY_ENDL;
      CW_BODY_STREAM << "\tchar* _compilerInternal_u = (char*) "
                        "(_compilerTemplateClause_dispatching.c_str());";
      CW_BODY_ENDL;
      CW_BODY_STREAM
        << "\twhile (*_compilerInternal_u != '\\0') _compilerInternal_hashCode "
           "= _compilerInternal_hashCode * 31 + (((long) "
           "*_compilerInternal_u++) % 31);";
      CW_BODY_ENDL;
      CW_BODY_STREAM << "\tswitch(_compilerInternal_hashCode) {";
      CW_BODY_ENDL;
      for (std::map<long, std::list<std::string>>::const_iterator k =
             keysByCode.begin();
           k != keysByCode.end();
           ++k) {
        CW_BODY_STREAM << "\t\tcase " << k->first << ": ";
        CW_BODY_ENDL;
        for (std::list<std::string>::const_iterator l = k->second.begin();
             l != k->second.end();
             ++l) {
          CW_BODY_STREAM << "\t\t\tif (_compilerTemplateClause_dispatching == ";
          CW_BODY_STREAM.writeString(*l);
          CW_BODY_STREAM << ") return " << sClauseName
                         << theCompilerEnvironment.convertTemplateKey(*l)
                         << "(_compilerClause_returnValue, ";
          for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
            CW_BODY_STREAM << (*i) << ", ";
          }
    ...
    if (_pGenericTemplateClause != NULL) {
      CW_BODY_STREAM << "\treturn _compilerTemplateClause_" + getName() +
                          "_compilerGeneric(_compilerClause_returnValue, ";
      for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
        CW_BODY_STREAM << (*i) << ", ";
      }
      CW_BODY_STREAM << "theEnvironment);";
      ...
      CW_BODY_STREAM << "\tif (!_compilerTemplateClause_dispatching.empty()) "
                        "throw UtlException(\"template clause '"
                     << getName()
                     << "<\\\"\" + _compilerTemplateClause_dispatching + "
                        "\"\\\">' hasn't been implemented\");";
      CW_BODY_ENDL;
      if (getNbCommands() == 0) {
        CW_BODY_STREAM << "\treturn true;";
        CW_BODY_ENDL;
      } else {
        CW_BODY_STREAM << "\treturn " << sClauseName
                       << "(_compilerClause_returnValue, ";
  ...
  CW_BODY_INDENT << "bool "
                 << DtaScript::convertFilenameAsIdentifier(
                      theCompilerEnvironment.getRadical())
                 << "::" << sClauseName
                 << "(const CppParsingTree_var& _compilerClause_returnValue, ";
  iIndex = 0;
  for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
    EXPRESSION_TYPE exprType = getParameterType(iIndex);
    if ((exprType & ~0x00FF) == 0) {
  ...
  CW_BODY_STREAM << "CGBNFRuntimeEnvironment& theEnvironment) {";
  CW_BODY_ENDL;
  theCompilerEnvironment.setPointerToDeclarations(
    CW_BODY_STREAM.getOutputLocation());
  ...
  if (_pBNFScript->hasCoverageRecording()) {
    ...
    CW_BODY_INDENT
      << "CGBNFRuntimeClauseMatchingAreaValidator _compilerClauseMatching(\""
      << CGRuntime::composeCLikeString(getSignature())
      << "\", &theEnvironment);";
    CW_BODY_ENDL;
  }
  CW_BODY_INDENT << "CGBNFClauseScope _compilerClauseScope;";
  CW_BODY_ENDL;
  CW_BODY_STREAM.newFloatingLocation("CLAUSE SCOPE DECLARATION");
  CW_BODY_INDENT << "bool _compilerClauseSuccess = true;";
  CW_BODY_ENDL;
  for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
  ...
  if (theCompilerEnvironment.hasEvaluatedExpressionInScope()) {
    for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
      std::string sDynamicVariable = theCompilerEnvironment.getIndentation() +
                                     "_compilerClauseScope.insertNode(\"" + *i +
                                     "\").setReference(" + *i + ");" +
                                     ScpStream::ENDL;
      ScpStream* pOwner = 0;
      CW_BODY_STREAM.insertText(
        sDynamicVariable,
        CW_BODY_STREAM.getFloatingLocation("CLAUSE SCOPE DECLARATION", pOwner));
    }
  }
  if (_pBNFScript->hasCoverageRecording()) {
    // coverage recording required
    CW_BODY_INDENT
      << "if (_compilerClauseSuccess) _compilerClauseMatching.validate();";
    CW_BODY_ENDL;
  }
  CW_BODY_INDENT << "return _compilerClauseSuccess;";
  CW_BODY_ENDL;
