...
bool BNFClause::isABNFCommand() const
{
    return true;
}
...
bool BNFClause::propagateParameters(ExprScriptExpression& theFilter, const std::string& sFunctionQuantity, const std::vector<std::string>& listOfParameters, const std::vector<EXPRESSION_TYPE>& listOfParameterTypes)
{
    ...
    if (bResult) {
        ...
        if (listOfParameters.size() < getArity()) {
            throw UtlException("while propagating parameters on non terminals, not enough parameters assigned to " + sFunctionQuantity + "='" + sSignature + "'");
        }
        ...
        for (i = 0; i < getArity(); ++i) {
            if (getParameterType(i) != listOfParameterTypes[i]) {
                throw UtlException("while propagating parameters on non terminals, incompatible parameter type assigned to " + sFunctionQuantity + "='" + sSignature + "'/parameter='" + listOfParameters[i] + "'");
            }
        }
    ...
    if ((_pGenericTemplateClause != NULL) && _pGenericTemplateClause->propagateParameters(theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes)) {
        bResult = true;
    }

    for (std::map<std::string, BNFClause*>::iterator i = _mapOfTemplateInstantiations.begin(); i != _mapOfTemplateInstantiations.end(); ++i) {
        if (i->second->propagateParameters(theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes)) {
            bResult = true;
        }
    }

    if ((_pOverloadClause != NULL) && _pOverloadClause->propagateParameters(theFilter, sFunctionQuantity, listOfParameters, listOfParameterTypes)) {
        bResult = true;
    }
...
EXPRESSION_TYPE BNFClause::getVariable(const std::string& sVarName) const
{
    ...
    if (result == UNKNOWN_EXPRTYPE) {
        if ((sVarName == "this") || (sVarName == "project") || (sVarName == "null") || (DtaProject::getInstance().getGlobalVariableType(sVarName) != UNKNOWN_EXPRTYPE)) {
...
SEQUENCE_INTERRUPTION_LIST BNFClause::executeClause(DtaScriptVariable& visibility, int iSuperCallDepth)
{
    ...
    if (iSuperCallDepth == 0) {
        ...
        if (iSuperCallDepth > 0) {
            throw UtlException("'#super' has failed: no overloaded non-terminal found for '" + getSignature() + "'");
        }
    }
...
void BNFClause::compileCppFunction(CppCompilerEnvironment& theCompilerEnvironment) const
{
    ...
    if (isATemplateInstantiation()) {
        if (_bGenericKey) {
            sClauseName = "_compilerTemplateClause_" + sClauseName + "_compilerGeneric";
        } else {
            sClauseName = "_compilerTemplateClause_" + sClauseName + "_compilerInstantiation_" + theCompilerEnvironment.convertTemplateKey(_sTemplateKey);
        }
    ...
    if (isATemplateInstantiation() && _sTemplateKey.empty()) {
        ...
        for (i = _parameters.begin(); i != _parameters.end(); i++, iIndex++) {
            ...
            case VALUE_EXPRTYPE:
                theCompilerEnvironment.getHeader() << "CodeWorker::CppParsingTree_value ";
                break;

            case ITERATOR_EXPRTYPE:
            case REFERENCE_EXPRTYPE:
                theCompilerEnvironment.getHeader() << "const CodeWorker::CppParsingTree_var& ";
                break;
    ...
    if (isATemplateInstantiation() && _sTemplateKey.empty()) {
        ...
        if (_pGenericTemplateClause != NULL) {
            ...
            CW_BODY_STREAM << "\tif (!_compilerTemplateClause_dispatching.empty()) throw UtlException(\"template clause '" << getName() << "<\\\"\" + _compilerTemplateClause_dispatching + \"\\\">' hasn't been implemented\");";
            CW_BODY_ENDL;
    ...
    if (_pBNFScript->hasCoverageRecording()) {
        ...
        CW_BODY_INDENT << "CGBNFRuntimeClauseMatchingAreaValidator _compilerClauseMatching(\"" << CGRuntime::composeCLikeString(getSignature()) << "\", &theEnvironment);";
        CW_BODY_ENDL;
    }
    ...
    if (_pBNFScript->hasCoverageRecording()) {
        // coverage recording required
        CW_BODY_INDENT << "if (_compilerClauseSuccess) _compilerClauseMatching.validate();";
        CW_BODY_ENDL;
    }
