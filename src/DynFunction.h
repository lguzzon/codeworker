...
public:
  inline DynFunction(Interpreter* pInterpreter, const std::string& sName)
    : GrfFunction(NULL, sName, "", false)
    , _pInterpreter(pInterpreter)
  {
    init();
  }
  DynFunction(Interpreter* pInterpreter, const char* tcName, bool bIsP1Node);
  DynFunction(Interpreter* pInterpreter,
              const char* tcName,
              bool bIsP1Node,
              bool bIsP2Node);
  DynFunction(Interpreter* pInterpreter,
  ...
  inline void setFunctionCall(void* pfFunctionCall)
  {
    _pfFunctionCall = pfFunctionCall;
  }
