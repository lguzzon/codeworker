...
BNFScanWindow::BNFScanWindow(DtaBNFScript* pBNFScript,
                             GrfBlock* pParent,
                             bool bContinue)
  : _pBNFScript(pBNFScript)
  , GrfBlock(pParent)
  , _bContinue(bContinue)
  , _pWindowSequence(NULL)
{}
...
void
BNFScanWindow::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFScanWindow(*this, env);
}

bool
BNFScanWindow::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFScanWindow::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if (result != CONTINUE_INTERRUPTION) {
    ...
    if (result == NO_INTERRUPTION) {
      ...
      if (iFinalLocation < iLocation)
        throw UtlException("Left member of '" +
                           CGRuntime::composeCLikeString(toString()) +
                           "' shouldn't set the cursor back in the sentence");
      if (_bContinue)
...
void
BNFScanWindow::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  CPP_COMPILER_BNF_SYMBOL_BEGIN;
  _pWindowSequence->compileCpp(theCompilerEnvironment);
  CW_BODY_INDENT << "int _compilerClauseFinalLocation" << iCursor
  ...
  CW_BODY_INDENT << "if (" << tcImplicitCopy
                 ...
                 << ");";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "if (_compilerClauseFinalLocation" << iCursor << " < "
                 << tcLocation << ") _compilerClauseSuccess = false;";
  CW_BODY_ENDL;
  ...
  CW_BODY_ENDL;
  theCompilerEnvironment.incrementIndentation();
  CW_BODY_INDENT << "CGBNFRuntimeResizeInput "
                    "_compilerClauseSizeAttrs(_compilerClauseFinalLocation"
                 << iCursor << ");";
  CW_BODY_ENDL;
  ...
  CW_BODY_INDENT << "if (_compilerClauseSuccess) {";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\tCGRuntime::setInputLocation(_compilerClauseFinalLocation"
                 << iCursor << ");";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "} else {";
  CW_BODY_ENDL;
  ...
  CW_BODY_ENDL;
  CW_BODY_INDENT << "\tif (_compilerClauseFinalLocation" << iCursor << " < "
                 << tcLocation << ") throw UtlException(\"Left member of '"
                 << CGRuntime::composeCLikeString(toString())
                 << "' shouldn't set the cursor back in the sentence\");";
  CW_BODY_ENDL;
  if (_bContinue) {
...
std::string
BNFScanWindow::toString() const
{
  std::string sText = "[" + _pWindowSequence->toString() + "] |> [" +
