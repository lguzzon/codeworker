...
public:
  inline DtaScriptVariableList(DtaScriptVariable* pNode = NULL,
                               DtaScriptVariableList* pNext = NULL)
    : _pNode(pNode)
    , _pNext(pNext)
  {}
...
private:
  ...
  enum VALUE_NODE_TYPE
  {
    NO_VALUE_NODE = 0,
    STRING_VALUE_NODE = 1,
    REFERENCE_VALUE_NODE = 2,
    INDEX_VALUE_NODE = 6,
    EXTERNAL_VALUE_NODE = 8
  };
...
public:
  ...
  inline ExternalValueNode* getExternalValueNode() const
  {
    return ((_eValueNode == EXTERNAL_VALUE_NODE) ? _value.ext : NULL);
  }
  ...
  DtaScriptVariable* getVariableForReferenceAssignment(
    const ExprScriptVariable& exprVariable);
  DtaScriptVariable* getExistingVariable(
    const ExprScriptVariable& exprVariable);
  ...
  DtaScriptVariable* getVariable(const char* sName,
                                 ...
                                 bool bCreateIfUnknown = true,
                                 bool bReferencedVariable = true);
  ...
  void setValueAtVariable(const char* sVariable,
                          const char* sValue,
  ...
  void slideNodeContent(DtaScriptVariable& visibility,
                        const ExprScriptVariable& varExpr);

  bool traceValue(int iDepth = 0,
