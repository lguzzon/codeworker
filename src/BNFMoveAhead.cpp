...
BNFMoveAhead::BNFMoveAhead(GrfBlock* pParent, bool bContinue)
  : GrfBlock(pParent)
...
void
BNFMoveAhead::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFMoveAhead(*this, env);
}

bool
BNFMoveAhead::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFMoveAhead::executeInternal(DtaScriptVariable& visibility)
{
...
void
BNFMoveAhead::compileCpp(CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  if (i != getCommands().end()) {
    ...
    CW_BODY_INDENT << "if (_compilerClauseSuccess && (" << tcLocation
                   << " == CGRuntime::getInputLocation())) {";
    CW_BODY_ENDL;
    if (_bContinue) {
