...
GrfJointPointCall::executeInternal(DtaScriptVariable& visibility)
{
  ...
  if (pContext != NULL) {
    if (CGRuntime::getActiveJointPoint() == NULL) {
      THROW_UTLEXCEPTION(
        "cannot execute a '#jointcut' directive out of an advice");
    }
    std::string sLabel =
      "#jointpoint '" + CGRuntime::getActiveJointPoint()->getName() + "'";
