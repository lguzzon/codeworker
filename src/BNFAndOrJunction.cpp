...
BNFAndOrJunction::BNFAndOrJunction(DtaBNFScript* pBNFScript,
                                   GrfBlock* pParent,
                                   bool bContinue)
...
void
BNFAndOrJunction::accept(DtaVisitor& visitor, DtaVisitorEnvironment& env)
{
  visitor.visitBNFAndOrJunction(*this, env);
}

bool
BNFAndOrJunction::isABNFCommand() const
{
  return true;
}

SEQUENCE_INTERRUPTION_LIST
BNFAndOrJunction::executeInternal(DtaScriptVariable& visibility)
{
  ...
  CppCompilerEnvironment& theCompilerEnvironment) const
{
  ...
  CW_BODY_INDENT << "bool _compilerClauseSuccess" << iCursor
                 << " = _compilerClauseSuccess;";
  CW_BODY_ENDL;
  ...
  CW_BODY_INDENT << "_compilerClauseSuccess = _compilerClauseSuccess" << iCursor
                 << ";";
  CW_BODY_ENDL;
  CW_BODY_INDENT << "if (!_compilerClauseSuccess) {";
  CW_BODY_ENDL;
...
std::string
BNFAndOrJunction::toString() const
{
  std::string sText =
    _pLeftMember->toString() + " &| " + getCommands()[0]->toString();
  if (_bContinue)
