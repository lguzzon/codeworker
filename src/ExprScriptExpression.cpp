...
DtaScriptVariable* ExprScriptExpression::getReferencedVariable() const
{
    return NULL;
}
...
bool ExprScriptExpression::compileCppBoolean(CppCompilerEnvironment& theCompilerEnvironment, bool bNegative) const
{
    ...
    case STRING_TYPE:
        if (bNegative) {
            CW_BODY_STREAM.insertText("!CGRuntime::toBoolean(", iLocation + (CW_BODY_STREAM.getFloatingLocation("INSERT AREA", pOwner) - iInsertAreaLocation));
        } else {
            CW_BODY_STREAM.insertText("CGRuntime::toBoolean(", iLocation + (CW_BODY_STREAM.getFloatingLocation("INSERT AREA", pOwner) - iInsertAreaLocation));
        }
...
DtaScriptVariable* ExprScriptReference::getReferencedVariable() const
{
    return _pVariable;
}
...
std::string ExprScriptConstant::getValue(DtaScriptVariable&) const
{
    return _sValue;
}
double ExprScriptConstant::getDoubleValue(DtaScriptVariable& visibility) const
{
    return atof(_sValue.c_str());
}
int ExprScriptConstant::getIntValue(DtaScriptVariable& visibility) const
{
...
std::string ExprScriptAlien::getValue(DtaScriptVariable&) const
{
    throw UtlException("internal error: it is forbidden to execute an alien expression");
}
double ExprScriptAlien::getDoubleValue(DtaScriptVariable& visibility) const
{
    throw UtlException("internal error: it is forbidden to execute an alien expression");
}
int ExprScriptAlien::getIntValue(DtaScriptVariable& visibility) const
{
    throw UtlException("internal error: it is forbidden to execute an alien expression");
}
